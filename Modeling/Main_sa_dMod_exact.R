...
out_dir <- paste0(out_dir, '/Profile_dMod_optimize')
dir.create(file.path(out_dir), showWarnings = FALSE)
...
out_dir_path <- paste0(scratch_dir,'/',folder,'/Profile_dMod_optimize/', condition)
...
if (!file.exists(paste0(out_dir_path,'/sa_bfgs_merged', gene_name, '_Finished.Rdata',sep=""))){
  ...
  # for (replicate in c('rep1', 'rep2', 'all', 'rep3')){
    if (!file.exists(paste0(out_dir_path,'/profile_dMod_', replicate, '_', gene_name, '_Finished.Rdata',sep=""))){
      ...
      #   
      #   val <- unname(TSE_bs_cp(par = pars_new_ordered[-which(par_names %in% c("k1'/k2", "k1'k2'/k2"))], fn_fit=fn_all_ode_cp, data = data_rpkm_all, time_data=time_data, convert_param=T, log_in=T, u=input_fun_cp, replicate=replicate, results = "sum", library_size = lib_mat, length = length))
      #   gradient_val <- TSE_bs_cp_grad(x = pars_new_ordered[-which(par_names %in% c("k1'/k2", "k1'k2'/k2"))], fn_fit=fn_all_ode_cp, data = data_rpkm_all, time_data=time_data, convert_param=T, log_in=T, u=input_fun_cp, replicate=replicate, results = "sum", library_size = lib_mat, length = length)
      ...
      #   }
      #   
      #   hessian_val <- TSE_bs_cp_hessian(x = pars_new_ordered[-which(par_names %in% c("k1'/k2", "k1'k2'/k2"))],fn_fit=fn_all_ode_cp, data = data_rpkm_all, time_data=time_data, convert_param=T, log_in=T, u=input_fun_cp, replicate=replicate, results = "sum", library_size = lib_mat, length = length)
      ...
      obj_vgh_main <- function(pars, fixed = NULL, scale=2, no_np = F, no_ca0 = F ){
        ...
        val <- unname(TSE_bs_oldpar(x = pars_ordered, fn_fit=fn_all_ode_cp, data = data_rpkm_all, time_data=time_data, convert_param=T, log_in=T, u=input_fun_cp, replicate=replicate, results = "sum", library_size = lib_mat, length = length))
        gradient_val <- TSE_bs_cp_grad(x = pars_ordered, fn_fit=fn_all_ode_cp, data = data_rpkm_all, time_data=time_data, convert_param=T, log_in=T, u=input_fun_cp, replicate=replicate, results = "sum", library_size = lib_mat, length = length)
        hessian_val <- TSE_bs_cp_hessian(x = pars_ordered,fn_fit=fn_all_ode_cp, data = data_rpkm_all, time_data=time_data, convert_param=T, log_in=T, u=input_fun_cp, replicate=replicate, results = "sum", library_size = lib_mat, length = length)
        if ( !is.null(fixed) ){
          # 2* because not included in the chisq
          res <- objlist(
            value = scale * val,
      ...
      obj_vgh_transformed <- function(pars, fixed = NULL, scale=2, no_np = F, no_ca0 = F ){
        ...
        val <- unname(TSE_bs_oldpar(x = pars_old_ordered, fn_fit=fn_all_ode_cp, data = data_rpkm_all, time_data=time_data, convert_param=T, log_in=T, u=input_fun_cp, replicate=replicate, results = "sum", library_size = lib_mat, length = length))
        gradient_val <- TSE_bs_cp_grad(x = pars_old_ordered, fn_fit=fn_all_ode_cp, data = data_rpkm_all, time_data=time_data, convert_param=T, log_in=T, u=input_fun_cp, replicate=replicate, results = "sum", library_size = lib_mat, length = length)
        ...
        hessian_val <- TSE_bs_cp_hessian(x = pars_old_ordered,fn_fit=fn_all_ode_cp, data = data_rpkm_all, time_data=time_data, convert_param=T, log_in=T, u=input_fun_cp, replicate=replicate, results = "sum", library_size = lib_mat, length = length)
        # add hessian for additional parnames
        for (parname_add in par_names[which(par_names %in% c("k1'/k2", "k1'k2'/k2"))]){
          if (parname_add == "k1'/k2"){
            vec <- rep(0,ncol(hessian_val))
      ...
      for(i in i_to_do){
        ...
        if(names(par_best_old)[i] %in% names(dat_g)){
          ...
          test.exact <- profile_dMod_with_save(obj = obj_vgh_main, pars = par_best_old, whichPar = i, limits = c(-3,3), method = "optimize", cores = 1, verbose = T, scale = 2, no_np = no_np, no_ca0 = no_ca0, file_name = paste0(out_dir_path,'/temp_par_',names(par_best_old)[i], '_', replicate, '_', gene_name, '.Rdata',sep=""))
      ...
      for(i in i_to_do){
        ...
        if(names(par_best_old_transformed)[i] %in% names(dat_g)){
          ...
          # test.exact <- profile(obj = obj_vgh, pars = par_best_old, whichPar = i, limits = c(-3,3), method = "optimize", cores = 1, verbose = T, scale = 2, no_np = no_np, no_ca0 = no_ca0)
          # test.exact <- profile_dMod_with_save(obj = obj_vgh, pars = par_best_old, whichPar = i, limits = c(-3,3), method = "optimize", cores = 1, verbose = T, scale = 2, no_np = no_np, no_ca0 = no_ca0, file_name = "/tmp/dMod_test_save.Rdata")
          test.exact <- profile_dMod_with_save(obj = obj_vgh_transformed, pars = par_best_old_transformed, whichPar = i, limits = c(-3,3), method = "optimize", cores = 1, verbose = T, scale = 2, no_np = no_np, no_ca0 = no_ca0, file_name = paste0(out_dir_path,'/temp_par_',gsub('/','over',names(par_best_old_transformed)[i]), '_', replicate, '_', gene_name, '.Rdata',sep=""))
