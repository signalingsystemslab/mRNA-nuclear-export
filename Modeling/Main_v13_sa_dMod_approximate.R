...
out_dir <- paste0(out_dir, '/Profile_dMod_integrate_v3')
dir.create(file.path(out_dir), showWarnings = FALSE)
...
out_dir_path <- paste0(scratch_dir,'/',folder,'/Profile_dMod_integrate_v3/', condition)
    ...
    if (!file.exists(paste0(out_dir_path,'/profile_dMod_', batch, '_', gene_name, '_Finished.Rdata',sep=""))){
      ...
      obj_vgh_transformed <- function(pars, fixed = NULL, scale=2, no_np = F, no_ca0 = F ){
        ...
        if (!no_np){
          ...
          trans_mat[1:4,1:4] <- diag(1,nrow = 4) # span, sigma_t, sigma_b, b1_ca0 unchanged
