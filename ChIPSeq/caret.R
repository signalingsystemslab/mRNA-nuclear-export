...
H3K27ac <- processdata(fread("Results/multicov_H3K27ac.bed"), fread("Results/H3K27acannotate.txt"))
H3K36me3 <- processdata(fread("Results/multicov_H3K36me3.bed"), fread("Results/H3K36me3annotate.txt"))
H3K4me3 <- processdata(fread("Results/multicov_H3K4me3.bed"), fread("Results/H3K4me3annotate.txt"))
H3K79me2 <- processdata(fread("Results/multicov_H3K79me2.bed"), fread("Results/H3K79me2annotate.txt"))

#mRNA paramater load
param <- fread('parameter.csv')[, -1]
colnames(param) <- c('name', 'k1prime', 'k2', 'k2prime', 'kdeg', 'transport')
curated <- fread('../Manual Curation/Curated list_naive.csv')
splicing <- readRDS('../Intron retention/Splicing_proba.Rds')
...
slide <- function(data, start, end, bins) {
    ...
    temp <- data %>% group_by(gene) %>% dplyr::summarise(naive = sum(naive))
    df$total <- unlist(temp[, 2])
    temp <- data %>% group_by(gene) %>% dplyr::summarise(pos = sum(naive[distance > 0]), neg = sum(naive[distance < 0]))
    temp[is.na(temp)] <- 0
...
for (i in runindex) {
    ...
    trainRowNumbers <- createDataPartition(combine2$param, p = 1, list = FALSE)
    ...
    for(i in 1:30) seeds[[i]]<- sample.int(n=1000, ncol(combine2)-1)
...
##  p-values
t.test(resamples$chip, resamples$length)
t.test(resamples$chip, resamples$intron)
t.test(resamples$chip, resamples$prob)

t.test(resamples$feat, resamples$chip)
t.test(resamples$feat, resamples$length)
t.test(resamples$feat, resamples$intron)
t.test(resamples$feat, resamples$prob)

t.test(resamples$chipfeat, resamples$feat)
...
aggmap[1,] <- c(1, NA,NA,NA)
aggmap[2,] <- c(NA, 1, NA,NA)
aggmap[3,] <- c(NA, NA, 1, NA)
aggmap[4,] <- c(NA, NA, NA, 1)
aggmap[5,] <- c(NA, unlist(data.frame(varImp(feat, scale = F)[1])))
aggmap[6,] <- c(1 - sum(varImp(chipfeat, scale =F)$importance[c("length", "prob", "intron"),]), varImp(chipfeat, scale =F)$importance[c("length", "prob", "intron"),])
...
number_to_display <- matrix("", ncol=6,nrow=4)
number_to_display[4, c(1,6)] <- c(sum(varImp(chip,scale=F)$importance > 0),sum(varImp(chipfeat,scale=F)$importance[-which(row.names(varImp(chipfeat,scale=F)$importance) %in% c("length", "intron", "prob")),] > 0))
important <- pheatmap(aggmap, cluster_cols = F, cluster_rows = F, legend = TRUE, main = 'Feature Importance for Models', color = myColor, fontsize = 10, cellheight = 40, cellwidth = 40, breaks = myBreaks, display_numbers = number_to_display, number_color = "black")
...
unique <- as.character(resamples3$model[10:1])
for (i in unique[10:length(unique)]) {
    temp <- wilcox.test(resamples2[which(resamples2$model == unique[9]), 1], resamples2[which(resamples2$model == i), 1])
    print(temp$p.value)
...
chipimp$feature <- as.factor(chipimp$feature)
mark <- c()
for (i in 1:34) {
...
chipimp$mark <- mark
ggplot(chipimp, aes(x = feature, y = importance)) + geom_point(stat = 'identity', aes(col = mark), size = 6) + coord_flip() + theme_bw()+
