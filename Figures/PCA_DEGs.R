...
plot_pca_many_exp <- function(data, barcodes,  interactive=F, high_threshold = 5, topnumber=200, ...){
...
LPA_induced_genes <- read.csv( file = '../Manual Curation/gene_list.csv', header = T, as.is = T) # need to add new annotation

# LPA_induced_genes <- read.csv( file = '../DEGs/top_table_summary.csv', header = T, as.is = T) # need to add new annotation
LPA_induced_genes <- LPA_induced_genes[LPA_induced_genes$Naive.Small.Model | LPA_induced_genes$LPA.Small.Model,]
...
p <- plot_pca_many_exp(chromatin_all_LPA_Naive, chromatin_all_LPA_Naive_barcodes)
