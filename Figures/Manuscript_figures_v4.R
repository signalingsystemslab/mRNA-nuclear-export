...
# set working directory - depending on computer
#setwd('/home/diane/Documents/Disk1/Documents/Ribo_Modeling_Reprocessed/Paper Final Run/')
setwd('/home/diane/Documents/UCLA/Ribo_New/')
...
list_genes_cleaned <- read.csv('DEGs/gene_list.csv', sep=',')
...
load('../Big_Summer/ActDAnalyser/data/BMDM_dataset.rda')
...
# load gene_infos
load('Modeling/All_genes_last_5kbs/Model1/Hoffman2/gene_infos_with_length.Rdata')
load('Modeling/All_genes_last_5kbs/Model1/Hoffman2/lib.size.Rdata')

# load modeling function
source(file = 'Modeling/All_genes_last_5kbs/Model1/Hoffman2/functions_weighting_for_smoothing_include_negbinom_error_in_model_v13.R')
...
#### Figure 1A - Load design ####---------------------------------------------------------------
# 
# # grImport2::
# p <- grImport2::readPicture( "Figures/Fig1A_v2.svg" )
...
dat <- cbind(caRNA_Naive_exons_cpm[,c(NA, 12, 24,1,13,25,2,14,26,3,15,27,4,16,28,5,17,29,6,7,18,30,8,19,31,9,20,32,21,33,10,22,34,11,23,35)], 
             npRNA_Naive_exons_cpm[,c(NA,NA,12,5,NA,13,1,NA,14,6,NA,15,2,NA,16,7,NA,17,3,8,NA,18,4,NA,19,9,NA,20,NA,21,10,NA,22,11,NA,23)], 
...
dat_cor2[1,] <- NA
dat_cor2[36 + c(1,2,5,8,11,14,17,21,24,27,29,32,35),] <- NA
dat_cor2[,36 + c(1,2,5,8,11,14,17,21,24,27,29,32,35)] <- NA
...
## Comparison of reannotations with DB

# setwd('/home/diane/Documents/Disk1/Documents/Ribo_Modeling_Reprocessed/Paper Final Run/5kb/')
setwd('/home/diane/Documents/Disk1/Documents/Ribo_Modeling_Reprocessed/Paper Final Run/5kb/')
...
dat <- data.frame(Position_TSS_Annotated = gene_table$'Annotations...TSS', 
                  ...
                  Distance_TSS_between_observed_and_annotated = abs(gene_table$'Annotations...TSS' - gene_table$'Observed...TSS')+1, 
                  Distance_TES_between_observed_and_annotated = abs(gene_table$'Annotations...TES' - gene_table$'Observed...TES')+1, 
...
p <- p + scale_color_continuous(trans='log10',breaks=c(1,11,101,1001),labels = c(0,10,100,1000), limits=c(1,2001), oob=squish)
p <- p + theme_bw() + theme(aspect.ratio = 1)+labs(color= 'Distance between\nobserved and\nannotated TSS (bp)', y='Distance of closest refTSS to observed TSS',x='Distance of closest refTSS to annotated TSS')
p
...
grid.arrange(arrangeGrob(g3, legend, nrow=1, widths=c(9,3), heights=9,respect = T))
...
p <- p + scale_y_continuous(trans='log10', breaks = 1+c(0,10,50,100,200,500,1000),labels = c(0,10,50,100,200,500,1000))
p <- p + scale_color_continuous(trans='log10',breaks=c(1,11,101,1001),labels = c(0,10,100,1000), limits=c(1,2001), oob=squish)
p <- p + theme_bw() + theme(aspect.ratio = 1)+labs(color= 'Distance between\nobserved and\nannotated TES (bp)', y='Distance of closest polyA site to observed TES',x='Distance of closest polyA site to annotated TES')
p
...
ca_smooth <- predict(smooth.spline(x=dat_g$Time[ dat_g$Fraction == "Chromatin" & dat_g$Batch == "b3"], y=dat_g$log2RPKM[ dat_g$Fraction == "Chromatin" & dat_g$Batch == "b3"], spar = 0.45),seq(0,121,by=1/60))$y
...
for (condition in c('naive','lpa')){
  ...
  for (g in gene_infos$ensembl_gene_id[order(gene_infos$external_gene_name)]){
    ...
    if (g %in% sapply(row.names(raw_data), function(s){substr(s,1,18)})){
      ...
      file <- paste0('Modeling/All_genes_last_5kbs/Model1/Hoffman2/Optimisation/Modeling_v13/Optimisation/BFGS/',condition, '/param_bfgs_1000ri_merged_', gene_name, '_Finished.Rdata')
      if (file.exists(file)){
        load(file)
        bfgs <- bfgs_merged
      }else{
      ...
      if (condition == "naive") {
        data_all <- list( caRNA   = as.matrix(create_dat_g_rpkm(caRNA_Naive_exons_rpkm_5kb,   g_id, batch=c("b1", "b2", "b3"), time = c("000","010","015","020","025","030","035","040","050","060","075","090","120"))), 
                          npRNA   = as.matrix(create_dat_g_rpkm(npRNA_Naive_exons_rpkm_5kb,   g_id, batch=c("b1", "b2", "b3"), time = c("000","010","015","020","025","030","035","040","050","060","075","090","120"))), 
                          cytoRNA = as.matrix(create_dat_g_rpkm(cytoRNA_Naive_exons_rpkm_5kb, g_id, batch=c("b1", "b2", "b3"), time = c("000","010","015","020","025","030","035","040","050","060","075","090","120"))))
        gene_length <- gene_infos$Length_5kb_naive[gene_infos$ensembl_gene_id == substr(g_id,1,18)]
      }else if (condition == "lpa"){
        data_all <- list( caRNA   = as.matrix(create_dat_g_rpkm(caRNA_LPA_exons_rpkm_5kb,   g_id, batch=c("b1", "b2", "b3"), time = c("000","010","015","020","025","030","035","040","050","060","075","090","120"))), 
                          npRNA   = as.matrix(create_dat_g_rpkm(npRNA_LPA_exons_rpkm_5kb,   g_id, batch=c("b1", "b2", "b3"), time = c("000","010","015","020","025","030","035","040","050","060","075","090","120"))), 
                          cytoRNA = as.matrix(create_dat_g_rpkm(cytoRNA_LPA_exons_rpkm_5kb, g_id, batch=c("b1", "b2", "b3"), time = c("000","010","015","020","025","030","035","040","050","060","075","090","120"))))
      ...
      for (bs in 2:ncol(data_all$caRNA)){
        dat_g_exp_ca <- rbind(dat_g_exp_ca, data.frame(RPKM=data_all$caRNA[,bs], Time = data_all$caRNA[,1], batch = c("b1", "b2", "b3")[bs-1], Cpt = "Chromatin", Type="Experiment", Condition = condition))
        dat_g_exp_np <- rbind(dat_g_exp_np, data.frame(RPKM=data_all$npRNA[,bs], Time = data_all$npRNA[,1], batch = c("b1", "b2", "b3")[bs-1], Cpt = "Nucleoplasm", Type="Experiment", Condition = condition))
        dat_g_exp_cyto <- rbind(dat_g_exp_cyto, data.frame(RPKM=data_all$cytoRNA[,bs], Time = data_all$cytoRNA[,1], batch = c("b1", "b2", "b3")[bs-1], Cpt = "Cytoplasm", Type="Experiment", Condition = condition))
      }
      ...
      for (b in c('all', 'b1', 'b2', 'b3')){   
        ...
        if (condition == "naive") {
          data <- list( caRNA   = as.matrix(create_dat_g_rpkm(caRNA_Naive_exons_rpkm_5kb,   g_id, batch=batch, time = c("000","010","015","020","025","030","035","040","050","060","075","090","120"))), 
                        npRNA   = as.matrix(create_dat_g_rpkm(npRNA_Naive_exons_rpkm_5kb,   g_id, batch=batch, time = c("000","010","015","020","025","030","035","040","050","060","075","090","120"))), 
                        cytoRNA = as.matrix(create_dat_g_rpkm(cytoRNA_Naive_exons_rpkm_5kb, g_id, batch=batch, time = c("000","010","015","020","025","030","035","040","050","060","075","090","120"))))
        }else if (condition == "lpa"){
          data <- list( caRNA   = as.matrix(create_dat_g_rpkm(caRNA_LPA_exons_rpkm_5kb,   g_id, batch=batch, time = c("000","010","015","020","025","030","035","040","050","060","075","090","120"))), 
                        npRNA   = as.matrix(create_dat_g_rpkm(npRNA_LPA_exons_rpkm_5kb,   g_id, batch=batch, time = c("000","010","015","020","025","030","035","040","050","060","075","090","120"))), 
                        cytoRNA = as.matrix(create_dat_g_rpkm(cytoRNA_LPA_exons_rpkm_5kb, g_id, batch=batch, time = c("000","010","015","020","025","030","035","040","050","060","075","090","120"))))
        }
        ...
        if(b %in% c('all', 'b1') & condition == 'naive'){
          ...
          names(par_old) <- colnames(par_old_all) <- c('spar', 'sigmat', 'sigmab', 'ca0_b1', "k1'", "k2", "k2'", "kdeg")
        }else{
        ...
        SSEs_g_total <- TSE_bs_cp(par = par, fn_fit=fn_all_ode_cp, data = data_all, time_data=time_data, convert_param=T, log_in=T, u=input_fun_cp, batch=b, results = "sum", library_size = lib_mat, length = gene_length)
        SSEs_g_det <- TSE_bs_cp(par = par, fn_fit=fn_all_ode_cp, data = data_all, time_data=time_data, convert_param=T, log_in=T, u=input_fun_cp, batch=b, results = "detailed", library_size = lib_mat, length = gene_length)
        
        par_theo <- par
        par_theo[2:3] <- c(0,1*10^-6)
        
        SSEs_g_total_theo <- TSE_bs_theo_cp(par = par_theo, fn_fit=fn_all_ode_cp, data = data_all, time_data=time_data, convert_param=T, log_in=T, u=input_fun_cp, batch=b, results = "sum", library_size = lib_mat, length = gene_length)
        ...
        for (bs in 2:(ncol(data$caRNA))){
          ...
          if(ncol(data$caRNA) > 2){
            weight_ca <- proba_ca[dat_g_exp_ca$batch == paste0('b',bs-1)]
            weight_np <- proba_np[dat_g_exp_np$batch == paste0('b',bs-1)]
            weight_cyto <- proba_cyto[dat_g_exp_cyto$batch == paste0('b',bs-1)]
          }else{
...
# => save data
merge_function_for_b2 <- function(dat_g, th){
  dat_g[["k1'k2'/k2"]] <- NULL
    ...
    if (g %in% sapply(row.names(raw_data), function(s){substr(s,1,18)})){
      ...
      for ( batch in c('all','b1','b2','b3')){
        ...
        if(!is.null(all_param[[condition]][[batch]][[gene_name]])){
          ...
          file_approx <- paste0('Modeling/All_genes_last_5kbs/Model1/Hoffman2/Optimisation/Modeling_v13/Profile_dMod_integrate_v2/', condition, '/profile_dMod_', batch, '_', gene_name,'_Finished.Rdata')
          ...
          if( length(consistency_check) == 0 || !all(consistency_check)){
            file_exact <- paste0('Modeling/All_genes_last_5kbs/Model1/Hoffman2/Optimisation/Modeling_v13/Profile_dMod/', condition, '/profile_dMod_', batch, '_', gene_name,'_Finished.Rdata')
