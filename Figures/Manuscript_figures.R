...
# plot(dat_ggplot2$color_rep1[dat_ggplot2$Condition == "naive" & dat_ggplot2$Parameter_name == "spar"], dat_ggplot2$color_rep2[dat_ggplot2$Condition == "naive" & dat_ggplot2$Parameter_name == "spar"], xlab="Fit Quality rep 1", ylab = "Fit Quality Rep2")

##### Fit Quality histogramm - tolerized #####----------------------------
# svg(filename = "Fit_quality_histogram_tolerized.svg", width=7, height=7)
# p <- ggplot(dat_ggplot[dat_ggplot$Batch %in% c("rep1", "rep2") & dat_ggplot$Parameter_name == "spar",]) + geom_density(aes(x=color, linetype=Batch, col=interaction(Condition,Batch), fill=interaction(Condition,Batch)), alpha=0.3)
# p <- p + scale_linetype_discrete(name = "Replicates", labels = c("rep1"="Rep1", "rep2"="Rep2"))
# p <- p + scale_fill_manual(values = c("lpa.rep1"=col_lpa_rep1, "lpa.rep2"=col_lpa_rep2, "naive.rep1"=col_naive_rep1, "naive.rep2"=col_naive_rep2), labels=c("lpa.rep1"="Rep1", "lpa.rep2"="Rep2", "naive.rep1"="Rep1", "naive.rep2"="Rep2"))
# p <- p + scale_color_manual(values = c("lpa.rep1"=col_lpa_rep1, "lpa.rep2"=col_lpa_rep2, "naive.rep1"=col_naive_rep1, "naive.rep2"=col_naive_rep2), labels=c("lpa.rep1"="Rep1", "lpa.rep2"="Rep2", "naive.rep1"="Rep1", "naive.rep2"="Rep2"))
# p <- p + theme_bw() + theme(aspect.ratio=0.5) + labs(x="Fit Quality", y="Density", col="Fit Quality")
# p
# dev.off()
...
sum(dat_ggplot$Batch == "b1" & dat_ggplot$Condition == "naive" & dat_ggplot$Parameter_name == "spar" & dat_ggplot$color > 0.06)
sum(dat_ggplot$Batch == "b3" & dat_ggplot$Condition == "naive" & dat_ggplot$Parameter_name == "spar" & dat_ggplot$color > 0.06)
sum(dat_ggplot$Batch == "b1" & dat_ggplot$Condition == "lpa" & dat_ggplot$Parameter_name == "spar" & dat_ggplot$color > 0.06)
sum(dat_ggplot$Batch == "b3" & dat_ggplot$Condition == "lpa" & dat_ggplot$Parameter_name == "spar" & dat_ggplot$color > 0.06)


##### Explaination of poorer fits #####------------------------------------
# try ca foldchange vs fit quality.
dat_ggplot_fit_qual <- data.frame(Gene_name = c(), ca_max =c(), np_max = c(), cyto_max = c(), ca_fc = c(), np_fc=c(), cyto_fc =c(),
                                  Condition = c(), Batch=c(), color =c())

for (condition in c('naive', 'lpa')){
  for(g in names(all_data[[condition]])){
    for (b in c("b1", "b3")){
      dat_ggplot_fit_qual <- rbind(dat_ggplot_fit_qual,
                                   data.frame(Gene_name = g, 
                                              ca_max = max(all_data[[condition]][[g]][["caRNA"]][,b], na.rm=T),
                                              np_max = max(all_data[[condition]][[g]][["npRNA"]][,b], na.rm=T),
                                              cyto_max = max(all_data[[condition]][[g]][["cytoRNA"]][,b], na.rm=T),
                                              ca_fc = max(all_data[[condition]][[g]][["caRNA"]][,b], na.rm=T) - ifelse(is.na(all_data[[condition]][[g]][["caRNA"]][1,b]), unname(unlist(all_best_param[[condition]][[b]][g,"ca0_b1"])),all_data[[condition]][[g]][["caRNA"]][1,b]), 
                                              np_fc = max(all_data[[condition]][[g]][["npRNA"]][,b], na.rm=T) - ifelse(is.na(all_data[[condition]][[g]][["npRNA"]][1,b]), unname(unlist(all_best_param[[condition]][[b]][g,"ca0_b1"]))+log2(10)*unname(unlist(all_best_param[[condition]][[b]][g,"k1'/k2"])),all_data[[condition]][[g]][["npRNA"]][1,b]), 
                                              cyto_fc = max(all_data[[condition]][[g]][["cytoRNA"]][,b], na.rm=T) - all_data[[condition]][[g]][["cytoRNA"]][1,b],
                                              Condition = condition, Batch=b,
                                              color=all_annot[[condition]][[b]][[g]]$annot[1,]))
    }
  }
}

plot(dat_ggplot_fit_qual$color, dat_ggplot_fit_qual$ca_fc)
cor.test(dat_ggplot_fit_qual$color, dat_ggplot_fit_qual$ca_fc, method="spearman")
plot(dat_ggplot_fit_qual$color, dat_ggplot_fit_qual$np_fc)
cor.test(dat_ggplot_fit_qual$color, dat_ggplot_fit_qual$np_fc, method="spearman")
plot(dat_ggplot_fit_qual$color, dat_ggplot_fit_qual$cyto_fc)
cor.test(dat_ggplot_fit_qual$color, dat_ggplot_fit_qual$cyto_fc, method="spearman")

plot(dat_ggplot_fit_qual$color, dat_ggplot_fit_qual$ca_max)
cor.test(dat_ggplot_fit_qual$color, dat_ggplot_fit_qual$ca_max, method="spearman")
plot(dat_ggplot_fit_qual$color, dat_ggplot_fit_qual$np_max)
cor.test(dat_ggplot_fit_qual$color, dat_ggplot_fit_qual$np_max, method="spearman")
plot(dat_ggplot_fit_qual$color, dat_ggplot_fit_qual$cyto_max)
cor.test(dat_ggplot_fit_qual$color, dat_ggplot_fit_qual$cyto_max, method="spearman")


plot(dat_ggplot_fit_qual$color, dat_ggplot_fit_qual$np_fc -  dat_ggplot_fit_qual$ca_fc)
cor.test(dat_ggplot_fit_qual$color, dat_ggplot_fit_qual$np_fc -  dat_ggplot_fit_qual$ca_fc, method="spearman")
plot(dat_ggplot_fit_qual$color, dat_ggplot_fit_qual$cyto_fc -  dat_ggplot_fit_qual$ca_fc)
cor.test(dat_ggplot_fit_qual$color, dat_ggplot_fit_qual$cyto_fc -  dat_ggplot_fit_qual$ca_fc, method="spearman")
plot(dat_ggplot_fit_qual$color, dat_ggplot_fit_qual$cyto_fc -  dat_ggplot_fit_qual$np_fc)
cor.test(dat_ggplot_fit_qual$color, dat_ggplot_fit_qual$cyto_fc -  dat_ggplot_fit_qual$np_fc, method="spearman")


plot(dat_ggplot_fit_qual$color, (dat_ggplot_fit_qual$np_fc -  dat_ggplot_fit_qual$ca_fc)/(dat_ggplot_fit_qual$cyto_fc -  dat_ggplot_fit_qual$ca_fc), ylim=c(-2,2))
cor.test(dat_ggplot_fit_qual$color,  (dat_ggplot_fit_qual$np_fc -  dat_ggplot_fit_qual$ca_fc)/(dat_ggplot_fit_qual$cyto_fc -  dat_ggplot_fit_qual$ca_fc), method="spearman")


p <- ggplot(data=dat_ggplot_fit_qual) + geom_point(aes(x=dat_ggplot_fit_qual$np_fc -  dat_ggplot_fit_qual$ca_fc, y = dat_ggplot_fit_qual$cyto_fc -  dat_ggplot_fit_qual$np_fc, color=color ))
p

p <- ggplot(data=dat_ggplot_fit_qual) + geom_point(aes(x=dat_ggplot_fit_qual$ca_fc, y = dat_ggplot_fit_qual$cyto_fc, color=color ))
p

p <- ggplot(data=dat_ggplot_fit_qual) + geom_point(aes(x=dat_ggplot_fit_qual$np_fc -  dat_ggplot_fit_qual$ca_fc, y = dat_ggplot_fit_qual$cyto_fc -  dat_ggplot_fit_qual$ca_fc, color=color ))
p

p <- ggplot(data=dat_ggplot_fit_qual) + geom_point(aes(x=(dat_ggplot_fit_qual$np_fc -  dat_ggplot_fit_qual$ca_fc)/(dat_ggplot_fit_qual$cyto_fc -  dat_ggplot_fit_qual$ca_fc), y = dat_ggplot_fit_qual$cyto_fc, color=color ))
p + coord_cartesian(xlim=c(-2,2))

p <- ggplot(data=dat_ggplot_fit_qual) + geom_point(aes(x=dat_ggplot_fit_qual$np_fc -  dat_ggplot_fit_qual$ca_fc, y = dat_ggplot_fit_qual$ca_fc, color=color ))

p <- ggplot(data=dat_ggplot_fit_qual) + geom_point(aes(x=dat_ggplot_fit_qual$np_fc -  dat_ggplot_fit_qual$ca_fc, y = dat_ggplot_fit_qual$np_fc, color=color ))
p <- ggplot(data=dat_ggplot_fit_qual) + geom_point(aes(x=dat_ggplot_fit_qual$ca_fc, y = dat_ggplot_fit_qual$cyto_fc, color=color ))
p
...
# svg(filename = 'Fig3B_v13_ident.svg')
p <- ggplot(dat_ggplot[dat_ggplot$Parameter_name %in% c("k1'", "k2", "k2'", "kdeg") & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1", "b3") & is.na(dat_ggplot$CIL_0.95_Min) & is.na(dat_ggplot$CIU_0.95_Max) ,]) 
p <- p + geom_density(mapping = aes(x=Parameter_value, fill=Parameter_name, color=Parameter_name, linetype=Batch), alpha=0.1)
p <- p + scale_color_manual(name="Parameter", values=c("red", "yellow","green", "blue"))
p <- p + scale_fill_manual(name="Parameter", values=c("red", "yellow", "green", "blue"))
p <- p + scale_linetype_manual(values=c("b1"=1, "b3" = 2), labels = c("Rep1", "Rep2"))
p <- p + theme_bw() + theme(aspect.ratio=1/4) + labs(x=expression(log[10]("Parameter value")), linetype="Replicate")
p
# dev.off()
stop()
...
# test similarity of distribution
ks.test(dat_ggplot$Parameter_value[dat_ggplot$Parameter_name == "k1'" & dat_ggplot$Condition == "naive" & dat_ggplot$Batch == "b1" & is.na(dat_ggplot$CIL_0.95_Min) & is.na(dat_ggplot$CIU_0.95_Max)],
        dat_ggplot$Parameter_value[dat_ggplot$Parameter_name == "k1'" & dat_ggplot$Condition == "naive" & dat_ggplot$Batch == "b3" & is.na(dat_ggplot$CIL_0.95_Min) & is.na(dat_ggplot$CIU_0.95_Max)])
ks.test(dat_ggplot$Parameter_value[dat_ggplot$Parameter_name == "k2'" & dat_ggplot$Condition == "naive" & dat_ggplot$Batch == "b1" & is.na(dat_ggplot$CIL_0.95_Min) & is.na(dat_ggplot$CIU_0.95_Max)],
        dat_ggplot$Parameter_value[dat_ggplot$Parameter_name == "k2'" & dat_ggplot$Condition == "naive" & dat_ggplot$Batch == "b3" & is.na(dat_ggplot$CIL_0.95_Min) & is.na(dat_ggplot$CIU_0.95_Max)])
ks.test(dat_ggplot$Parameter_value[dat_ggplot$Parameter_name == "k2" & dat_ggplot$Condition == "naive" & dat_ggplot$Batch == "b1" & is.na(dat_ggplot$CIL_0.95_Min) & is.na(dat_ggplot$CIU_0.95_Max)],
        dat_ggplot$Parameter_value[dat_ggplot$Parameter_name == "k2" & dat_ggplot$Condition == "naive" & dat_ggplot$Batch == "b3" & is.na(dat_ggplot$CIL_0.95_Min) & is.na(dat_ggplot$CIU_0.95_Max)])
ks.test(dat_ggplot$Parameter_value[dat_ggplot$Parameter_name == "kdeg" & dat_ggplot$Condition == "naive" & dat_ggplot$Batch == "b1" & is.na(dat_ggplot$CIL_0.95_Min) & is.na(dat_ggplot$CIU_0.95_Max)],
        dat_ggplot$Parameter_value[dat_ggplot$Parameter_name == "kdeg" & dat_ggplot$Condition == "naive" & dat_ggplot$Batch == "b3" & is.na(dat_ggplot$CIL_0.95_Min) & is.na(dat_ggplot$CIU_0.95_Max)])

# test JSD: 
x_b1 <- dat_ggplot$Parameter_value[dat_ggplot$Parameter_name == "kdeg" & dat_ggplot$Condition == "naive" & dat_ggplot$Batch == "b1" & is.na(dat_ggplot$CIL_0.95_Min) & is.na(dat_ggplot$CIU_0.95_Max)]
x_b3 <- dat_ggplot$Parameter_value[dat_ggplot$Parameter_name == "kdeg" & dat_ggplot$Condition == "naive" & dat_ggplot$Batch == "b3" & is.na(dat_ggplot$CIL_0.95_Min) & is.na(dat_ggplot$CIU_0.95_Max)]
ecdf_b1 <- ecdf(x_b1)
ecdf_b3 <- ecdf(x_b3)
philentropy::distance(x = rbind(ecdf_b1(sort(c(x_b1,x_b3))),ecdf_b3(sort(c(x_b1,x_b3)))), method = "jensen-shannon")
philentropy::distance(x = rbind(ecdf_b1(seq(min(x_b1,x_b3),max(x_b1,x_b3),by=0.0001)),ecdf_b3(seq(min(x_b1,x_b3),max(x_b1,x_b3),by=0.0001))), method = "jensen-shannon")
philentropy::JSD(x = rbind(ecdf_b1(seq(min(x_b1,x_b3),max(x_b1,x_b3),by=0.00001)),ecdf_b3(seq(min(x_b1,x_b3),max(x_b1,x_b3),by=0.00001))))

epsilon <- 0.5 * min(c(diff(sort(x_b1)),diff(sort(x_b3)))) 
KLb1 = (1/length(x_b1))*sum(log((ecdf_b1(x_b1) - ecdf_b1(x_b1-epsilon))/(ecdf_b3(x_b1) - ecdf_b3(x_b1-epsilon)))) # need to linearly interpolate ecdf
KLb3 = (1/length(x_b3))*sum(log((ecdf_b1(x_b3) - ecdf_b1(x_b3-epsilon))/(ecdf_b3(x_b3) - ecdf_b3(x_b3-epsilon)))) # need to linearly interpolate ecdf
JSD <- 0.5 * (KLb1 + KLb3)

dat_ggplot_b1 <- dat_ggplot[dat_ggplot$Batch == 'b1',]
dat_ggplot_b3 <- dat_ggplot[dat_ggplot$Batch == 'b3',]

dat_ggplot2 <- dat_ggplot_b1

# Add column for b3 value and CI
colnames(dat_ggplot2) <- gsub(pattern = "Parameter_value" , replacement = "Parameter_value_b1", x = colnames(dat_ggplot2))
dat_ggplot2$Parameter_value_b3 <- dat_ggplot_b3$Parameter_value[match(paste(dat_ggplot_b1$Gene_name,dat_ggplot_b1$Parameter_name,dat_ggplot_b1$Condition,sep = '.'),
                                                                      paste(dat_ggplot_b3$Gene_name,dat_ggplot_b3$Parameter_name,dat_ggplot_b3$Condition,sep = '.'))]

colnames(dat_ggplot2) <- gsub(pattern = "CIL_0.95_Min" , replacement = "CIL_0.95_Min_b1", x = colnames(dat_ggplot2))
dat_ggplot2$CIL_0.95_Min_b3 <- dat_ggplot_b3$CIL_0.95_Min[match(paste(dat_ggplot_b1$Gene_name,dat_ggplot_b1$Parameter_name,dat_ggplot_b1$Condition,sep = '.'),
                                                                paste(dat_ggplot_b3$Gene_name,dat_ggplot_b3$Parameter_name,dat_ggplot_b3$Condition,sep = '.'))]

colnames(dat_ggplot2) <- gsub(pattern = "CIL_0.95_Max" , replacement = "CIL_0.95_Max_b1", x = colnames(dat_ggplot2))
dat_ggplot2$CIL_0.95_Max_b3 <- dat_ggplot_b3$CIL_0.95_Max[match(paste(dat_ggplot_b1$Gene_name,dat_ggplot_b1$Parameter_name,dat_ggplot_b1$Condition,sep = '.'),
                                                                paste(dat_ggplot_b3$Gene_name,dat_ggplot_b3$Parameter_name,dat_ggplot_b3$Condition,sep = '.'))]

colnames(dat_ggplot2) <- gsub(pattern = "CIU_0.95_Min" , replacement = "CIU_0.95_Min_b1", x = colnames(dat_ggplot2))
dat_ggplot2$CIU_0.95_Min_b3 <- dat_ggplot_b3$CIU_0.95_Min[match(paste(dat_ggplot_b1$Gene_name,dat_ggplot_b1$Parameter_name,dat_ggplot_b1$Condition,sep = '.'),
                                                                paste(dat_ggplot_b3$Gene_name,dat_ggplot_b3$Parameter_name,dat_ggplot_b3$Condition,sep = '.'))]

colnames(dat_ggplot2) <- gsub(pattern = "CIU_0.95_Max" , replacement = "CIU_0.95_Max_b1", x = colnames(dat_ggplot2))
dat_ggplot2$CIU_0.95_Max_b3 <- dat_ggplot_b3$CIU_0.95_Max[match(paste(dat_ggplot_b1$Gene_name,dat_ggplot_b1$Parameter_name,dat_ggplot_b1$Condition,sep = '.'),
                                                                paste(dat_ggplot_b3$Gene_name,dat_ggplot_b3$Parameter_name,dat_ggplot_b3$Condition,sep = '.'))]

# add color for annot
dat_ggplot2$color_b1 <- annot[paste(dat_ggplot2$Gene_name, dat_ggplot2$Condition, 'b1', sep="_"),1]
dat_ggplot2$color_b3 <- annot[paste(dat_ggplot2$Gene_name, dat_ggplot2$Condition, 'b3', sep="_"),1]
dat_ggplot2$color_max <- apply(dat_ggplot2[,paste("color", c("b1", "b3"), sep="_")],1, max)
...
make_replicate_plot2 <- function(data, condition,parameter, xylim=NULL, color_facet=NULL, font_size=12, legend = "none"){
  ...
  p <- ggplot(data[data$Condition == condition & data$Parameter_name == parameter & apply(data[,c("CIL_0.95_Min_b1", "CIU_0.95_Max_b1", "CIL_0.95_Min_b3", "CIU_0.95_Max_b3")],1,function(x){all(is.na(x))}),],aes(x=Parameter_value_b1,y=Parameter_value_b3))
  p <- p + geom_point(mapping=aes(col=color_max), size=2) + facet_wrap(.~facet_label, scale='free', drop = T, labeller = "label_parsed")
  p <- p + geom_segment(mapping = aes(x=CIL_0.95_Min_b1,y=Parameter_value_b3,xend=CIL_0.95_Max_b1,yend = Parameter_value_b3, col=color_b1), alpha=0.3, linetype=2)
  p <- p + geom_segment(mapping = aes(x=CIU_0.95_Min_b1,y=Parameter_value_b3,xend=CIU_0.95_Max_b1,yend = Parameter_value_b3, col=color_b1), alpha=0.3, linetype=2)
  p <- p + geom_segment(mapping = aes(x=CIL_0.95_Max_b1,y=Parameter_value_b3,xend=CIU_0.95_Min_b1,yend = Parameter_value_b3, col=color_b1), alpha=0.3)
  p <- p + geom_segment(mapping = aes(y=CIL_0.95_Min_b3,x=Parameter_value_b1,yend=CIL_0.95_Max_b3,xend = Parameter_value_b1, col=color_b3), alpha=0.3, linetype = 2)
  p <- p + geom_segment(mapping = aes(y=CIU_0.95_Min_b3,x=Parameter_value_b1,yend=CIU_0.95_Max_b3,xend = Parameter_value_b1, col=color_b3), alpha=0.3, linetype = 2)
  p <- p + geom_segment(mapping = aes(y=CIL_0.95_Max_b3,x=Parameter_value_b1,yend=CIU_0.95_Min_b3,xend = Parameter_value_b1, col=color_b3), alpha=0.3)
  p <- p + theme_bw(base_size = font_size) + theme(aspect.ratio=1) + scale_color_gradientn(colours = c("green3", "gold2", "darkorange2", "darkred"), limits=range(c(data$color_b1,data$color_b3)))+labs(col='Fit Quality')
  p <- p + geom_abline(slope=1, intercept=0, linetype=1, col='black', alpha=0.5) + geom_abline(slope=1, intercept=log10(2), linetype=2, col='red', alpha=0.5) + geom_abline(slope=1, intercept=-log10(2), linetype=2, col='red', alpha=0.5)
  p <- p + labs(x="Replicate 1", y="Replicate 2")
  p <- p + theme(strip.text = element_text(margin=margin(0.2,0,0.2,0,unit = "line")))
...
dat_ggplot_HL_2 <- cbind(dat_ggplot_HL[dat_ggplot_HL$Batch == 'b1',], dat_ggplot_HL[dat_ggplot_HL$Batch == 'b3',21:28])
colnames(dat_ggplot_HL_2)[29:36] <- paste(colnames(dat_ggplot_HL_2)[21:28], '_b3', sep='')

dat_ggplot_HL$Batch <- factor(dat_ggplot_HL$Batch, levels=c('all','b1','b2','b3'), labels=c('Together', 'Rep1', 'Rep3', 'Rep2'))
...
p <- p + geom_segment(data = dat_ggplot_HL_2[is.na(dat_ggplot_HL_2$HL_CIU_0.95_Max_Naive) & is.na(dat_ggplot_HL_2$HL_CIL_0.95_Min_Naive) & is.na(dat_ggplot_HL_2$HL_CIU_0.95_Max_Naive_b3) & is.na(dat_ggplot_HL_2$HL_CIL_0.95_Min_Naive_b3),], aes(x = HL_Naive, xend = HL_Naive_b3, y = ActD_HL, yend = ActD_HL), alpha=0.1, linetype=2)
...
p <- ggplot(dat_ggplot2[dat_ggplot2$Parameter_name %in% c("k1'k2'/k2") & dat_ggplot2$Condition == "naive" & apply(dat_ggplot2[,c("CIL_0.95_Min_b1", "CIU_0.95_Max_b1", "CIL_0.95_Min_b3", "CIU_0.95_Max_b3")],1,function(x){all(is.na(x))}),]) 
p <- p + geom_density(mapping = aes(x=Parameter_value_b1, fill="k1'k2'/k2", color="k1'k2'/k2", linetype="Rep1"), alpha=0.1)
p <- p + geom_density(mapping = aes(x=Parameter_value_b3, fill="k1'k2'/k2", color="k1'k2'/k2", linetype="Rep2"), alpha=0.1)
...
p <- p + geom_segment(data = dat_ggplot2[dat_ggplot2$Gene_name == "Egr1" & dat_ggplot2$Parameter_name %in% c("k1'k2'/k2") & dat_ggplot2$Condition == "naive",], aes(x = Parameter_value_b1, xend=Parameter_value_b1, y=0, yend=0.1, linetype="Rep1", col = "Egr1")) 
p <- p + geom_segment(data = dat_ggplot2[dat_ggplot2$Gene_name == "Egr1" & dat_ggplot2$Parameter_name %in% c("k1'k2'/k2") & dat_ggplot2$Condition == "naive",], aes(x = Parameter_value_b3, xend=Parameter_value_b3, y=0, yend=0.1, linetype="Rep2", col="Egr1")) 
p <- p + geom_segment(data = dat_ggplot2[dat_ggplot2$Gene_name == "Malt1" & dat_ggplot2$Parameter_name %in% c("k1'k2'/k2")& dat_ggplot2$Condition == "naive",], aes(x = Parameter_value_b1, xend=Parameter_value_b1, y=0, yend=0.1, linetype="Rep1", col="Malt1"))
p <- p + geom_segment(data = dat_ggplot2[dat_ggplot2$Gene_name == "Malt1" & dat_ggplot2$Parameter_name %in% c("k1'k2'/k2")& dat_ggplot2$Condition == "naive",], aes(x = Parameter_value_b3, xend=Parameter_value_b3, y=0, yend=0.1, linetype="Rep2", col="Malt1")) 
p <- p + annotate("text", x = mean(unlist(dat_ggplot2[dat_ggplot2$Gene_name == "Egr1" & dat_ggplot2$Parameter_name %in% c("k1'k2'/k2") & dat_ggplot2$Condition == "naive",c("Parameter_value_b1","Parameter_value_b3")]), na.rm=T), y = 0.15, label = "Egr1", color="blue", fontface="bold", size=rel(6))
p <- p + annotate("text", x = mean(unlist(dat_ggplot2[dat_ggplot2$Gene_name == "Malt1" & dat_ggplot2$Parameter_name %in% c("k1'k2'/k2") & dat_ggplot2$Condition == "naive",c("Parameter_value_b1","Parameter_value_b3")]), na.rm=T), y = 0.15, label = "Malt1", color="darkred", fontface="bold", size=rel(6))
...
p <- ggplot(dat_ggplot2[dat_ggplot2$Parameter_name %in% c("k2'/k2") & dat_ggplot2$Condition == "naive" & apply(dat_ggplot2[,c("CIL_0.95_Min_b1", "CIU_0.95_Max_b1", "CIL_0.95_Min_b3", "CIU_0.95_Max_b3")],1,function(x){all(is.na(x))}),]) 
p <- p + geom_density(mapping = aes(x=Parameter_value_b1, fill="k2'/k2", color="k2'/k2", linetype="Rep1"), alpha=0.1)
p <- p + geom_density(mapping = aes(x=Parameter_value_b3, fill="k2'/k2", color="k2'/k2", linetype="Rep2"), alpha=0.1)
...
dat_ggplot4 <- rbind(cbind(dat_ggplot2[,c(1:5,7:8,10,12:13,19)], Batch="Rep1"),setNames(cbind(dat_ggplot2[,c(1,14,3,15:18,10,12:13, 20)], Batch="Rep2"),c(names(dat_ggplot2[,c(1:5,7:8,10,12:13,19)]),"Batch")))
colnames(dat_ggplot4) <- gsub("_b1","", colnames(dat_ggplot4))
...
p <- p + theme_bw(base_size = 24) + theme(aspect.ratio=1) + scale_color_gradientn(colours = c("green3", "gold2", "darkorange2", "darkred"), limits=range(c(dat_ggplot2$color_b1,dat_ggplot2$color_b3)))+labs(col='Fit Quality', shape="Replicate")
...
for ( go in unique(gene_gobp$go_id)){
  if(length(unique(gene_gobp$external_gene_name[gene_gobp$go_id == go]))>=5){
    p <- ggplot(dat_ggplot[dat_ggplot$Parameter_name == c("k1'k2'/k2") & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1","b3") & apply(dat_ggplot[,c("CIL_0.95_Min", "CIU_0.95_Max")],1,function(x){all(is.na(x))}),]) 
    p <- p + geom_density(mapping = aes(x=Parameter_value, linetype=Batch), fill="black", alpha=0.1)
    p <- p + geom_segment(data = dat_ggplot[dat_ggplot$Gene_name %in% unique(gene_gobp$external_gene_name[gene_gobp$go_id == go]) & dat_ggplot$Parameter_name == "k1'k2'/k2" & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1","b3") & apply(dat_ggplot[,c("CIL_0.95_Min", "CIU_0.95_Max")],1,function(x){all(is.na(x))}),], aes(x = Parameter_value, xend=Parameter_value, y=0, yend=0.1, linetype=Batch, group = Gene_name)) 
    # p <- p + annotate("text", x = mean(unlist(dat_ggplot2[dat_ggplot2$Gene_name == "Egr1" & dat_ggplot2$Parameter_name %in% c("k1'k2'/k2") & dat_ggplot2$Condition == "naive",c("Parameter_value_b1","Parameter_value_b3")]), na.rm=T), y = 0.15, label = "Egr1", color="blue", fontface="bold", size=rel(6))
    p <- p + theme_bw() + theme(aspect.ratio=0.5) + labs(x="Effective Transport (log10)", fill="Parameter", color="Gene", y="Density", linetype="Replicate")
    p <- p + labs(title=unique(gene_gobp$name_1006[gene_gobp$go_id == go]))
    print(p)
    tmp <- readline()
    if(tmp == "q"){
      break
...
gene_cat <- read.csv('../Category_genes_Test.csv', header=F, row.names = 1)

for ( go  in row.names(gene_cat)){
  p <- ggplot(dat_ggplot[dat_ggplot$Parameter_name == c("k1'k2'/k2") & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1","b3") & apply(dat_ggplot[,c("CIL_0.95_Min", "CIU_0.95_Max")],1,function(x){all(is.na(x))}),]) 
  p <- p + geom_density(mapping = aes(x=Parameter_value, linetype=Batch), fill="black", alpha=0.1)
  p <- p + geom_segment(data = dat_ggplot[dat_ggplot$Gene_name %in% gene_cat[go,] & dat_ggplot$Parameter_name == "k1'k2'/k2" & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1","b3") & apply(dat_ggplot[,c("CIL_0.95_Min", "CIU_0.95_Max")],1,function(x){all(is.na(x))}),], aes(x = Parameter_value, xend=Parameter_value, y=0, yend=0.1, linetype=Batch, group = Gene_name)) 
  # p <- p + annotate("text", x = mean(unlist(dat_ggplot2[dat_ggplot2$Gene_name == "Egr1" & dat_ggplot2$Parameter_name %in% c("k1'k2'/k2") & dat_ggplot2$Condition == "naive",c("Parameter_value_b1","Parameter_value_b3")]), na.rm=T), y = 0.15, label = "Egr1", color="blue", fontface="bold", size=rel(6))
  p <- p + theme_bw() + theme(aspect.ratio=0.5) + labs(x="Effective Transport (log10)", fill="Parameter", color="Gene", y="Density", linetype="Replicate")
  p <- p + labs(title=go)
  print(p)
...
# violin plots
dat_ggplot_6 <- data.frame()
for ( go  in row.names(gene_cat)){
  for (gene in gene_cat[go,]){
    tmp <- dat_ggplot[dat_ggplot$Gene_name == gene &dat_ggplot$Parameter_name == c("k1'k2'/k2") & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1","b3") & apply(dat_ggplot[,c("CIL_0.95_Min", "CIU_0.95_Max")],1,function(x){all(is.na(x))}),]
    if(nrow(tmp) > 0){
      dat_ggplot_6 <- rbind(dat_ggplot_6, cbind(tmp, Go_term = go))
    }
  }
}

library(ggridges)
p <- ggplot(dat_ggplot_6, aes(y=Go_term, x=Parameter_value, fill = Go_term, linetype=Batch, shape=Batch)) 
p <- p + geom_density_ridges(mapping = aes(point_shape = Batch),
                             jittered_points = TRUE,
                             position = position_points_jitter(width = 0.05, height = 0), point_size = 3, point_alpha = 1, alpha = 0.7,
)
p + theme_bw()

dat_ggplot_7 <- dat_ggplot_6[dat_ggplot_6$Go_term %in% c("Pro-Inflammatory signals ", "Inactivation of mapk GO:0000188", "Inflammatory response TF", "negative regulation of nfkb (GO:0032088)", "death or survival genes" , "Tolerance genes", "Cytokines", "Chemokine"),]
dat_ggplot_7$Go_term <- factor(dat_ggplot_7$Go_term, levels=rev(c("Pro-Inflammatory signals ", "Inflammatory response TF", "Cytokines", "Chemokine", "Tolerance genes", "negative regulation of nfkb (GO:0032088)", "Inactivation of mapk GO:0000188", "death or survival genes")))
p <- ggplot(dat_ggplot_7, aes(y=Go_term, x=Parameter_value, fill = Go_term, linetype=Batch, shape=Batch)) 
p <- p + geom_density_ridges(mapping = aes(point_shape = Batch), panel_scaling=F, scale=0.95,
                             jittered_points = TRUE,
                             position = position_points_jitter(width = 0, height = 0), point_size = 2, point_alpha = 1, alpha = 0.7,
)
p + theme_bw() + labs(x="Effective Transport (log10)", y="", fill="") + theme(aspect.ratio=1.5)


# using gprofiler to get go term
library("gprofiler2")
gostres <- gost(query = unique(dat_ggplot$Gene_name), organism = "mmusculus", significant = T, user_threshold = 0.05, domain_scope = "annotated", exclude_iea = T, evcodes = TRUE)
head(gostres$result)

go_list <- c()
go_pass <- c()
pdf('go_term_distribution.pdf')
for ( go in gostres$result$term_name){
  if(!(go %in% c(go_list,go_pass))){
    p <- ggplot(dat_ggplot[dat_ggplot$Parameter_name == c("k1'k2'/k2") & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1","b3") & apply(dat_ggplot[,c("CIL_0.95_Min", "CIU_0.95_Max")],1,function(x){all(is.na(x))}),]) 
    p <- p + geom_density(mapping = aes(x=Parameter_value, linetype=Batch), fill="black", alpha=0.1)
    p <- p + geom_segment(data = dat_ggplot[dat_ggplot$Gene_name %in% unlist(strsplit(gostres$result$intersection[gostres$result$term_name == go], split=",")) & dat_ggplot$Parameter_name == "k1'k2'/k2" & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1","b3") & apply(dat_ggplot[,c("CIL_0.95_Min", "CIU_0.95_Max")],1,function(x){all(is.na(x))}),], aes(x = Parameter_value, xend=Parameter_value, y=0, yend=0.1, linetype=Batch, group = Gene_name)) 
    p <- p + geom_density(data = dat_ggplot[dat_ggplot$Gene_name %in% unlist(strsplit(gostres$result$intersection[gostres$result$term_name == go], split=",")) & dat_ggplot$Parameter_name == "k1'k2'/k2" & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1","b3") & apply(dat_ggplot[,c("CIL_0.95_Min", "CIU_0.95_Max")],1,function(x){all(is.na(x))}),], aes(x=Parameter_value, linetype=Batch), fill="red", alpha=0.1) 
    # p <- p + annotate("text", x = mean(unlist(dat_ggplot2[dat_ggplot2$Gene_name == "Egr1" & dat_ggplot2$Parameter_name %in% c("k1'k2'/k2") & dat_ggplot2$Condition == "naive",c("Parameter_value_b1","Parameter_value_b3")]), na.rm=T), y = 0.15, label = "Egr1", color="blue", fontface="bold", size=rel(6))
    p <- p + theme_bw() + theme(aspect.ratio=0.5) + labs(x="Effective Transport (log10)", fill="Parameter", color="Gene", y="Density", linetype="Replicate")
    p <- p + labs(title=paste0(go, " - ", gostres$result$term_id[gostres$result$term_name == go]))
    print(p)
    #  tmp <- readline()
    # if(tmp == "q"){
    #  break
    # }else if(tmp == "k"){
    #   go_list <- c(go_list, go)
    # }else if(tmp == "p"){
    #   go_pass <- c(go_pass, go)
    # }
  }
} 
dev.off()

go_list_id <- paste0("GO:", c("0042127","0050790","0008284","0010942","0048585","0045785","0060326",
                              "0019221","0002684","0043068","0045859","0070372","0043549","0051338",
                              "0044092","0000188","0060548","0051348","0033673","0043086"))
...
# difference in the mean based on go term
library(GOfuncR)
go_id <- get_anno_categories(unique(dat_ggplot$Gene_name[dat_ggplot$Condition == "naive"]),database = 'Mus.musculus')
go_id <- c(unique(go_id$go_id), sapply(unique(go_id$go_id), function(x){get_parent_nodes(go_ids = x)$parent_go_id}))
length(go_id)

go_id4 <- get_anno_genes(go_ids = go_id, database = "Mus.musculus")
go_id4 <- go_id4[go_id4$gene %in% unique(dat_ggplot$Gene_name[dat_ggplot$Condition == "naive"]),]
go_id4 <- data.frame(unique(go_id4$go_id), sapply(unique(go_id4$go_id), function(x){sum(go_id4$gene[go_id4$go_id ==x] %in% unique(dat_ggplot$Gene_name[dat_ggplot$Condition == "naive"]))}))
go_id4 <- go_id4[go_id4[,2] >=4,]

dim(go_id4)

pdf('go_term_distribution.pdf')
for (go in sort(go_id4[,1])){
  genes_go <- get_anno_genes(go_ids = go, database = "Mus.musculus") 
  genes_go <- genes_go$gene[genes_go$gene %in% unique(dat_ggplot$Gene_name[dat_ggplot$Condition == "naive"])]
  
  test <- t.test(x = dat_ggplot$Parameter_value[dat_ggplot$Parameter_name == c("k1'k2'/k2") & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1","b3") & apply(dat_ggplot[,c("CIL_0.95_Min", "CIU_0.95_Max")],1,function(x){all(is.na(x))})],
                 dat_ggplot$Parameter_value[dat_ggplot$Gene_name %in% genes_go & dat_ggplot$Parameter_name == c("k1'k2'/k2") & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1","b3") & apply(dat_ggplot[,c("CIL_0.95_Min", "CIU_0.95_Max")],1,function(x){all(is.na(x))})])
  
  test_ks <- ks.test(dat_ggplot$Parameter_value[dat_ggplot$Parameter_name == c("k1'k2'/k2") & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1","b3") & apply(dat_ggplot[,c("CIL_0.95_Min", "CIU_0.95_Max")],1,function(x){all(is.na(x))})],
                     dat_ggplot$Parameter_value[dat_ggplot$Gene_name %in% genes_go & dat_ggplot$Parameter_name == c("k1'k2'/k2") & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1","b3") & apply(dat_ggplot[,c("CIL_0.95_Min", "CIU_0.95_Max")],1,function(x){all(is.na(x))})])
  
  if(test$p.value < 0.05 | test_ks$p.value < 0.05 ){
    print(go)
    p <- ggplot(dat_ggplot[dat_ggplot$Parameter_name == c("k1'k2'/k2") & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1","b3") & apply(dat_ggplot[,c("CIL_0.95_Min", "CIU_0.95_Max")],1,function(x){all(is.na(x))}),]) 
    p <- p + geom_density(mapping = aes(x=Parameter_value, linetype=Batch), fill="black", alpha=0.1)
    p <- p + geom_segment(data = dat_ggplot[dat_ggplot$Gene_name %in% genes_go & dat_ggplot$Parameter_name == "k1'k2'/k2" & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1","b3") & apply(dat_ggplot[,c("CIL_0.95_Min", "CIU_0.95_Max")],1,function(x){all(is.na(x))}),], aes(x = Parameter_value, xend=Parameter_value, y=0, yend=0.1, linetype=Batch, group = Gene_name)) 
    p <- p + geom_density(data = dat_ggplot[dat_ggplot$Gene_name %in% genes_go & dat_ggplot$Parameter_name == "k1'k2'/k2" & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1","b3") & apply(dat_ggplot[,c("CIL_0.95_Min", "CIU_0.95_Max")],1,function(x){all(is.na(x))}),], aes(x=Parameter_value, linetype=Batch), fill="red", alpha=0.1) 
    # p <- p + annotate("text", x = mean(unlist(dat_ggplot2[dat_ggplot2$Gene_name == "Egr1" & dat_ggplot2$Parameter_name %in% c("k1'k2'/k2") & dat_ggplot2$Condition == "naive",c("Parameter_value_b1","Parameter_value_b3")]), na.rm=T), y = 0.15, label = "Egr1", color="blue", fontface="bold", size=rel(6))
    p <- p + theme_bw() + theme(aspect.ratio=0.5) + labs(x="Effective Transport (log10)", fill="Parameter", color="Gene", y="Density", linetype="Replicate")
    p <- p + labs(title=paste0(go, " - ", get_names(go)$go_name))
    print(p)
  }
}
dev.off()
...
go_list_id <- paste0("GO:", c("0000187", "0000188", # activation of MAPK activity
                              "0001558", "0030307", "0030308", # regulation of cell growth
                              "0001817", "0001818", "0001819", # regulation of cytokine production
                              "0001932", "0001933", "0001934", # regulation of protein phosphorylation
                              "0002250", # adaptive immune response
                              "0002819", "0002820", "0002821", # regulation of adaptive immune response
                              "0002768", # regulating cell surface receptor signalling pathway
                              "0005794", # Golgi apparatus
                              "0045859", "0045860", "0006469", # regulation of protein kinase activity
                              "0006935", # chemotaxis 
                              "0006959", # humoral immune response
                              "0002920", "0002921", "0002922", # regulation of humoral immune response
                              "0030155", "0007162", "0045785", # regulation of cell adhesion
                              "0008083", # growth factor activity 
                              "0008134", # transcription factor binding
                              "0008283", # cell population proliferation
                              "0042127", "0008284", "0008285", # regulation of cell population proliferation
                              "0010941", "0010942", "0060548", # regulation of cell death
                              "0043549", "0033673", "0033674", # regulation of kinase activity
                              "0042325", "0042326", "0042327", # regulation of phosphorylation
                              "0042981", "0043065", "0043066", # regulation of apoptotic process 
                              "0043067", "0043068", "0043069", # regulation of programmed cell death 
                              "0043405", "0043406", "0043407", # regulation of MAP kinase activity
                              "0043408", "0043409", "0043410", # regulation of MAPK cascade
                              "0060326"  # cell chemotaxis
))

pdf('go_term_distribution_v2.pdf')
for (go in go_list_id){
  genes_go <- get_anno_genes(go_ids = go, database = "Mus.musculus") 
  genes_go <- genes_go$gene[genes_go$gene %in% unique(dat_ggplot$Gene_name[dat_ggplot$Condition == "naive"])]
  
  if(length(genes_go) != 0 ){
    # test <- t.test(x = dat_ggplot$Parameter_value[dat_ggplot$Parameter_name == c("k1'k2'/k2") & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1","b3") & apply(dat_ggplot[,c("CIL_0.95_Min", "CIU_0.95_Max")],1,function(x){all(is.na(x))})],
    #                dat_ggplot$Parameter_value[dat_ggplot$Gene_name %in% genes_go & dat_ggplot$Parameter_name == c("k1'k2'/k2") & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1","b3") & apply(dat_ggplot[,c("CIL_0.95_Min", "CIU_0.95_Max")],1,function(x){all(is.na(x))})])
...
go_list_id <- paste0("GO:", c("0000187", "0000188", # activation of MAPK activity
                              "0001558", # regulation of cell growth
                              "0001819", "0001818", # regulation of cytokine production
                              "0001932", # regulation of protein phosphorylation
                              "0002250", # adaptive immune response
                              "0006959", # humoral immune response
                              "0002768", # regulating cell surface receptor signalling pathway
                              # "0005794", # Golgi apparatus
                              "0045860", "0006469", # regulation of protein kinase activity
                              # "0006935", # chemotaxis 
                              "0045785", "0007162",  # regulation of cell adhesion
                              "0008083", # growth factor activity 
                              "0008134", # transcription factor binding
                              "0033674", "0033673", # regulation of kinase activity
                              "0060326"  # cell chemotaxis
))

dat_ggplot_6 <- data.frame()
for ( go  in go_list_id){
  name <- get_names(go)$go_name
  print(name)
  ...
  for (gene in genes_go){
    tmp <- dat_ggplot[dat_ggplot$Gene_name == gene &dat_ggplot$Parameter_name == c("k1'k2'/k2") & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1","b3") & apply(dat_ggplot[,c("CIL_0.95_Min", "CIU_0.95_Max")],1,function(x){all(is.na(x))}),]
    if(nrow(tmp) > 0){
      dat_ggplot_6 <- rbind(dat_ggplot_6, cbind(tmp, Go_term = go, Go_names = name))
    }
  }
}

library(ggridges)
dat_ggplot_6$Go_names <- factor(dat_ggplot_6$Go_names, levels=rev(unique(dat_ggplot_6$Go_names)))
p <- ggplot(dat_ggplot_6, aes(y=Go_names, x=Parameter_value, fill = Go_names, linetype=Batch, shape=Batch)) 
p <- p + geom_density_ridges(mapping = aes(point_shape = Batch), panel_scaling=F, scale=0.95,
                             jittered_points = TRUE,
                             position = position_points_jitter(width = 0, height = 0), point_size = 2, point_alpha = 1, alpha = 0.7,
)
...
# Figure 4 E
gene_cat <- read.csv('../Category_genes_Test_v2.csv', header=F, row.names = 1)
...
gene_cat <- list("Cell growth genes" = c("Cd44", "Cdkn1a", "Cxcl16", "Dcbld2", "Edn1", "Gng4", "Hbegf", "Ifrd1", "Mmp14", "Sphk1"),
                 ...
                 "Cell adhesion genes" =  c("Adora2a", "Bmp2", "Cav1", "Ccl2", "Ccl5", "Cd44", "Cd74", "Cd83", "Dusp1", "Icam1", "Icosl", "Ifnb1", "Il10", "Il12b", "Il1b", "Il1rn", "Il23a", "Il6", "Irf1", "Itga5", "Itgav", "Jag1", "Malt1", "Mmp14", "Nfkbid", "Pdpn", "Plau", "Rnd1", "S100a10", "Sdc4", "Serpine1", "Spp1", "Tgm2", "Tnf", "Tnfaip3", "Tnfsf4", "Tnfsf9", "Nlrp3", "Src"),
                 ...
                 "Tolerance genes" = c("Nfkbia", "Tnfaip3", "Nfkb2", "Nfkbid", "Nfkbib", "Tnfaip2", "Nfkbie", "Dusp1", "Dusp2", "Zfp36", "Socs3", "Dusp4", "Dusp8", "Dusp14", "Dusp16", "Dusp5", "Phlda1", "Il1rn"),
                 ...
                 "Negative regulation of NFKB" = c("Nfkbia", "Nfkbib", "Nfkbid", "Tnfaip3", "Nfkb2", "Tnfaip2", "Nfkbie"),
                 "Cytokines" = c("Tnf", "Ifnb1", "Il1a", "Il10", "Il6", "Il1b", "Il23a", "Il12b"),
                 "Chemokine" = c("Cxcl10", "Ccl7", "Cxcl2", "Cxcl1", "Ccl2", "Ccl4", "Ccl12", "Ccl9", "Cxcl9", "Ccl5", "Cxcl16"))

dat_ggplot_6 <- cbind(dat_ggplot[dat_ggplot$Parameter_name == c("k1'k2'/k2") & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1","b3") & apply(dat_ggplot[,c("CIL_0.95_Min", "CIU_0.95_Max")],1,function(x){all(is.na(x))}),], Cat = "All induced genes")
...
dat_ggplot_6$Batch <- factor(dat_ggplot_6$Batch, levels = c("b1","b3"), labels=c("Rep1", "Rep2"))
...
dat_lpa <- data.frame(batch = "b1", 
                      do.call("rbind", lapply(1:length(all_param_CI$lpa$b1), function(i){cbind(Gene=names(all_param_CI$lpa$b1)[i],all_param_CI$lpa$b1[[i]]$CI[,c(1:4,7:8)])})))
dat_lpa <- rbind(dat_lpa, data.frame(batch = "b3", 
                                     do.call("rbind", lapply(1:length(all_param_CI$lpa$b3), function(i){cbind(Gene=names(all_param_CI$lpa$b3)[i],all_param_CI$lpa$b3[[i]]$CI[,c(1:4,7:8)])}))))

dat_lpa$batch <- factor(dat_lpa$batch, levels=c("b1", "b3"), labels=c("Rep1", "Rep2"))
...
p <- ggplot(dat_ggplot[dat_ggplot$Parameter_name %in% c("k1'", "k2", "k2'", "kdeg") & dat_ggplot$Condition == "lpa" & dat_ggplot$Batch %in% c("b1", "b3") & is.na(dat_ggplot$CIL_0.95_Min) & is.na(dat_ggplot$CIU_0.95_Max),]) 
p <- p + geom_density(mapping = aes(x=Parameter_value, fill=Parameter_name, color=Parameter_name, linetype=Batch), alpha=0.1)
p <- p + scale_color_manual(values=c("red", "yellow","green", "blue")) 
p <- p + scale_fill_manual(values=c("red", "yellow", "green", "blue"))
p <- p + scale_linetype_manual(values=c("b1"=1, "b3" = 2), labels = c("Rep1", "Rep3"))
...
for (condition in c('naive', 'lpa')){
  ...
  for (batch in c('all','b1','b3','b2')){
    sheet_name <- paste0(condition_name, "_", c("Together", "Rep1", "Rep2", "Rep3")[c('all','b1','b3','b2') ==batch])
...
openxlsx::saveWorkbook(wb, "../Supplementary Documents/Parameters_fit_v2.xlsx", overwrite = TRUE)
...
make_condition_plot2 <- function(data, batch, parameter, xylim=NULL, color_facet=NULL, font_size=12, legend = "none"){
  ...
  p <- p + geom_abline(slope=1, intercept=0, linetype=1, col='black', alpha=0.5) + geom_abline(slope=1, intercept=log10(2), linetype=2, col='red', alpha=0.5) + geom_abline(slope=1, intercept=-log10(2), linetype=2, col='red', alpha=0.5)
  p <- p + scale_shape_manual(values=c("all"=18, "b1"=16,"b2"=15,"b3"=17), labels = function(x){gsub('b','Rep',x)})
...
p <- make_condition_plot2(data = dat_ggplot3, batch = c('b1','b3'), parameter =  "k1'", color_facet = "red", xylim= c(-2,0.5),legend = "none", font_size=24)
p
# dev.off()

# svg(filename = 'Figures/Fig6B_k2_v2.svg', width=5, height=5)
p <- make_condition_plot2(data = dat_ggplot3, batch = c('b1','b3'), parameter =  "k2", color_facet = "yellow", xylim= c(-3.5,0),legend = "none", font_size=24)
p
# dev.off()

# svg(filename = 'Figures/Fig6B_k2prime_v2.svg', width=5, height=5)
p <- make_condition_plot2(data = dat_ggplot3, batch = c('b1','b3'), parameter =  "k2'", color_facet = "green", xylim= c(-3,0),legend = "none", font_size=24)
p
# dev.off()

# svg(filename = 'Figures/Fig6B_kdeg_v2.svg', width=5, height=5)
p <- make_condition_plot2(data = dat_ggplot3, batch = c('b1','b3'), parameter =  "kdeg", color_facet = "blue", xylim = c(-4,-0.5), legend = "none", font_size=24)
p
# dev.off()

# svg(filename = 'Figures/Fig6B_legend_v2.svg', width=5, height=5)
p <- make_condition_plot2(data = dat_ggplot3, batch = c('b1','b3'), parameter =  "kdeg", color_facet = "blue", xylim= c(-4,0.5),legend = "only", font_size=24)
grid.draw(p)
# dev.off()

# svg(filename = 'Figures/Fig6B_k1prime_over_k2_v2.svg', width=5, height=5)
p <- make_condition_plot2(data = dat_ggplot3, batch = c('b1','b3'), parameter =  "k1'/k2", xylim= c(-0.2,2),legend = "none", font_size=24)
p
# dev.off()

# svg(filename = 'Figures/Fig6B_k1primek2prime_over_k2_v2.svg', width=5, height=5)
p <- make_condition_plot2(data = dat_ggplot3, batch = c('b1','b3'), parameter =  "k1'k2'/k2", xylim= c(-3,0.5),legend = "none", font_size=24)
p
# dev.off()

# svg(filename = 'Figures/Fig6B_k2prime_over_k2_v2.svg', width=5, height=5)
p <- make_condition_plot2(data = dat_ggplot3, batch = c('b1','b3'), parameter =  "k2'/k2", xylim= c(-2,2),legend = "none", font_size=24)
p
# dev.off()
...
dat_ggplot_HL_2 <- cbind(dat_ggplot_HL2[dat_ggplot_HL2$Batch == 'b1',], dat_ggplot_HL2[dat_ggplot_HL2$Batch == 'b3',c(13,23:36)])
colnames(dat_ggplot_HL_2)[37:51] <- paste(colnames(dat_ggplot_HL_2)[c(13,23:36)], '_b3', sep='')
...
#### Test other genes ####-------------------------------------
dat_genes <- rbind(cbind(dat_g_Tnf, Name="Tnf"),
                   cbind(dat_g_Zhx2, Name="Zhx2"), 
                   cbind(dat_g_Tnfaip3, Name="Tnfaip3"),
                   cbind(dat_g_Fos,Name="Fos"),
                   cbind(dat_g_Arl5c, Name="Arl5c"),
                   cbind(dat_g_Snx18, Name="Snx18"),
                   cbind(dat_g_Cd44, Name="Cd44"),
                   cbind(dat_g_Socs3, Name="Socs3"),
                   cbind(dat_g_Nfkbie, Name="Nfkbie"))
dat_genes$Cpt <- factor(dat_genes$Cpt, levels=c("Chromatin", "Nucleoplasm", "Cytoplasm"),ordered=T)

# svg(filename = 'Figures/Figure4C_Malt1_Egr1.svg', width=7, height=3)
p <- ggplot(data = dat_genes[dat_genes$Condition == "naive",], mapping = aes(x=Time,y=RPKM, col=Name, group=interaction(Name,batch))) + geom_point(data = dat_genes[dat_genes$Type == "Experiment" & dat_genes$Condition == "naive",]) + facet_grid(.~Cpt)
p <- p + geom_line(data=dat_genes[dat_genes$Type == "Fit" & dat_genes$Condition == "naive",], alpha=0.5)
p <- p + theme_bw() + theme(aspect.ratio=1) + scale_x_continuous(breaks = c(0,30,60,90,120))
p <- p + labs(x="Time (min)", y=parse(text="log[2](FPKM)"))
g <- ggplotGrob(p)
col <- c("Chromatin" = col_ca, "Nucleoplasm" = col_np, "Cytoplasm" = col_cyto)
for(i in grep("strip-t",g$layout$name)){
  Cpt <- g$grobs[[i]]$grobs[[1]]$children[[grep("text", names(g$grobs[[i]]$grobs[[1]]$children))]]$children[[1]]$label
  g$grobs[[i]]$grobs[[1]]$children[[grep("rect", names(g$grobs[[i]]$grobs[[1]]$children))]]$gp$fill <- col[Cpt]
}
grid.draw(g)
...
log(2)/10^unlist(all_best_param$naive$b3[c("Tnf", "Zhx2", "Tnfaip3", "Fos", "Arl5c", "Snx18", "Cd44", "Socs3", "Nfkbie"), "kdeg"])


#### Process Intron data #####---------------------------------------------------------

# PI_naive_b1 <- read.table('../../../../SQUID/npRNA_timeseries/Naive/SQUID/Nuc.Naive.b1/Result/intron_PI.txt', header=T, stringsAsFactors = F)
# PI_naive_b3 <- read.table('../../../../SQUID/npRNA_timeseries/Naive/SQUID/Nuc.Naive.b3/Result/intron_PI.txt', header=T, stringsAsFactors = F)
# PI_lpa_b1 <- read.table('../../../../SQUID/npRNA_timeseries/LPA/SQUID/Nuc.LPA.b1/Result/intron_PI.txt', header=T, stringsAsFactors = F)
# PI_lpa_b3 <- read.table('../../../../SQUID/npRNA_timeseries/LPA/SQUID/Nuc.LPA.b3/Result/intron_PI.txt', header=T, stringsAsFactors = F)

PI_naive_b1 <- read.table('SQUID/Naive/Nuc.Naive.b1.intron_PI.txt', header=T, stringsAsFactors = F)
PI_naive_b3 <- read.table('SQUID/Naive/Nuc.Naive.b3.intron_PI.txt', header=T, stringsAsFactors = F)
PI_lpa_b1 <- read.table('SQUID/LPA/Nuc.LPA.b1.intron_PI.txt', header=T, stringsAsFactors = F)
PI_lpa_b3 <- read.table('SQUID/LPA/Nuc.LPA.b3.intron_PI.txt', header=T, stringsAsFactors = F)

ncol <- max(count.fields('SQUID/Intron_transcript.txt', sep = "\t"))
intron_transcript_mapping <- read.table('SQUID/Intron_transcript.txt', header=F, col.names=paste0('V', seq_len(ncol)), skip = 0, fill = T, stringsAsFactors = F)

PI_naive_b1$Time <- paste0(c(10,15,20,25,30,35,40,50,60,90,120), collapse=",")
PI_naive_b3$Time <- paste0(c(0,10,15,20,25,30,40,50,60,75,90,120), collapse=",")
PI_lpa_b1$Time   <- paste0(c(0,10,15,20,25,30,35,40,50,60,90,120), collapse=",")
PI_lpa_b3$Time   <- paste0(c(0,10,15,20,25,30,40,50,60,75,90,120), collapse=",")

# select intron belonging to observed transcripts of induced genes
curated_list_naive <- read.table(file = "RBP/Curated list_naive.csv", header=T, sep ="\t")
...
g_keep <- c()
for(g in unique(PI_naive_b1$Gene_id)){
  if(all(intron_annotated$Attribute[intron_annotated$Gene_id == g] == "U")){
    g_keep <- c(g_keep, g)
  }
}
PI_naive_U <- list(b1 = PI_naive_b1[PI_naive_b1$Intron_id %in% intron_annotated$Intrond_id[intron_annotated$Gene_id %in% g_keep], ],
                   b3 = PI_naive_b3[PI_naive_b3$Intron_id %in% intron_annotated$Intrond_id[intron_annotated$Gene_id %in% g_keep], ])
...
for (g in intersect(PI_naive_U[['b1']]$Gene_id, PI_naive_U[['b3']]$Gene_id)){
  b1_g <- PI_naive_U[['b1']][PI_naive_U[[b]]$Gene_id==g,]
  b1_g$PI_Junction_avg_late[b1_g$PI_Junction_avg_late_count < count_threshold] <- NA
  b1_g$PI_Junction_wavg_late[b1_g$PI_Junction_wavg_late_count < count_threshold] <- NA
  
  b3_g <- PI_naive_U[['b3']][PI_naive_U[[b]]$Gene_id==g,]
  b3_g$PI_Junction_avg_late[b3_g$PI_Junction_avg_late_count < count_threshold] <- NA
  b3_g$PI_Junction_wavg_late[b3_g$PI_Junction_wavg_late_count < count_threshold] <- NA
  
  b3_g <- b3_g[match(b1_g$Intron_id,b3_g$Intron_id),]
  
  PI_naive_avg_avg_late <- mean(rowMeans(cbind(b1_g$PI_Junction_avg_late, b3_g$PI_Junction_avg_late)))
  PI_naive_avg_wavg_late <- mean(rowMeans(cbind(b1_g$PI_Junction_wavg_late, b3_g$PI_Junction_wavg_late)))
  
  PI_naive_sd_avg_late <- sd(c(b1_g$PI_Junction_avg_late, b3_g$PI_Junction_avg_late))
  PI_naive_sd_wavg_late <- sd(c(b1_g$PI_Junction_wavg_late, b3_g$PI_Junction_wavg_late))
  
  PI_naive_max_avg_late <- max(rowMeans(cbind(b1_g$PI_Junction_avg_late, b3_g$PI_Junction_avg_late))) 
  PI_naive_max_wavg_late <- max(rowMeans(cbind(b1_g$PI_Junction_wavg_late, b3_g$PI_Junction_wavg_late))) 
  
  PI_naive_prod_avg_late <- prod(1- rowMeans(cbind(b1_g$PI_Junction_avg_late, b3_g$PI_Junction_avg_late)))
  PI_naive_prod_wavg_late <- prod(1- rowMeans(cbind(b1_g$PI_Junction_wavg_late, b3_g$PI_Junction_wavg_late)))
...
splicing_prob_for_kevin <- data.frame()
for (g in unique(all_intron_summary_measure[['b1']]$GeneId, all_intron_summary_measure[['b3']]$GeneId)){
  tmp <- data.frame("GeneId" = g, 'Name'= unique(all_intron_summary_measure[['b1']]$Name, all_intron_summary_measure[['b3']]$Name)[match(g, unique(all_intron_summary_measure[['b1']]$GeneId, all_intron_summary_measure[['b3']]$GeneId))])
  for (b in c('b1', 'b3', 'all')){
  ...
  colnames(tmp) <- c('GeneId', 'Name', 'b1', 'b3', 'Avg')
  splicing_prob_for_kevin <- rbind(splicing_prob_for_kevin, tmp)
}

saveRDS(splicing_prob_for_kevin, file='Figures/Splicing_proba_kevin.Rds')
...
dat_ggplot5_naive_b1b3 <- dat_ggplot5_naive[dat_ggplot5_naive$Batch %in% c('b1','b3'),]


cor.test(dat_ggplot5_naive_b1b3$transport[dat_ggplot5_naive_b1b3$Batch == "b1" & apply(dat_ggplot5_naive_b1b3[,c(4,8)],1,function(x){all(is.na(x))})], log10(dat_ggplot5_naive_b1b3$Length[dat_ggplot5_naive_b1b3$Batch == "b1" & apply(dat_ggplot5_naive_b1b3[,c(4,8)],1,function(x){all(is.na(x))})]), use="pairwise.complete.obs")
cor.test(dat_ggplot5_naive_b1b3$transport[dat_ggplot5_naive_b1b3$Batch == "b3" & apply(dat_ggplot5_naive_b1b3[,c(4,8)],1,function(x){all(is.na(x))})], log10(dat_ggplot5_naive_b1b3$Length[dat_ggplot5_naive_b1b3$Batch == "b3" & apply(dat_ggplot5_naive_b1b3[,c(4,8)],1,function(x){all(is.na(x))})]), use="pairwise.complete.obs")


cor_test <- cor.test(dat_ggplot5_naive_b1b3$transport[apply(dat_ggplot5_naive_b1b3[,c(4,8)],1,function(x){all(is.na(x))})], log10(dat_ggplot5_naive_b1b3$Length[ apply(dat_ggplot5_naive_b1b3[,c(4,8)],1,function(x){all(is.na(x))})]), use="pairwise.complete.obs")
...
p <- ggplot(dat_ggplot5_naive_b1b3[apply(dat_ggplot5_naive_b1b3[,c(4,8)],1,function(x){all(is.na(x))}),], aes(y=transport, x=Length, col=type)) + geom_point(mapping=aes(shape=Batch))
...
cor.test(dat_ggplot5_naive_b1b3$transport[dat_ggplot5_naive_b1b3$Batch == "b1" & apply(dat_ggplot5_naive_b1b3[,c(4,8)],1,function(x){all(is.na(x))})], dat_ggplot5_naive_b1b3$Nb_introns[dat_ggplot5_naive_b1b3$Batch == "b1" & apply(dat_ggplot5_naive_b1b3[,c(4,8)],1,function(x){all(is.na(x))})], use="pairwise.complete.obs")
cor.test(dat_ggplot5_naive_b1b3$transport[dat_ggplot5_naive_b1b3$Batch == "b3" & apply(dat_ggplot5_naive_b1b3[,c(4,8)],1,function(x){all(is.na(x))})], dat_ggplot5_naive_b1b3$Nb_introns[dat_ggplot5_naive_b1b3$Batch == "b3" & apply(dat_ggplot5_naive_b1b3[,c(4,8)],1,function(x){all(is.na(x))})], use="pairwise.complete.obs")


cor_test <- cor.test(dat_ggplot5_naive_b1b3$transport[apply(dat_ggplot5_naive_b1b3[,c(4,8)],1,function(x){all(is.na(x))})], dat_ggplot5_naive_b1b3$Nb_introns[apply(dat_ggplot5_naive_b1b3[,c(4,8)],1,function(x){all(is.na(x))})], use="pairwise.complete.obs")


fit_intron <- lmer(transport ~ Nb_introns + (1 | Gene_name), data = dat_ggplot5_naive_b1b3)
tmp <- summary(fit_intron)


# svg(filename = 'Figures/Figure5B_transport_vs_Intons_v3.svg', width=8,height=4)
p <- ggplot(dat_ggplot5_naive_b1b3[apply(dat_ggplot5_naive_b1b3[,c(4,8)],1,function(x){all(is.na(x))}),], aes(y=transport, x=Nb_introns, col=type)) + geom_point(mapping=aes(shape=Batch))
...
cor(log10(dat3$PI_naive_prod_avg_late)[ dat3$Batch == 'b1' & apply(dat3[,c("CIL_0.95_Min_transport","CIU_0.95_Max_transport")],1, function(x){all(is.na(x))})], dat3$`k1'k2'/k2`[ dat3$Batch == 'b1' & apply(dat3[,c("CIL_0.95_Min_transport","CIU_0.95_Max_transport")],1, function(x){all(is.na(x))})], use="pairwise.complete.obs")
cor(log10(dat3$PI_naive_prod_avg_late)[dat3$Batch == 'b3' & apply(dat3[,c("CIL_0.95_Min_transport","CIU_0.95_Max_transport")],1, function(x){all(is.na(x))})], dat3$`k1'k2'/k2`[ dat3$Batch == 'b3' & apply(dat3[,c("CIL_0.95_Min_transport","CIU_0.95_Max_transport")],1, function(x){all(is.na(x))})], use="pairwise.complete.obs")
...
cor(dat3$kdeg[dat3$Batch == 'b1' & apply(dat3[,c("CIL_0.95_Min_kdeg","CIU_0.95_Max_kdeg","CIL_0.95_Min_transport","CIU_0.95_Max_transport")],1, function(x){all(is.na(x))})], dat3$`k1'k2'/k2`[dat3$Batch == 'b1' & apply(dat3[,c("CIL_0.95_Min_kdeg","CIU_0.95_Max_kdeg", "CIL_0.95_Min_transport","CIU_0.95_Max_transport")],1, function(x){all(is.na(x))})], use="pairwise.complete.obs")
cor(dat3$kdeg[dat3$Batch == 'b3' & apply(dat3[,c("CIL_0.95_Min_kdeg","CIU_0.95_Max_kdeg", "CIL_0.95_Min_transport","CIU_0.95_Max_transport")],1, function(x){all(is.na(x))})], dat3$`k1'k2'/k2`[dat3$Batch == 'b3' & apply(dat3[,c("CIL_0.95_Min_kdeg","CIU_0.95_Max_kdeg", "CIL_0.95_Min_transport","CIU_0.95_Max_transport")],1, function(x){all(is.na(x))})], use="pairwise.complete.obs")
...
cor(log10(dat3$PI_naive_prod_avg_late)[dat3$Batch == 'b1' & apply(dat3[,c("CIL_0.95_Min_kdeg","CIU_0.95_Max_kdeg")],1, function(x){all(is.na(x))})], dat3$kdeg[dat3$Batch == 'b1' & apply(dat3[,c("CIL_0.95_Min_kdeg","CIU_0.95_Max_kdeg")],1, function(x){all(is.na(x))})], use="pairwise.complete.obs")
cor(log10(dat3$PI_naive_prod_avg_late)[dat3$Batch == 'b3' & apply(dat3[,c("CIL_0.95_Min_kdeg","CIU_0.95_Max_kdeg")],1, function(x){all(is.na(x))})], dat3$kdeg[dat3$Batch == 'b3' & apply(dat3[,c("CIL_0.95_Min_kdeg","CIU_0.95_Max_kdeg")],1, function(x){all(is.na(x))})], use="pairwise.complete.obs")
...
cor.test(log10(dat3$PI_naive_prod_avg_late)[dat3$Batch == "b1" & apply(dat3[,c("CIL_0.95_Min_transport","CIU_0.95_Max_transport")],1, function(x){all(is.na(x))})], dat3$`k1'k2'/k2`[dat3$Batch == "b1" & apply(dat3[,c("CIL_0.95_Min_transport","CIU_0.95_Max_transport")],1, function(x){all(is.na(x))})], use="pairwise.complete.obs")
cor.test(log10(dat3$PI_naive_prod_avg_late)[dat3$Batch == "b3" & apply(dat3[,c("CIL_0.95_Min_transport","CIU_0.95_Max_transport")],1, function(x){all(is.na(x))})], dat3$`k1'k2'/k2`[dat3$Batch == "b3" & apply(dat3[,c("CIL_0.95_Min_transport","CIU_0.95_Max_transport")],1, function(x){all(is.na(x))})], use="pairwise.complete.obs")
...
dat_ggplot5_naive_b1b3$PI_naive_prod_avg_late <- dat3$PI_naive_prod_avg_late[match(interaction(dat_ggplot5_naive_b1b3$Gene_name, dat_ggplot5_naive_b1b3$Batch),interaction(dat3$Name, dat3$Batch))]

fit_splicing <- rlm(transport ~ PI_naive_prod_avg_late, data = dat3)
tmp <- summary(fit_splicing)

dat3_b1b3 <- dat3[dat3$Batch %in% c("b1","b3"),]

library(ggrepel)
cor_test <- cor.test(log10(dat3_b1b3$PI_naive_prod_avg_late)[apply(dat3_b1b3[,c("CIL_0.95_Min_transport","CIU_0.95_Max_transport")],1, function(x){all(is.na(x))})], dat3_b1b3$`k1'k2'/k2`[apply(dat3_b1b3[,c("CIL_0.95_Min_transport","CIU_0.95_Max_transport")],1, function(x){all(is.na(x))})], use="pairwise.complete.obs")

# svg(filename = 'Figures/Figure5C_transport vs PI_v3.svg', width=4, height=4)
p <- ggplot(dat3_b1b3[apply(dat3_b1b3[,c("CIL_0.95_Min_transport","CIU_0.95_Max_transport")],1, function(x){all(is.na(x))}),], aes(x=PI_naive_prod_avg_late, y=`k1'k2'/k2`, col=type)) + geom_point(mapping = aes(shape=Batch)) 
p <- p + theme_bw() + theme(aspect.ratio=1)
p <- p + scale_x_continuous(trans="log10") + scale_shape_discrete(labels=c("b1"="Rep1", "b3"="Rep2")) 
...
dat4 <- dat3_b1b3[apply(dat3_b1b3[,c("CIL_0.95_Min_transport","CIU_0.95_Max_transport")],1, function(x){all(is.na(x))}),]
...
cor_test <- cor.test(log10(dat3_b1b3$PI_naive_prod_avg_late)[apply(dat3_b1b3[,c("CIL_0.95_Min_transport","CIU_0.95_Max_transport")],1, function(x){all(is.na(x))})], dat3_b1b3$`k1'k2'/k2`[apply(dat3_b1b3[,c("CIL_0.95_Min_transport","CIU_0.95_Max_transport")],1, function(x){all(is.na(x))})], use="pairwise.complete.obs")
# 
# # svg(filename = 'Figures/Figure5C_transport vs PI.svg', width=4, height=4)
# # p <- ggplot(dat, aes(x=PI_naive_prod_avg_late, y=`k1'k2'/k2`, color=HL)) + geom_point(mapping = aes(shape=Batch)) 
# d <- highlight_key(dat3, ~Name )
# p <- ggplot(d, aes(x=PI_naive_prod_avg_late, y=`k1'k2'/k2`)) + geom_point(mapping = aes(shape=Batch, group=Name)) 
# p <- p + scale_color_continuous(limits=log(2)/10^c(-0.5,-3), oob=squish, trans="log10")
# p <- p + theme_bw() + theme(aspect.ratio=1)
# p <- p + scale_x_continuous(trans="log10") + scale_shape_discrete(labels=c("b1"="Rep1", "b3"="Rep3")) 
# p <- p + labs(x = "prod(1-PI)", y="Relative Transport (log10)", color=parse(text="HL (min)"), shape="Replicate")
# p <- p + geom_segment(aes(x=PI_naive_prod_avg_late,xend=PI_naive_prod_avg_late, y= CIL_0.95_Max_transport, yend=CIU_0.95_Min_transport), alpha=0.05)
# p <- p + geom_segment(aes(x=PI_naive_prod_avg_late,xend=PI_naive_prod_avg_late, y= CIL_0.95_Min_transport, yend=CIL_0.95_Max_transport), alpha=0.05, linetype=2)
# p <- p + geom_segment(aes(x=PI_naive_prod_avg_late,xend=PI_naive_prod_avg_late, y= CIU_0.95_Min_transport, yend=CIU_0.95_Max_transport), alpha=0.05, linetype=2)
# p <- p + coord_cartesian(ylim=c(-2.5,0.5))
# # p <- p + annotate(geom = "text", x=0.001, y=0.5, hjust=0, vjust=1,label=bquote("Pearson's" ~ rho == .(signif(cor_test$estimate,2)) ~ ', p-value' == .(ifelse(cor_test$p.value < 2.2e-16, "< 2.2e-16", signif(cor_test$p.value,2)))))
# # p <- p + annotate(geom = "text", x=0.001, y=0.5, hjust=0, vjust=1,label=bquote("Pearson's" ~ rho == .(signif(cor_test$estimate,2))))
# # p <- p + geom_label_repel(aes(label = Name, color=Batch), label.size = 0, box.padding = 0, point.padding = 0, segment.color = 'grey50', size=2) 
# p <- p + theme(legend.position = "none")
# # dev.off()
# 
# pp <- ggplotly(p, tooltip="Name")
# highlight( pp, on = "plotly_hover", off = "plotly_deselect", color = "red" )
# 
# dat3$efficiency <- dat3$`k2'/k2`
# cor_test <- cor.test(log10(dat3$PI_naive_prod_avg_late), dat3$efficiency, use="pairwise.complete.obs")
# 
# p <- ggplot(dat3, aes(x=PI_naive_prod_avg_late, y=efficiency)) + geom_point(mapping = aes(shape=Batch)) 
# # p <- p + scale_color_continuous(limits=log(2)/10^c(-0.5,-3), oob=squish, trans="log10")
# p <- p + theme_bw() + theme(aspect.ratio=1)
# p <- p + scale_x_continuous(trans="log10") + scale_shape_discrete(labels=c("b1"="Rep1", "b3"="Rep3")) 
# p <- p + labs(x = parse(text="prod((1-PI[intron]), intron)"), y=bquote("Effiency "~(log[10])), color=parse(text="HL (min)"), shape="Replicate")
# p <- p + coord_cartesian(ylim=c(-2,1))
# # p <- p + annotate(geom = "text", x=0.001, y=0.5, hjust=0, vjust=1,label=bquote("Pearson's" ~ rho == .(signif(cor_test$estimate,2)) ~ ', p-value' == .(ifelse(cor_test$p.value < 2.2e-16, "< 2.2e-16", signif(cor_test$p.value,2)))))
# p <- p + annotate(geom = "text", x=0.001, y=0.5, hjust=0, vjust=1,label=bquote("Pearson's" ~ rho == .(signif(cor_test$estimate,2))))
# p + theme(legend.position = "none")
# 
# cor_test <- cor.test(log10(dat3$PI_naive_prod_avg_late)[apply(dat3[,c(23,27)],1,function(x){all(is.na(x))})], dat3$efficiency[apply(dat3[,c(23,27)],1,function(x){all(is.na(x))})], use="pairwise.complete.obs")
# 
# p <- ggplot(dat3[apply(dat3[,c(23,27)],1,function(x){all(is.na(x))}),], aes(x=PI_naive_prod_avg_late, y=efficiency)) + geom_point(mapping = aes(shape=Batch)) 
# # p <- p + scale_color_continuous(limits=log(2)/10^c(-0.5,-3), oob=squish, trans="log10")
# p <- p + theme_bw() + theme(aspect.ratio=1)
# p <- p + scale_x_continuous(trans="log10") + scale_shape_discrete(labels=c("b1"="Rep1", "b3"="Rep3")) 
# p <- p + labs(x = parse(text="prod((1-PI[intron]), intron)"), y=bquote("Effiency "~(log[10])), color=parse(text="HL (min)"), shape="Replicate")
...
cor_test <- cor.test(dat3_b1b3$PI_naive_max_avg_late[apply(dat3_b1b3[,c("CIL_0.95_Min_transport","CIU_0.95_Max_transport")],1,function(x){all(is.na(x))})], dat3_b1b3$`k1'k2'/k2`[apply(dat3_b1b3[,c("CIL_0.95_Min_transport","CIU_0.95_Max_transport")],1,function(x){all(is.na(x))})], use="pairwise.complete.obs")

fit_splicing <- rlm(transport ~ PI_naive_max_avg_late, data = dat3_b1b3)
tmp <- summary(fit_splicing)
# dev.off()
...
### Figure 5 - comparison linear models ##-------
dat3$Length <- curated_list_naive$Length[match(dat3$Name, curated_list_naive$Name)]/1000
dat3$Nb_introns <- curated_list_naive$NbIntron[match(dat3$Name, curated_list_naive$Name)]

dat_ggplot5_naive_all_bis <- dat3[dat3$Batch == "all" & !is.na(dat3$kdeg) & !is.na(dat3$PI_naive_prod_avg_late) & !is.na(dat3$Nb_introns) & !is.na(dat3$Length),]
dat_ggplot5_naive_b1b3_bis <- dat3[dat3$Batch %in% c("b1", "b3") & !is.na(dat3$kdeg) & !is.na(dat3$PI_naive_prod_avg_late) & !is.na(dat3$Nb_introns) & !is.na(dat3$Length),]

dat_ggplot5_naive_all_ter <- dat_ggplot5_naive_all_bis[apply(dat_ggplot5_naive_b1b3_bis[,c("CIL_0.95_Min_transport","CIU_0.95_Max_transport", "CIL_0.95_Min_kdeg","CIU_0.95_Max_kdeg")],1,function(x){all(is.na(x))}),]
dat_ggplot5_naive_b1b3_ter <- dat_ggplot5_naive_b1b3_bis[apply(dat_ggplot5_naive_b1b3_bis[,c("CIL_0.95_Min_transport","CIU_0.95_Max_transport", "CIL_0.95_Min_kdeg","CIU_0.95_Max_kdeg")],1,function(x){all(is.na(x))}),]

fit_length <- lmer(transport ~ Length + (1 | Name), data = dat_ggplot5_naive_b1b3_ter)
tmp <- summary(fit_length)
tmp$coefficients

fit_intron <- lmer(transport ~ Nb_introns + (1 | Name), data =dat_ggplot5_naive_b1b3_ter)
tmp <- summary(fit_intron)
tmp$coefficients

fit_splicing <- lmer(transport ~ PI_naive_prod_avg_late + (1 | Name), data = dat_ggplot5_naive_b1b3_ter)
tmp <- summary(fit_splicing)
tmp$coefficients

fit_kdeg <- lmer(transport ~ kdeg + (1 | Name), data = dat_ggplot5_naive_b1b3_ter)
tmp <- summary(fit_kdeg)
tmp$coefficients

# model 2 param
fit_length_intron <- lmer(transport ~ Length + Nb_introns + (1 | Name), data = dat_ggplot5_naive_b1b3_ter)
tmp <- summary(fit_length_intron)
tmp$coefficients

anova(fit_length_intron, fit_length)
anova(fit_length_intron, fit_intron) # => fit_intron

# model 2 param
fit_length_splicing <- lmer(transport ~ Length + PI_naive_prod_avg_late + (1 | Name), data = dat_ggplot5_naive_b1b3_ter)
tmp <- summary(fit_length_splicing)
tmp$coefficients

anova(fit_length_splicing, fit_length)
anova(fit_length_splicing, fit_splicing) # => fit_length_splicing

# model 2 param
fit_intron_splicing <- lmer(transport ~ Nb_introns + PI_naive_prod_avg_late + (1 | Name), data = dat_ggplot5_naive_b1b3_ter)
tmp <- summary(fit_intron_splicing)
tmp$coefficients

anova(fit_intron_splicing, fit_intron)
anova(fit_intron_splicing, fit_splicing) # => fit_intron_splicing

# model 2 param
fit_length_kdeg <- lmer(transport ~ Length + kdeg + (1 | Name), data = dat_ggplot5_naive_b1b3_ter)
tmp <- summary(fit_length_kdeg)
tmp$coefficients

anova(fit_length_kdeg, fit_length)
anova(fit_length_kdeg, fit_kdeg) # => fit_length_splicing

# model 2 param
fit_intron_kdeg <- lmer(transport ~ Nb_introns + kdeg + (1 | Name), data = dat_ggplot5_naive_b1b3_ter)
tmp <- summary(fit_intron_kdeg)
tmp$coefficients

anova(fit_intron_kdeg, fit_intron)
anova(fit_intron_kdeg, fit_kdeg) # => fit_intron_kdeg

# model 2 param
fit_splicing_kdeg <- lmer(transport ~ PI_naive_prod_avg_late + kdeg + (1 | Name), data = dat_ggplot5_naive_b1b3_ter)
tmp <- summary(fit_splicing_kdeg)
tmp$coefficients

anova(fit_splicing_kdeg, fit_splicing)
anova(fit_splicing_kdeg, fit_kdeg) # => fit_splicing_kdeg

# model 3 param
fit_length_intron_splicing <- lmer(transport ~ Length + Nb_introns + PI_naive_prod_avg_late + (1 | Name), data = dat_ggplot5_naive_b1b3_ter)
tmp <- summary(fit_length_intron_splicing)
tmp$coefficients

anova(fit_length_intron_splicing, fit_length_intron)
anova(fit_length_intron_splicing, fit_length_splicing) 
anova(fit_length_intron_splicing, fit_intron_splicing) # => fit_intron_splicing

# model 3 param
fit_length_intron_kdeg <- lmer(transport ~ Length + Nb_introns + kdeg + (1 | Name), data = dat_ggplot5_naive_b1b3_ter)
tmp <- summary(fit_length_intron_kdeg)
tmp$coefficients

anova(fit_length_intron_kdeg, fit_length_intron)
anova(fit_length_intron_kdeg, fit_length_kdeg) 
anova(fit_length_intron_kdeg, fit_intron_kdeg) # => fit_intron_kdeg

# model 3 param
fit_length_splicing_kdeg <- lmer(transport ~ Length + PI_naive_prod_avg_late + kdeg + (1 | Name), data = dat_ggplot5_naive_b1b3_ter)
tmp <- summary(fit_length_splicing_kdeg)
tmp$coefficients

anova(fit_length_splicing_kdeg, fit_length_splicing)
anova(fit_length_splicing_kdeg, fit_length_kdeg) 
anova(fit_length_splicing_kdeg, fit_splicing_kdeg) # => fit_length_splicing_kdeg

# model 3 param
fit_intron_splicing_kdeg <- lmer(transport ~ Nb_introns + PI_naive_prod_avg_late + kdeg + (1 | Name), data = dat_ggplot5_naive_b1b3_ter)
tmp <- summary(fit_intron_splicing_kdeg)
tmp$coefficients

anova(fit_intron_splicing_kdeg, fit_intron_splicing)
anova(fit_intron_splicing_kdeg, fit_intron_kdeg) 
anova(fit_intron_splicing_kdeg, fit_splicing_kdeg) # => fit_intron_splicing_kdeg

# model 4 param
fit_length_intron_splicing_kdeg <- lmer(transport ~ Length + Nb_introns + PI_naive_prod_avg_late + kdeg + (1 | Name), data = dat_ggplot5_naive_b1b3_ter)
tmp <- summary(fit_length_intron_splicing_kdeg)
tmp$coefficients

anova(fit_length_intron_splicing_kdeg, fit_length_intron_splicing)
anova(fit_length_intron_splicing_kdeg, fit_length_intron_kdeg)
anova(fit_length_intron_splicing_kdeg, fit_length_splicing_kdeg)
anova(fit_length_intron_splicing_kdeg, fit_intron_splicing_kdeg) # => fit_intron_splicing_kdeg

# kdeg only
plot(predict(fit_kdeg), fit_kdeg@frame$transport)
cor(predict(fit_kdeg), fit_kdeg@frame$transport)

# best model without kdeg
tmp <- summary(fit_intron_splicing)
tmp$coefficients

plot(predict(fit_intron_splicing), fit_intron_splicing@frame$transport)
cor(predict(fit_intron_splicing), fit_intron_splicing@frame$transport)

# best model with kdeg
tmp<-summary(fit_intron_splicing_kdeg)
tmp$coefficients

plot(predict(fit_intron_splicing_kdeg), fit_intron_splicing_kdeg@frame$transport)
cor(predict(fit_intron_splicing_kdeg), fit_intron_splicing_kdeg@frame$transport)

#### Try model with all

fit_length <- lm(transport ~ Length, data = dat_ggplot5_naive_all_ter)
tmp <- summary(fit_length)
tmp$coefficients

fit_intron <- lm(transport ~ Nb_introns, data =dat_ggplot5_naive_all_ter)
tmp <- summary(fit_intron)
tmp$coefficients

fit_splicing <- lm(transport ~ PI_naive_prod_avg_late, data = dat_ggplot5_naive_all_ter)
tmp <- summary(fit_splicing)
tmp$coefficients

fit_kdeg <- lm(transport ~ kdeg, data = dat_ggplot5_naive_all_ter)
tmp <- summary(fit_kdeg)
tmp$coefficients

# model 2 param
fit_length_intron <- lm(transport ~ Length + Nb_introns, data = dat_ggplot5_naive_all_ter)
tmp <- summary(fit_length_intron)
tmp$coefficients
...
# model 2 param
fit_length_splicing <- lm(transport ~ Length + PI_naive_prod_avg_late, data = dat_ggplot5_naive_all_ter)
tmp <- summary(fit_length_splicing)
tmp$coefficients
...
# model 2 param
fit_intron_splicing <- lm(transport ~ Nb_introns + PI_naive_prod_avg_late, data = dat_ggplot5_naive_all_ter)
tmp <- summary(fit_intron_splicing)
tmp$coefficients
...
# model 2 param
fit_length_kdeg <- lm(transport ~ Length + kdeg, data = dat_ggplot5_naive_all_ter)
tmp <- summary(fit_length_kdeg)
tmp$coefficients
...
# model 2 param
fit_intron_kdeg <- lm(transport ~ Nb_introns + kdeg, data = dat_ggplot5_naive_all_ter)
tmp <- summary(fit_intron_kdeg)
tmp$coefficients
...
# model 2 param
fit_splicing_kdeg <- lm(transport ~ PI_naive_prod_avg_late + kdeg, data = dat_ggplot5_naive_all_ter)
tmp <- summary(fit_splicing_kdeg)
tmp$coefficients
...
# model 3 param
fit_length_intron_splicing <- lm(transport ~ Length + Nb_introns + PI_naive_prod_avg_late, data = dat_ggplot5_naive_all_ter)
tmp <- summary(fit_length_intron_splicing)
tmp$coefficients
...
# model 3 param
fit_length_intron_kdeg <- lm(transport ~ Length + Nb_introns + kdeg, data = dat_ggplot5_naive_all_ter)
tmp <- summary(fit_length_intron_kdeg)
tmp$coefficients
...
# model 3 param
fit_length_splicing_kdeg <- lm(transport ~ Length + PI_naive_prod_avg_late + kdeg, data = dat_ggplot5_naive_all_ter)
tmp <- summary(fit_length_splicing_kdeg)
tmp$coefficients
...
# model 3 param
fit_intron_splicing_kdeg <- lm(transport ~ Nb_introns + PI_naive_prod_avg_late + kdeg, data = dat_ggplot5_naive_all_ter)
tmp <- summary(fit_intron_splicing_kdeg)
tmp$coefficients
...
# model 4 param
fit_length_intron_splicing_kdeg <- lm(transport ~ Length + Nb_introns + PI_naive_prod_avg_late + kdeg, data = dat_ggplot5_naive_all_ter)
tmp <- summary(fit_length_intron_splicing_kdeg)
tmp$coefficients
...
# kdeg only
plot(predict(fit_kdeg), fit_kdeg$model$transport)
cor(predict(fit_kdeg), fit_kdeg$model$transport)^2

# best model without kdeg
tmp <- summary(fit_intron_splicing)
tmp$coefficients

plot(predict(fit_intron_splicing), fit_intron_splicing$model$transport)
cor(predict(fit_intron_splicing), fit_intron_splicing$model$transport)^2

# best model with kdeg
tmp<-summary(fit_intron_splicing_kdeg)
tmp$coefficients

plot(predict(fit_splicing_kdeg), fit_splicing_kdeg$model$transport)
cor(predict(fit_splicing_kdeg), fit_splicing_kdeg$model$transport)^2


#### Figure 5D - HL vs Transport ####---------------------
head(dat_ggplot)

dat_ggplot6 <- cbind(dat_ggplot[dat_ggplot$Parameter_name=="kdeg" & dat_ggplot$Batch %in% c('b1','b3'),-c(3,12,13)],
                     dat_ggplot[dat_ggplot$Parameter_name=="k1'k2'/k2" & dat_ggplot$Batch %in% c('b1','b3'),-c(1,3,10,11,12,13,14)])
...
p <- p + scale_shape(labels = c("Rep1","Rep3")) + coord_cartesian(ylim=c(-2.5,0.5))
...
pheatmap::pheatmap(log10(responsiveness), cluster_rows = F, cluster_cols = F, color=colorRampPalette(RColorBrewer::brewer.pal(n = 7, name = "RdYlBu"))(100))


hm.palette <- colorRampPalette(RColorBrewer::brewer.pal(11, 'Spectral'))
...
for (condition in names(all_data)){
  ...
  for (g in names(all_data[[condition]])){
    ...
    row.names(SSE_table) <- c(paste("-NLL",rep(c("b1", "b2","b3"), each=3), rep(c("ca", "np","cyto"), 3), sep=" "), "Total", "Total with regul")
    ...
    row.names(par_table) <- c("spar", "sigmab", "sigmat", "ca0_b1", "k1'", "k2", "k2'", "kdeg", "k1'k2'", "k1'/k2", "k1'k2'/k2")
    ...
    row.names(par_table) <-  c("spar", "sigma[b]", "sigma[t]", "ca[paste(0,',', b[1])]", "log[10](k[1]*minute)", "log[10](k[2])", "log[10](k[2]*minute)", "log[10](k[deg])", "log[10](paste(k[1]*minute,k[2]*minute))", "log[10](k[1]*minute/k[2])", "transport == log[10](paste(k[1]*minute, k[2]*minute)/k[2])")
    ...
    dat_g$batch <- factor(dat_g$batch, levels = c("b1","b2","b3"), labels = c("Rep1", "Rep2", "Rep3"))
...
for (condition in c('naive', 'lpa')){
  pdf(file = paste0('Supplementary Documents/Profile_dMod_merged_', condition,'_with_optim_points_v2.pdf'), width=19)
  ...
  for (g in names(all_param_CI[[condition]][[1]])){
    ...
    dat_ggplot$whichPar[dat_ggplot$whichPar %in% c("k2", "kdeg") & dat_ggplot$batch == "b2"] <- "k2 or kdeg"
    ...
    dat_CI$whichPar[dat_CI$whichPar %in% c("min(k2,kdeg)", "max(k2,kdeg)") & dat_CI$batch == "b2"] <- "k2 or kdeg"
    dat_CI$whichPar <- factor(dat_CI$whichPar, 
                              levels = c("k1'", "k2", "k2'", "kdeg", "k1'k2'", "k2 or kdeg", "k1'/k2", "k1'k2'/k2", "k2'/k2"),
                              labels = c("log[10](k[1]*minute)", "log[10](k[2])", "log[10](k[2]*minute)", "log[10](k[deg])", "log[10](paste(k[1]*minute, k[2]*minute))", "log[10](paste(k[2], ' or ', k[deg]))", "log[10](paste(k[1]*minute/k[2]))", "log[10](paste(k[1]*minute,k[2]*minute)/k[2])", "log[10](k[2]*minute/k[2])"))
    
    dat_ggplot_points$whichPar <- as.character(dat_ggplot_points$whichPar)
    dat_ggplot_points$whichPar[dat_ggplot_points$whichPar %in% c("min(k2,kdeg)", "max(k2,kdeg)") & dat_ggplot_points$batch == "b2"] <- "k2 or kdeg"
    dat_ggplot_points$whichPar <- factor(dat_ggplot_points$whichPar, 
                                         levels = c("k1'", "k2", "k2'","kdeg", "k1'k2'", "k2 or kdeg", "k1'/k2", "k1'k2'/k2", "k2'/k2"),
                                         labels = c("log[10](k[1]*minute)", "log[10](k[2])", "log[10](k[2]*minute)", "log[10](k[deg])", "log[10](paste(k[1]*minute, k[2]*minute))", "log[10](paste(k[2], ' or ', k[deg]))", "log[10](paste(k[1]*minute/k[2]))", "log[10](paste(k[1]*minute,k[2]*minute)/k[2])", "log[10](k[2]*minute/k[2])"))
    
    dat_blank$whichPar <- as.character(dat_blank$whichPar)
    dat_blank$whichPar[dat_blank$whichPar %in% c("min(k2,kdeg)", "max(k2,kdeg)") & dat_blank$batch == "b2"] <- "k2 or kdeg"
...
#### Compare old CI vs new CI ####
dir_old <- '../Modeling/All_genes_last_5kbs/Model1/Hoffman2/Optimisation/Modeling_v13/Profile_dMod_integrate_v2/naive/'
dir_new <- gsub('v2', 'v3', dir_old)

dir_old <- '../Modeling/All_genes_last_5kbs/Model1/Hoffman2/Optimisation/Modeling_v13/Profile_dMod/naive/'
dir_new <- gsub('_dMod', '_dMod_optimize_v3', dir_old)

dir_old <- '../Modeling/All_genes_last_5kbs/Model1/Hoffman2/Optimisation/Modeling_v13/Profile_dMod_integrate_v2/lpa/'
dir_new <- gsub('v2', 'v3', dir_old)

dir_old <- '../Modeling/All_genes_last_5kbs/Model1/Hoffman2/Optimisation/Modeling_v13/Profile_dMod/lpa/'
dir_new <- gsub('_dMod', '_dMod_optimize_v3', dir_old)


files <- list.files(dir_old)
files <- files[-grep('_b2',files)]
files <- files[-grep('merged',files)]
files <- grep('Finished', files, value = T)
for (f in  files){
  load(paste0(dir_old, f))
  dat_old <- dat_g
  if(f %in% list.files(dir_new)){
    load(paste0(dir_new,f))
    dat_new <- dat_g
    
    for (par in names(dat_old)){
      if(dat_old[[par]]$CI != dat_new[[par]]$CI){
        print(f)
        print(dat_old[[par]]$CI)
        print(dat_new[[par]]$CI)
      }
    }
  }else{
    print(f)
...
files_old <- list.files(dir_old)
files_old <- files_old[-grep('_b2',files_old)]
files_old <- files_old[-grep('merged',files_old)]
files_old <- grep('Finished', files_old, value = T)

files_new <- list.files(dir_new)
files_new <- grep('Finished', files_new, value = T)

files_old[!(files_old %in% files_new)]
files_new[!(files_new %in% files_old)]
...
# diag(cor_mat) <- NA
# 
# pheatmap(dat_cor2,cluster_rows = F, cluster_cols = F, gaps_row=c(13,26), gaps_col=c(13,26))
# stop()
# 
# #### Figure S1A - Correlation between replicates ####--------------------------------------------------------------
# dat <- cbind(caRNA_Naive_exons_cpm[,c(NA, 12, 24,1,13,25,2,14,26,3,15,27,4,16,28,5,17,29,6,7,18,30,8,19,31,9,20,32,21,33,10,22,34,11,23,35)], 
#              npRNA_Naive_exons_cpm[,c(NA,12,NA,5,13,NA,1,14,NA,6,15,NA,2,16,NA,7,17,NA,3,8,18,NA,4,19,NA,9,20,NA,21,NA,10,22,NA,11,23,NA)], 
#              cytoRNA_Naive_exons_cpm[,c(5,13,25,6,14,26,1,15,27,7,16,28,2,17,29,8,18,30,3,9,19,31,4,20,32,10,21,33,22,34,11,23,35,12,24,36)]) # switched b2 and b3 in the data
# colnames(dat)
# dat2 <- dat[row.names(dat) %in% list_genes_cleaned$Geneid[list_genes_cleaned$Naive.Selected],]
# 
# dat_cor <- cor(dat)
# dat_cor2 <- cor(dat2)
# 
# dat_cor2[is.na(colnames(dat_cor2)),] <- NA
# dat_cor2[,is.na(colnames(dat_cor2))] <- NA
# 
# dat_cor2[1:(12*3),-c(1:(12*3))] <- NA
# dat_cor2[37:72,c(1:36,73:108)] <- NA
...
# stop()
# 
# #### Figure S4B - Heatmap of LPA replicates ####--------------------------------------------------------------
# dat_ca <- caRNA_LPA_exons_rpkm[match(list_genes_cleaned$Geneid[list_genes_cleaned$Naive.Selected],rownames(caRNA_LPA_exons_rpkm)),]
# dat_np <- npRNA_LPA_exons_rpkm[match(list_genes_cleaned$Geneid[list_genes_cleaned$Naive.Selected],rownames(npRNA_LPA_exons_rpkm)),]
# dat_np <- dat_np[,c(5,6,1,7,2,8,3,9,4,10,11,12:24)] # reorder
# dat_cyto <- cytoRNA_LPA_exons_rpkm[match(list_genes_cleaned$Geneid[list_genes_cleaned$Naive.Selected],rownames(cytoRNA_LPA_exons_rpkm)),]
...
# dat_2[!(rownames(dat_2) %in% list_genes_cleaned$Geneid[list_genes_cleaned$LPA.Selected]),40:78] <- NA
# 
# # pheatmap(as.matrix(dat_2[match(h$labels, row.names(dat_2)),]), color = colors_spectral, cluster_rows = h, cluster_cols = F, show_rownames = F,gaps_col = c(13,26,39,52,65), breaks = seq(-5,10,length.out = 101), border_color = NA)
# # pheatmap(as.matrix(dat_2[match(h$labels, row.names(dat_2)),][reorder,]), color = colors_spectral, cluster_rows = F, cluster_cols = F, show_rownames = F,gaps_col = c(13,26,39,52,65), breaks = seq(-5,10,length.out = 101))
# 
# ## plot FC
# head(dat)
# datFC <- data.frame(dat[,1:13] - dat[,1],dat[,14:26]- dat[,14],dat[,27:39] - dat[,27], dat[,40:52]-dat[,40], dat[,53:65]-dat[,53], dat[,66:78]-dat[,66])
...
# vp_legend <- viewport(name="legend", x = 2 * h_gap + vp_lpa$x + vp_lpa$width, y=0, width = lgd_Heatmap@grob$vp$width, just=c(0,0))
# 
# # svg(file = 'Figure6A_Heatmap_FC_Naive_LPA.svg', width = 7, height = 7)
# grid.newpage()
# pushViewport(vp_naive)
# grid.draw(gb_hnaive)
# popViewport()
...
#       if(length(indexes_ca) == 1){
#         dat_g <- rbind(dat_g, data.frame(log2RPKM = caRNA_Naive_exons_rpkm_5kb[g1,indexes_ca], Time = as.numeric(t), Fraction = "Chromatin", Batch = b, Gene=gene_name[1]))
#         # dat_g <- rbind(dat_g, data.frame(log2RPKM = caRNA_Naive_exons_rpkm_5kb[g2,indexes_ca], Time = as.numeric(t), Fraction = "Chromatin", Batch = b, Gene=gene_name[2]))
#       }
#       indexes_np <- intersect(grep(t, colnames(npRNA_Naive_exons_rpkm_5kb)), grep(b, colnames(npRNA_Naive_exons_rpkm_5kb)))
#       if(length(indexes_np) == 1){
#         dat_g <- rbind(dat_g,data.frame(log2RPKM = npRNA_Naive_exons_rpkm_5kb[g1,indexes_np], Time = as.numeric(t), Fraction = "Nucleoplasm", Batch = b, Gene=gene_name[1]))
#         # dat_g <- rbind(dat_g,data.frame(log2RPKM = npRNA_Naive_exons_rpkm_5kb[g2,indexes_np], Time = as.numeric(t), Fraction = "Nucleoplasm", Batch = b, Gene=gene_name[2]))
#       }
#       indexes_cyt <- intersect(grep(t, colnames(cytoRNA_Naive_exons_rpkm_5kb)), grep(b, colnames(cytoRNA_Naive_exons_rpkm_5kb)))
#       if(length(indexes_cyt) == 1){
#         dat_g <- rbind(dat_g,data.frame(log2RPKM = cytoRNA_Naive_exons_rpkm_5kb[g1,indexes_cyt], Time = as.numeric(t), Fraction = "Cytoplasm", Batch = b, Gene=gene_name[1]))
#         # dat_g <- rbind(dat_g,data.frame(log2RPKM = cytoRNA_Naive_exons_rpkm_5kb[g2,indexes_cyt], Time = as.numeric(t), Fraction = "Cytoplasm", Batch = b, Gene=gene_name[2]))
#       }
#     }
#   }
# dat_g$Fraction <- factor(dat_g$Fraction, levels = c("Chromatin", "Nucleoplasm", "Cytoplasm"))
...
# 
# ca_smooth <- predict(smooth.spline(x=dat_g$Time[ dat_g$Fraction == "Chromatin" & dat_g$Batch == "rep2"], y=dat_g$log2RPKM[ dat_g$Fraction == "Chromatin" & dat_g$Batch == "rep2"], spar = 0.45),seq(0,121,by=1/60))$y
# p <- ggplot(dat_g[ dat_g$Fraction == "Chromatin" & dat_g$Batch == "rep2",], aes(x = Time, y=log2RPKM)) + geom_point(alpha=0.3)
# p <- p + geom_line(data = data.frame(x=seq(0,121,1/60), y=ca_smooth), aes(x=x, y=y),col="black", size=1) 
# p <- p + theme_bw() + theme(aspect.ratio=1, panel.background = element_rect(fill = "transparent")) + labs(y='log2(RPKM)', title=paste0(gene_name, collapse = " and "))
# p <- p + scale_x_continuous(breaks=c(0,30,60,90,120))
# print(p)
...
#       dat_g_exp_ca <- dat_g_exp_np <- dat_g_exp_cyto <- data.frame(RPKM=c(), Time = c(), batch = c(), Cpt = c(), Type=c(), Condition=c())
#       for (bs in 2:ncol(data_all$caRNA)){
#         dat_g_exp_ca <- rbind(dat_g_exp_ca, data.frame(RPKM=data_all$caRNA[,bs], Time = data_all$caRNA[,1], replicate = c("rep1", "rep2", "rep3")[bs-1], Cpt = "Chromatin", Type="Experiment", Condition = condition))
#         dat_g_exp_np <- rbind(dat_g_exp_np, data.frame(RPKM=data_all$npRNA[,bs], Time = data_all$npRNA[,1], replicate = c("rep1", "rep2", "rep3")[bs-1], Cpt = "Nucleoplasm", Type="Experiment", Condition = condition))
#         dat_g_exp_cyto <- rbind(dat_g_exp_cyto, data.frame(RPKM=data_all$cytoRNA[,bs], Time = data_all$cytoRNA[,1], replicate = c("rep1", "rep2", "rep3")[bs-1], Cpt = "Cytoplasm", Type="Experiment", Condition = condition))
#       }
#       
#       for (b in c('all', 'rep1', 'rep2', 'rep3')){   
#         if(b == "all"){
...
#         }
#         if(!no_np){
#           par_old <- c(par_old, "k1'/k2" = unname(par_old["k1'"] - par_old["k2"]), "k1'k2'/k2" = unname(par_old["k1'"] + par_old["k2'"]- par_old["k2"]))
#           par_old_all <- cbind(par_old_all, "k1'/k2" = unname(par_old_all[,"k1'"] - par_old_all[,"k2"]), "k1'k2'/k2" = unname(par_old_all[,"k1'"] + par_old_all[,"k2'"]- par_old_all[,"k2"]))
#         }
#         
#         if(!(b %in% names(all_best_param[[condition]]))){
#           all_best_param[[condition]][[b]] <- list()
...
#         data_extrapolation <- data_extrapolation[!to_remove,]
#         
#         dat_g_exp_pattern_ca <- paste0('chromatin.',sapply(nchar(as.character(dat_g_exp_ca$Time)),function(x){paste0(rep(0,3-x),collapse='')}), as.character(dat_g_exp_ca$Time),'.', dat_g_exp_ca$replicate)
#         lib_indices_ca <- match(dat_g_exp_pattern_ca, row.names(lib_mat))
#         libsize_ca <- as.integer(floor((lib_mat[,"lib.size"]*lib_mat[,"norm.factors"])[lib_indices_ca]))
#         names(libsize_ca) <- dat_g_exp_pattern_ca
#         
#         dat_g_exp_pattern_np <- paste0('Nuc.',sapply(nchar(as.character(dat_g_exp_np$Time)),function(x){paste0(rep(0,3-x),collapse='')}), as.character(dat_g_exp_np$Time),'.', dat_g_exp_np$replicate)
#         lib_indices_np <- match(dat_g_exp_pattern_np, row.names(lib_mat))
#         libsize_np <- as.integer(floor((lib_mat[,"lib.size"]*lib_mat[,"norm.factors"])[lib_indices_np]))
#         names(libsize_np) <- dat_g_exp_pattern_np
#         
#         dat_g_exp_pattern_cyto <- paste0('cytoplasmic.',sapply(nchar(as.character(dat_g_exp_cyto$Time)),function(x){paste0(rep(0,3-x),collapse='')}), as.character(dat_g_exp_cyto$Time),'.', dat_g_exp_cyto$replicate)
#         lib_indices_cyto <- match(dat_g_exp_pattern_cyto, row.names(lib_mat))
#         libsize_cyto <- as.integer(floor((lib_mat[,"lib.size"]*lib_mat[,"norm.factors"])[lib_indices_cyto]))
#         names(libsize_cyto) <- dat_g_exp_pattern_cyto
#         
...
#         SSEs_g_det <- TSE_reps_cp(par = par, fn_fit=fn_all_ode_cp, data = data_all, time_data=time_data, convert_param=T, log_in=T, u=input_fun_cp, replicate=b, results = "detailed", library_size = lib_mat, length = gene_length)
#         
#         if(SSEs_g_total != min(bfgs[[b]]$value)){stop("Doesn't mach best values")}
#         
#         par_theo <- par
#         par_theo[2:3] <- c(0,1*10^-6)
#         
...
#           
#           if(ncol(data$caRNA) > 2){
#             weight_ca <- proba_ca[dat_g_exp_ca$replicate == paste0('rep',bs-1)]
#             weight_np <- proba_np[dat_g_exp_np$replicate == paste0('rep',bs-1)]
#             weight_cyto <- proba_cyto[dat_g_exp_cyto$replicate == paste0('rep',bs-1)]
#           }else{
#             weight_ca <- proba_ca[dat_g_exp_ca$replicate == batch]
#             weight_np <- proba_np[dat_g_exp_np$replicate == batch]
#             weight_cyto <- proba_cyto[dat_g_exp_cyto$replicate == batch]
#           }
#           weight_ca <- weight_ca/max(weight_ca, na.rm=T)
#           weight_np <- weight_np/max(weight_np, na.rm=T)
#           weight_cyto <- weight_cyto/max(weight_cyto, na.rm=T)
...
#           }
#           
#           libsize <- data.frame(caRNA = libsize_ca[dat_g_exp_ca$replicate == ifelse(length(batch)>1, colnames(data$caRNA)[bs],b)],
#                                 npRNA = if(!all(is.na(libsize_np[dat_g_exp_np$replicate == ifelse(length(batch)>1, colnames(data$npRNA)[bs],b)]))){libsize_np[dat_g_exp_np$replicate == ifelse(length(batch)>1, colnames(data$npRNA)[bs],b)]}else{NA},
#                                 cytoRNA = libsize_cyto[dat_g_exp_cyto$replicate == ifelse(length(batch)>1, colnames(data$cytoRNA)[bs],b)])
#           
#           if(no_np){
#             libsize <- libsize[, -which(colnames(libsize) == "npRNA")]
#           }                      
...
#           m <- 1/(exp(sigma^2) - 1)
#           m[!is.na(sigma) & exp(sigma^2) == 1] <- (1/sigma^2)[!is.na(sigma) & exp(sigma^2) == 1] # approx if sigma close to zero
#           mu <- exp(log(2^fit[match(dat_g_exp_ca$Time[dat_g_exp_ca$replicate == ifelse(length(batch)>1,colnames(data$caRNA)[bs],b)],fit[,1]),match(colnames(libsize), colnames(fit))]) + log(libsize/10^6*gene_length/10^3) + 1/2*sigma[,match(colnames(libsize), colnames(sigma))]^2)
# 
#           ub_ca <- log2(sapply(1:nrow(libsize), function(i){qnbinom(p = 0.025, size = m[i,"caRNA"], mu = mu[i, "caRNA"], lower.tail=F)})/(libsize[,"caRNA"]/10^6*gene_length/10^3))
#           lb_ca <- log2(sapply(1:nrow(libsize), function(i){qnbinom(p = 0.025, size = m[i,"caRNA"], mu = mu[i, "caRNA"], lower.tail=T)})/(libsize[,"caRNA"]/10^6*gene_length/10^3))
#           
...
#           
#           # if(ncol(data$caRNA) == 2){
#           #   SSEs_g_det <- list(SSEs_g_det[[1]][1], SSEs_g_det[[1]][2], SSEs_g_det[[1]][3])
#           # }
#           
#           tmp <- c(max(abs(c(autocor_error_ca_times_mean_error_over_range,autocor_error_np_times_mean_error_over_range, autocor_error_cyto_times_mean_error_over_range)), na.rm=T), 
#                    max((1+abs(c(autocor_error_ca,autocor_error_np, autocor_error_cyto)))*c(mean_error_over_range_ca,mean_error_over_range_np,mean_error_over_range_cyto), na.rm=T),
...
# }
# 
# # save(all_annot, all_best_param, all_param, all_data, all_fit, all_fit_CI, all_library, file = 'Figures/all_results.Rdata')
# stop()
# 
# # Get Profile and CI data
# # => save data
...
#     # tmp_max <- tmp[tmp$type == "max",]
#     
#     # tmp_min[tmp_min$whichPar == "k2","k2"] <- tmp_min[tmp_min$whichPar == "k2","kdeg"]
#     # tmp_min[tmp_min$whichPar == "k2","kdeg"] <- tmp_min[tmp_min$whichPar == "k2","kval"]
#     # tmp_min$whichPar <- "kdeg"
#     # 
#     # tmp_max[tmp_max$whichPar == "kdeg","kdeg"] <- tmp_max[tmp_max$whichPar == "kdeg","k2"]
#     # tmp_max[tmp_max$whichPar == "kdeg","k2"] <- tmp_max[tmp_max$whichPar == "kdeg","kval"]
#     # tmp_max$whichPar <- "k2"
#     
#     tmp_kdeg <- tmp_k2 <- tmp
#     
...
#     dat_g$k2$Profile <- tmp_k2[,!colnames(tmp_k2) %in% c("type", "kval")]
#     dat_g$kdeg$Profile <- tmp_kdeg[,!colnames(tmp_kdeg) %in% c("type", "kval")]
#     # dat_g$kdeg$CI$upper <- dat_g$k2$CI$upper <- mean(c(tmp_min$kdeg[nrow(tmp_min$kdeg)],tmp_max$k2[1])) # not really upper because I didn't force ordering in the profile, stop when better to invert min max
#     # dat_g$k2$CI$lower <- mean(c(tmp_min$kdeg[nrow(tmp_min$kdeg)],tmp_max$k2[1])) # not really lower because I didn't force ordering in the profile, stop when better to invert min max
#   }else{
#     plot(dat_g$kdeg$Profile$kdeg,dat_g$kdeg$Profile$value , col="red", pch=20, cex=0.5, xlim=range(c(dat_g$kdeg$Profile$kdeg, dat_g$k2$Profile$k2)))
#     points(dat_g$k2$Profile$k2, dat_g$k2$Profile$value , col="blue", pch=20, cex=0.5, )
#     
...
#           # keep results to avoid reloading
#           
#           file_approx <- paste0('../Modeling/Optimisation/Profile_dMod_integrate/', condition, '/profile_dMod_', batch, '_', gene_name,'_Finished.Rdata')
#           if(file.exists(file_approx)){
#             load(file_approx)
#             
#             if(batch == 'rep3'){
#               print("approx")
#               dat_g <- merge_function_for_b2(dat_g, th = 2*(min_val + qchisq(0.95,1,lower.tail = T)/2))
#             }
#             
#             for(i in length(dat_g)){
#               dat_g[[i]]$computing_time <- NULL
#             }
#             
#             dat_profile_tmp_all[["approx"]] <- dat_g 
#             
#             for (l in dat_g){
#               if (batch == 'rep2' && l$CI$name %in% c("min(k2,kdeg)","max(k2,kdeg)") && max(dat_g$"min(k2,kdeg)"$Profile$kdeg) >= min(dat_g$"max(k2,kdeg)"$Profile$k2)){ 
#                 check_lower <- all(all_param[[condition]][[batch]][[gene_name]][,"value"][all_param[[condition]][[batch]][[gene_name]][, "max(k2,kdeg)"] < ifelse(is.na(l$CI$lower),-Inf,l$CI$lower) | all_param[[condition]][[batch]][[gene_name]][, "min(k2,kdeg)"] < ifelse(is.na(l$CI$lower),-Inf,l$CI$lower)] > min_val + qchisq(0.95,1,lower.tail = T)/2)
#                 check_upper <- all(all_param[[condition]][[batch]][[gene_name]][,"value"][all_param[[condition]][[batch]][[gene_name]][, "max(k2,kdeg)"] > ifelse(is.na(l$CI$upper),Inf,l$CI$upper) | all_param[[condition]][[batch]][[gene_name]][, "min(k2,kdeg)"] > ifelse(is.na(l$CI$upper),Inf,l$CI$upper)] > min_val + qchisq(0.95,1,lower.tail = T)/2)
#               }else{
#                 check_lower <- all(all_param[[condition]][[batch]][[gene_name]][,"value"][all_param[[condition]][[batch]][[gene_name]][,as.character(l$CI$name)] < ifelse(is.na(l$CI$lower),-Inf,l$CI$lower)] > min_val + qchisq(0.95,1,lower.tail = T)/2)
...
#                 consistency_check_approx <- c(consistency_check_approx,T)
#                 dat_CI_tmp <- rbind(dat_CI_tmp, data.frame(l$CI, method_lower='approximate', method_upper="approximate"))
#                 dat_profile_tmp[[as.character(l$CI$name)]] <- cbind(dat_g[[as.character(l$CI$name)]]$Profile, method = "approximate")
#               }else{
#                 consistency_check_approx <- c(consistency_check_approx, F)
#               }
#               names(consistency_check_approx)[length(consistency_check_approx)] <- as.character(l$CI$name)
#               names(consistency_check_lower_approx)[length(consistency_check_lower_approx)] <- as.character(l$CI$name)
#               names(consistency_check_upper_approx)[length(consistency_check_upper_approx)] <- as.character(l$CI$name)
#               
#             }
#             consistency_check <- consistency_check_approx
#             consistency_check_lower <- consistency_check_lower_approx
...
#           }  
#           
#           if( length(consistency_check) == 0 || !all(consistency_check)){
#             file_exact <- paste0('../Modeling/Profile_dMod/', condition, '/profile_dMod_', batch, '_', gene_name,'_Finished.Rdata')
#             if(file.exists(file_exact)){
#               load(file_exact)
#               
#               if(batch == 'rep2'){
...
#               
#               for (l in dat_g){
#                 if (batch == 'b2' && l$CI$name %in% c("min(k2,kdeg)","max(k2,kdeg)") && max(dat_g$"min(k2,kdeg)"$Profile$kdeg) >= min(dat_g$"max(k2,kdeg)"$Profile$k2)){ 
#                   check_lower <- all(all_param[[condition]][[batch]][[gene_name]][,"value"][all_param[[condition]][[batch]][[gene_name]][, "max(k2,kdeg)"] < ifelse(is.na(l$CI$lower),-Inf,l$CI$lower) | all_param[[condition]][[batch]][[gene_name]][, "min(k2,kdeg)"] < ifelse(is.na(l$CI$lower),-Inf,l$CI$lower)] > min_val + qchisq(0.95,1,lower.tail = T)/2)
#                   check_upper <- all(all_param[[condition]][[batch]][[gene_name]][,"value"][all_param[[condition]][[batch]][[gene_name]][, "max(k2,kdeg)"] > ifelse(is.na(l$CI$upper),Inf,l$CI$upper) | all_param[[condition]][[batch]][[gene_name]][, "min(k2,kdeg)"] > ifelse(is.na(l$CI$upper),Inf,l$CI$upper)] > min_val + qchisq(0.95,1,lower.tail = T)/2)
#                 # if (batch == 'b2' & l$CI$name %in% c("kdeg","k2")){ 
#                 #   if(l$CI$name == "k2"){ # max
#                 #     check_lower <- all(all_param[[condition]][[batch]][[gene_name]][,"value"][all_param[[condition]][[batch]][[gene_name]][, "max(k2,kdeg)"] < ifelse(is.na(l$CI$lower),-Inf,l$CI$lower)] > min_val + qchisq(0.95,1,lower.tail = T)/2)
#                 #     check_upper <- all(all_param[[condition]][[batch]][[gene_name]][,"value"][all_param[[condition]][[batch]][[gene_name]][, "max(k2,kdeg)"] > ifelse(is.na(l$CI$upper), Inf,l$CI$upper)] > min_val + qchisq(0.95,1,lower.tail = T)/2)
#                 #   }else{
#                 #     check_lower <- all(all_param[[condition]][[batch]][[gene_name]][,"value"][all_param[[condition]][[batch]][[gene_name]][, "min(k2,kdeg)"] < ifelse(is.na(l$CI$lower),-Inf,l$CI$lower)] > min_val + qchisq(0.95,1,lower.tail = T)/2)
#                 #     check_upper <- all(all_param[[condition]][[batch]][[gene_name]][,"value"][all_param[[condition]][[batch]][[gene_name]][, "min(k2,kdeg)"] > ifelse(is.na(l$CI$upper), Inf,l$CI$upper)] > min_val + qchisq(0.95,1,lower.tail = T)/2)
#                 #   }
#                 }else{
#                   check_lower <- all(all_param[[condition]][[batch]][[gene_name]][,"value"][all_param[[condition]][[batch]][[gene_name]][,as.character(l$CI$name)] < ifelse(is.na(l$CI$lower),-Inf,l$CI$lower)] > min_val + qchisq(0.95,1,lower.tail = T)/2)
#                   check_upper <- all(all_param[[condition]][[batch]][[gene_name]][,"value"][all_param[[condition]][[batch]][[gene_name]][,as.character(l$CI$name)] > ifelse(is.na(l$CI$upper), Inf,l$CI$upper)] > min_val + qchisq(0.95,1,lower.tail = T)/2)
...
#                 consistency_check_lower_exact <- c(consistency_check_lower_exact,check_lower)
#                 consistency_check_upper_exact <- c(consistency_check_upper_exact,check_upper)
#                 names(consistency_check_lower_exact)[length(consistency_check_lower_exact)] <- as.character(l$CI$name)
#                 names(consistency_check_upper_exact)[length(consistency_check_upper_exact)] <- as.character(l$CI$name)
#                 
#                 if(length(consistency_check_approx)>0){
#                   if(!consistency_check_approx[as.character(l$CI$name)] & check_lower & check_upper){
#                     consistency_check_exact <- c(consistency_check_exact,T)
#                     dat_CI_tmp <- rbind(dat_CI_tmp, data.frame(l$CI, method_lower='exact', method_upper='exact'))
#                     dat_profile_tmp[[as.character(l$CI$name)]] <- cbind(dat_g[[as.character(l$CI$name)]]$Profile, method ='exact')
#                   }else{
#                     consistency_check_exact <- c(consistency_check_exact, F)
#                   }
#                 }else{
...
#                   }
#                 } 
#                 names(consistency_check_exact)[length(consistency_check_exact)] <- as.character(l$CI$name)
#                 
#               }
#               
#               if(length(consistency_check_approx)>0){
...
#           if(length(consistency_check) == 0 || !all(consistency_check)){
#             for (par in colnames(all_param[[condition]][[batch]][[gene_name]])){
#               if(par %in% c("value", "spar", "sigmat", "sigmab", "ca0_b1")){
#                 next
#               }
#               if(length(consistency_check) == 0 || !consistency_check[par]){
#                 # if approx method lower bound worked use that, else if exact method lower bound worked use that, else use optim
#                 if (length(consistency_check_approx) !=0 && consistency_check_lower_approx[par]){
#                   l <- dat_profile_tmp_all[["approx"]][[par]]
#                   dat_CI_tmp <- rbind(dat_CI_tmp, data.frame(l$CI, method_lower='approximate', method_upper=NA, lower_min = NA, upper_max = NA))
#                   dat_profile_tmp[[par]] <- cbind(l$Profile[l$Profile$constraint <= 0, ], method ='approximate')
#                 }else if(length(consistency_check_exact) !=0 && consistency_check_lower_exact[par]) {
#                   l <- dat_profile_tmp_all[["exact"]][[par]]
#                   dat_CI_tmp <- rbind(dat_CI_tmp, data.frame(l$CI, method_lower='exact', method_upper=NA, lower_min = NA, upper_max = NA))
#                   dat_profile_tmp[[par]] <- cbind(l$Profile[l$Profile$constraint <= 0, ], method = "exact")
#                 }else{
#                   lower <- min(all_param[[condition]][[batch]][[gene_name]][which(all_param[[condition]][[batch]][[gene_name]][,"value"] <= min_val + 1/2 * qchisq(0.95,1)), par])
#                   lower_min <- -Inf
#                   dat_CI_tmp <- rbind(dat_CI_tmp, data.frame(name=par, value=NA, lower = lower, upper = NA, method_lower = 'optim', method_upper = NA, lower_min = ifelse(length(lower_min)>0,lower_min,-Inf), upper_max = NA))
...
#                 
#                 # if approx method upper bound worked use that, else if exact method lower bound worked use that, else use optim
#                 if (length(consistency_check_approx) != 0 && consistency_check_upper_approx[par]){
#                   l <- dat_profile_tmp_all[["approx"]][[par]]
#                   if(par %in% dat_CI_tmp$name){
#                     dat_CI_tmp$upper[dat_CI_tmp$name == par] <- l$CI$upper 
#                     dat_CI_tmp$method_upper[dat_CI_tmp$name == par] <- 'approximate'
#                     dat_profile_tmp[[as.character(l$CI$name)]] <- rbind(dat_profile_tmp[[as.character(l$CI$name)]],cbind(l$Profile[l$Profile$constraint >= 0, ], method="approximate"))
#                   }else{
#                     dat_profile_tmp[[as.character(l$CI$name)]] <- cbind(l$Profile[l$Profile$constraint <= 0, ], method = "approximate")
#                     dat_CI_tmp <- rbind(dat_CI_tmp, data.frame(l$CI, method_lower=NA, method_upper='approximate', lower_min = NA, upper_max = NA))
#                   }
#                 }else if( length(consistency_check_exact) != 0 && consistency_check_upper_exact[par]) {
#                   l <- dat_profile_tmp_all[["exact"]][[par]]
#                   if(par %in% dat_CI_tmp$name){
#                     dat_CI_tmp$upper[dat_CI_tmp$name == par] <- l$CI$upper 
#                     dat_CI_tmp$method_upper[dat_CI_tmp$name == par] <- 'exact'
#                     dat_profile_tmp[[as.character(l$CI$name)]] <- rbind(dat_profile_tmp[[as.character(l$CI$name)]],cbind(l$Profile[l$Profile$constraint >= 0, ], method="exact"))
#                   }else{
#                     dat_profile_tmp[[as.character(l$CI$name)]] <- cbind(l$Profile[l$Profile$constraint <= 0, ], method = "exact")
#                     dat_CI_tmp <- rbind(dat_CI_tmp, data.frame(l$CI, method_lower=NA, method_upper='exact', lower_min = NA, upper_max = NA))
#                   }
#                 }else{
#                   upper <- max(all_param[[condition]][[batch]][[gene_name]][which(all_param[[condition]][[batch]][[gene_name]][,"value"] <= min_val + 1/2 * qchisq(0.95,1)), par])
...
#             
#             # check both bound for b2 k2, kdeg if disjunct
#             if(batch == 'b2' && max(dat_profile_tmp[["min(k2,kdeg)"]]$k2) < min(dat_profile_tmp[["max(k2,kdeg)"]]$kdeg)){
#               if( dat_CI_tmp["max(k2,kdeg)", "method_lower"] == "optim" & dat_CI_tmp["min(k2,kdeg)", "method_upper"] != "optim" ){ # if disjunct
#                 dat_CI_tmp["max(k2,kdeg)", "lower_min"] <- dat_CI_tmp["min(k2,kdeg)", "upper"]
#               }else if(dat_CI_tmp["max(k2,kdeg)", "method_lower"] == "optim" & dat_CI_tmp["min(k2,kdeg)", "method_lower"] != "optim" ){
#                 dat_CI_tmp["max(k2,kdeg)", "lower_min"] <- dat_CI_tmp["min(k2,kdeg)", "lower"]
...
# }
# 
# # save(all_annot, all_best_param, all_param, all_data, all_fit, all_fit_CI, all_library, all_param_CI, file = 'Figures/all_results.Rdata')
# 
# stop()
# 
# 
...
#       
#       data_all <- all_data[[condition]][[g]]  
#       data <- list(caRNA = data_all$caRNA[,c("t", "b3")],
#                    npRNA = data_all$npRNA[,c("t", "b3")],
#                    cytoRNA = data_all$cytoRNA[,c("t", "b3")])
#       fit <- all_fit[[condition]][[batch]][[g]]
#         
#       annot_tmp <- rbind(annot_tmp, unlist(all_annot[[condition]][[batch]][[g]][["annot"]]))
#       row.names(annot_tmp)[nrow(annot_tmp)] <- g
...
# stop()
# 
# #### Figure S5A - Heatmap of data vs Fit LPA ####---------------------------------------------------------------
# dat_2 <- as.matrix(dat_sim[['lpa']])
# dat_2[dat_2 > 10] <- 10
# dat_2[dat_2 < -5] <- -5
# 
...
# p_naive <- p_naive + labs(title=paste0(gene_name,  " - Naive"), color="Replicates") + scale_x_continuous(breaks = c(0,30,60,90,120))
# p_naive <- p_naive + theme(plot.title = element_text(face = "bold"), plot.subtitle = element_text(size = rel(0.8)))
# p_naive <- p_naive + facet_wrap(~ Cpt)  + scale_color_manual(values = c("b1" = "#FFd280FF", "b2"="#FFA540FF","b3"="#BF7C00FF"), labels=c("Rep1","Rep2","Rep3"))
# p_naive <- p_naive + labs(y=expression(log[2](FPKM))) + theme(legend.position = "bottom")
# 
# g1 <- ggplotGrob(p_naive)
# col <- c("Chromatin" = col_ca, "Nucleoplasm" = col_np, "Cytoplasm" = col_cyto)
...
# 
# 
# #### Figure 3A - Distribution of parameters for Naive ####---------------------------------------------------------------
# palette <- colorRampPalette(colors = c("green", "yellow", "orange", "red"))(100)
# make_palette <- function(value, palette){
#   res <- palette[1+(value-min(value, na.rm = T))/(max(value, na.rm = T) -min(value, na.rm = T))*99]
# }
# 
# dat_ggplot$facet_label <- factor(dat_ggplot$Parameter_name, levels=unique(dat_ggplot$Parameter_name), labels=c("spar","sigma[t]","sigma[b]","ca0[b1]", "log[10](k[1]*minute)", "log[10](k[2])", "log[10](k[2]*minute)", "log[10](k[deg])", "log[10](k[1]*minute/k[2])", "log[10](paste(k[1]*minute,k[2]*minute)/k[2])", "log[10](paste(k[1]*minute,k[2]*minute))", "log[10](max(k[2],k[deg]))", "log[10](min(k[2],k[deg]))"))
# dat_ggplot$color_label <- factor(dat_ggplot$Parameter_name, levels=unique(dat_ggplot$Parameter_name), labels=c("spar","sigma[t]","sigma[b]","ca0[b1]", "k[1]*minute", "k[2]", "k[2]*minute", "k[deg]", "k[1]*minute/k[2]", "paste(k[1]*minute,k[2]*minute)/k[2]", "paste(k[1]*minute,k[2]*minute)", "max(k[2],k[deg])", "min(k[2],k[deg])"))
# 
# # svg(filename = 'Figures/Fig3A_v13.svg')
# p <- ggplot(dat_ggplot[dat_ggplot$Parameter_name %in% c("k1'", "k2", "k2'", "kdeg") & dat_ggplot$Condition == "naive" & dat_ggplot$Batch %in% c("b1", "b3"),]) 
# p <- p + geom_density(mapping = aes(x=Parameter_value, fill=Parameter_name, color=Parameter_name, linetype=Batch), alpha=0.1)
# p <- p + scale_color_manual(name="Parameter", values=c("red", "yellow","green", "blue"))
# p <- p + scale_fill_manual(name="Parameter", values=c("red", "yellow", "green", "blue"))
# p <- p + scale_linetype_manual(values=c("b1"=1, "b3" = 2), labels = c("Rep1", "Rep3"))
...
# stop()
# 
# #### Figure 3B - Reproducibility of parameters fits for Naive ####---------------------------------------------------------------
# dat_ggplot_b1 <- dat_ggplot[dat_ggplot$Batch == 'b1',]
# dat_ggplot_b3 <- dat_ggplot[dat_ggplot$Batch == 'b3',]
# 
# dat_ggplot2 <- dat_ggplot_b1
...
# }
# 
# make_replicate_plot <- function(data,condition,parameter, xylim=NULL, color_facet=NULL, font_size=12, legend = "none"){
#   
#   p <- ggplot(data[data$Condition == condition & data$Parameter_name == parameter,],aes(x=Parameter_value_b1,y=Parameter_value_b3))
#   p <- p + geom_point(mapping=aes(col=color_max), size=2) + facet_wrap(.~facet_label, scale='free', drop = T, labeller = "label_parsed")
#   p <- p + geom_segment(mapping = aes(x=CIL_0.95_Min_b1,y=Parameter_value_b3,xend=CIL_0.95_Max_b1,yend = Parameter_value_b3, col=color_b1), alpha=0.3, linetype=2)
#   p <- p + geom_segment(mapping = aes(x=CIU_0.95_Min_b1,y=Parameter_value_b3,xend=CIU_0.95_Max_b1,yend = Parameter_value_b3, col=color_b1), alpha=0.3, linetype=2)
#   p <- p + geom_segment(mapping = aes(x=CIL_0.95_Max_b1,y=Parameter_value_b3,xend=CIU_0.95_Min_b1,yend = Parameter_value_b3, col=color_b1), alpha=0.3)
...
#   p <- p + theme_bw(base_size = font_size) + theme(aspect.ratio=1) + scale_color_gradientn(colours = c("green3", "gold2", "darkorange2", "darkred"), limits=range(c(data$color_b1,data$color_b3)))+labs(col='Fit Quality')
#   p <- p + geom_abline(slope=1, intercept=0, linetype=1, col='black', alpha=0.5) + geom_abline(slope=1, intercept=log10(2), linetype=2, col='red', alpha=0.5) + geom_abline(slope=1, intercept=-log10(2), linetype=2, col='red', alpha=0.5)
#   p <- p + labs(x="Replicate 1", y="Replicate 3")
#   p <- p + theme(strip.text = element_text(margin=margin(0.2,0,0.2,0,unit = "line")))
#     
#   if(!is.null(xylim)){
#     p <- p + coord_cartesian(xlim=xylim, ylim=xylim)
...
# }
# 
# # Croped version for figure (reduce total size 2x afterward)
# # svg(filename = 'Figures/Fig3B_k1prime.svg', width=5, height=5)
# p <- make_replicate_plot(data = dat_ggplot2, condition='naive', parameter =  "k1'", color_facet = "red", legend = "none", font_size=24) # xylim=c(-1.5,1.1)
# p 
# # dev.off()
# 
# # svg(filename = 'Figures/Fig3B_k2.svg', width=5, height=5)
# p <- make_replicate_plot(data = dat_ggplot2, condition='naive', parameter =  "k2", color_facet = "yellow", legend = "none", font_size=24) # xylim=c(-1.5,1.1)
# p
# # dev.off()
# 
# # svg(filename = 'Figures/Fig3B_k2prime.svg', width=5, height=5)
# p <- make_replicate_plot(data = dat_ggplot2, condition='naive', parameter =  "k2'", color_facet = "green", xylim=c(-3,0), legend = "none", font_size=24)
# p
# # dev.off()
# 
# # svg(filename = 'Figures/Fig3B_kdeg.svg', width=5, height=5)
# p <- make_replicate_plot(data = dat_ggplot2, condition='naive', parameter =  "kdeg", color_facet = "blue", xylim=c(-4,0), legend = "none", font_size=24)
# p
# # dev.off()
# 
# # svg(filename = 'Figures/Fig3B_legend.svg', width=5, height=5)
# p <- make_replicate_plot(data = dat_ggplot2, condition='naive', parameter =  "kdeg", color_facet = "blue", xylim=c(-4,0), legend = "only", font_size=24)
# grid.draw(p)
# # dev.off()
# 
# # svg(filename = 'Figures/Fig3B_k1prime_over_k2.svg', width=5, height=5)
# p <- make_replicate_plot(data = dat_ggplot2, condition='naive', parameter =  "k1'/k2", xylim=c(-0.2,0.8), legend = "none", font_size=24)
# p
# # dev.off()
# stop()
# 
# #### Figure 4B - Reproducibility of Transport ####------------------------------
# # svg(filename = 'Figures/Fig4B_tranport.svg', width=5, height=5)
# p <- make_replicate_plot(data = dat_ggplot2, condition='naive', parameter =  "k1'k2'/k2", xylim=c(-2.7,0.3), legend = "none", font_size=24)
# p
# # dev.off()
# stop()
# 
# #### Figure 4A - Example gene location on transport distribution ####---------------------------------------------------------------
# # svg(filename = 'Figures/Fig4A_transport_dist_v13.svg', width=5, height=2.5)
# p <- ggplot(dat_ggplot2[dat_ggplot2$Parameter_name %in% c("k1'k2'/k2") & dat_ggplot2$Condition == "naive" ,]) 
# p <- p + geom_density(mapping = aes(x=Parameter_value_b1, fill="k1'k2'/k2", color="k1'k2'/k2", linetype="Rep1"), alpha=0.1)
# p <- p + geom_density(mapping = aes(x=Parameter_value_b3, fill="k1'k2'/k2", color="k1'k2'/k2", linetype="Rep3"), alpha=0.1)
# p <- p + scale_x_continuous(limits = c(-3,1)) 
# p <- p + scale_color_manual(values=c("k1'k2'/k2"="black", "Egr1"="blue", "Malt1"="darkred"), guide=FALSE) 
# p <- p + scale_fill_manual(values=c("k1'k2'/k2"="black"), guide=FALSE)
# p <- p + scale_linetype_manual(values=c("Rep1"=1, "Rep3" = 2))
# p <- p + geom_segment(data = dat_ggplot2[dat_ggplot2$Gene_name == "Egr1" & dat_ggplot2$Parameter_name %in% c("k1'k2'/k2") & dat_ggplot2$Condition == "naive",], aes(x = Parameter_value_b1, xend=Parameter_value_b1, y=0, yend=0.1, linetype="Rep1", col = "Egr1")) 
# p <- p + geom_segment(data = dat_ggplot2[dat_ggplot2$Gene_name == "Egr1" & dat_ggplot2$Parameter_name %in% c("k1'k2'/k2") & dat_ggplot2$Condition == "naive",], aes(x = Parameter_value_b3, xend=Parameter_value_b3, y=0, yend=0.1, linetype="Rep3", col="Egr1")) 
# p <- p + geom_segment(data = dat_ggplot2[dat_ggplot2$Gene_name == "Malt1" & dat_ggplot2$Parameter_name %in% c("k1'k2'/k2")& dat_ggplot2$Condition == "naive",], aes(x = Parameter_value_b1, xend=Parameter_value_b1, y=0, yend=0.1, linetype="Rep1", col="Malt1"))
# p <- p + geom_segment(data = dat_ggplot2[dat_ggplot2$Gene_name == "Malt1" & dat_ggplot2$Parameter_name %in% c("k1'k2'/k2")& dat_ggplot2$Condition == "naive",], aes(x = Parameter_value_b3, xend=Parameter_value_b3, y=0, yend=0.1, linetype="Rep3", col="Malt1")) 
# p <- p + annotate("text", x = mean(unlist(dat_ggplot2[dat_ggplot2$Gene_name == "Egr1" & dat_ggplot2$Parameter_name %in% c("k1'k2'/k2") & dat_ggplot2$Condition == "naive",c("Parameter_value_b1","Parameter_value_b3")]), na.rm=T), y = 0.15, label = "Egr1", color="blue", fontface="bold", size=rel(6))
# p <- p + annotate("text", x = mean(unlist(dat_ggplot2[dat_ggplot2$Gene_name == "Malt1" & dat_ggplot2$Parameter_name %in% c("k1'k2'/k2") & dat_ggplot2$Condition == "naive",c("Parameter_value_b1","Parameter_value_b3")]), na.rm=T), y = 0.15, label = "Malt1", color="darkred", fontface="bold", size=rel(6))
# p + theme_bw() + theme(aspect.ratio=0.5) + labs(x="Relative Transport (log10)", fill="Parameter", color="Gene", y="Density", linetype="Replicate")
# # dev.off()
# 
# stop()
# 
# 
# #### Figure S5B - Distribution of parameter fits in Tolerized ####----------------
# # svg(filename = 'Figures/FigS5B_v13.svg', width=7, height=4)
# p <- ggplot(dat_ggplot[dat_ggplot$Parameter_name %in% c("k1'", "k2", "k2'", "kdeg") & dat_ggplot$Condition == "lpa" & dat_ggplot$Batch %in% c("b1", "b3"),]) 
# p <- p + geom_density(mapping = aes(x=Parameter_value, fill=Parameter_name, color=Parameter_name, linetype=Batch), alpha=0.1)
# p <- p + scale_color_manual(values=c("red", "yellow","green", "blue")) 
# p <- p + scale_fill_manual(values=c("red", "yellow", "green", "blue"))
# p <- p + scale_linetype_manual(values=c("b1"=1, "b3" = 2), labels = c("Rep1", "Rep3"))
...
# stop()
# 
# #### Figure S5C - Reproducibility of parameter fits in Tolerized ####----------------
# # svg(filename = 'Figures/FigS5C_k1prime.svg', width=5, height=5)
# p <- make_replicate_plot(data = dat_ggplot2, condition='lpa', parameter =  "k1'", color_facet = "red", legend = "none", font_size=24, xylim = c(-3,8.2)) # xylim=c(-1.5,1.1)
# p 
# # dev.off()
# 
# # svg(filename = 'Figures/FigS5C_k2.svg', width=5, height=5)
# p <- make_replicate_plot(data = dat_ggplot2, condition='lpa', parameter =  "k2", color_facet = "yellow", legend = "none", font_size=24,xylim = c(-3.2,8.2)) # xylim=c(-1.5,1.1)
# p
# # dev.off()
# 
# # svg(filename = 'Figures/FigS5C_k2prime.svg', width=5, height=5)
# p <- make_replicate_plot(data = dat_ggplot2, condition='lpa', parameter =  "k2'", color_facet = "green", xylim=c(-3,0), legend = "none", font_size=24)
# p
# # dev.off()
# 
# # svg(filename = 'Figures/FigS5C_kdeg.svg', width=5, height=5)
# p <- make_replicate_plot(data = dat_ggplot2, condition='lpa', parameter =  "kdeg", color_facet = "blue", xylim=c(-3.5,-0.3), legend = "none", font_size=24)
# p
# # dev.off()
# 
# # svg(filename = 'Figures/FigS5C_legend.svg', width=5, height=5)
# p <- make_replicate_plot(data = dat_ggplot2, condition='lpa', parameter =  "kdeg", color_facet = "blue", xylim=c(-4,0), legend = "only", font_size=24)
# grid.draw(p)
# # dev.off()
# 
# # svg(filename = 'Figures/FigS5C_k1prime_over_k2.svg', width=5, height=5)
# p <- make_replicate_plot(data = dat_ggplot2, condition='lpa', parameter =  "k1'/k2", legend = "none", font_size=24)
# p
# # dev.off()
# 
# # svg(filename = 'Figures/FigS5C_transport.svg', width=5, height=5)
# p <- make_replicate_plot(data = dat_ggplot2, condition='lpa', parameter =  "k1'k2'/k2", xylim=c(-2.7,0.5), legend = "none", font_size=24)
# p
# # dev.off()
# 
# stop()
...
#     sheet_name <- paste0(condition_name, "_", batch)
#     openxlsx::addWorksheet(wb, sheet_name)
#     
#     tmp <- all_best_param[[condition]][[batch]][,colnames(all_best_param[[condition]][[batch]]) %in% c("k1'","k2","k2'","kdeg","k1'/k2", "k1'k2'/k2", "k1'k2'", "max(k2,kdeg)", "min(k2,kdeg)")]
#     tmp2 <- tmp
#     for(col in colnames(tmp)){
#       i <- which(colnames(tmp2) == col)
#       if(i != ncol(tmp2)){
...
#         tmp2[row.names(tmp2) == gene, paste0(par,'_CI95')] <- paste0("(", ifelse(!is.na(tmp_CI[,'lower_min']), paste0("< ", signif(tmp_CI[,"lower"],3)), signif(tmp_CI[,"lower"],3)),", ",
#                                                                       ifelse(!is.na(tmp_CI[,'upper_max']), paste0("> ", signif(tmp_CI[,"upper"],3)), signif(tmp_CI[,"upper"],3)),")")
#         # tmp2[row.names(tmp2) == gene, paste0(par,'_CI95L_method')] <- tmp_CI[,'method_lower']
#         # tmp2[row.names(tmp2) == gene, paste0(par,'_CI95U_method')] <- tmp_CI[,'method_upper'] 
#       }
#     }
#     
#     for (par in c("k1'","k2","k2'","kdeg","k1'/k2", "k1'k2'/k2", "k1'k2'", "max(k2,kdeg)", "min(k2,kdeg)")){
#       if( par %in% colnames(tmp2)){
#         column_i <- which(colnames(tmp2) == par)
#         colnames(tmp2)[column_i + 0:1] <- gsub(par,paste0("log10(",par,")"), colnames(tmp2)[column_i + 0:1], fixed=T)
#       }
...
#   }
# }
# openxlsx::saveWorkbook(wb, "Supplementary Documents/Parameters_fit.xlsx", overwrite = TRUE)
# 
# 
# 
# #### Figure 6B - Naive vs LPA parameter fits ####---------------------------------
...
# dat_ggplot3$color_max <- apply(dat_ggplot3[,paste("color", c("Naive", "LPA"), sep="_")],1, max)
# 
# make_condition_plot <- function(data, batch, parameter, xylim=NULL, color_facet=NULL, font_size=12, legend = "none"){
#   
#   p <- ggplot(data[data$Batch %in% batch & data$Parameter_name == parameter,], aes(x=Parameter_value_Naive,y=Parameter_value_LPA))
#   p <- p + geom_segment(mapping = aes(x=CIL_0.95_Min_Naive,y=Parameter_value_LPA,xend=CIL_0.95_Max_Naive,yend = Parameter_value_LPA, col=color_Naive), alpha=0.3, linetype=2)
#   p <- p + geom_segment(mapping = aes(x=CIU_0.95_Min_Naive,y=Parameter_value_LPA,xend=CIU_0.95_Max_Naive,yend = Parameter_value_LPA, col=color_Naive), alpha=0.3, linetype=2)
#   p <- p + geom_segment(mapping = aes(x=CIL_0.95_Max_Naive,y=Parameter_value_LPA,xend=CIU_0.95_Min_Naive,yend = Parameter_value_LPA, col=color_Naive), alpha=0.3)
#   p <- p + geom_segment(mapping = aes(y=CIL_0.95_Min_LPA,x=Parameter_value_Naive,yend=CIL_0.95_Max_LPA,xend = Parameter_value_Naive, col=color_LPA), alpha=0.3, linetype = 2)
...
# }
# 
# # svg(filename = 'Figures/Fig6B_k1prime.svg', width=5, height=5)
# p <- make_condition_plot(data = dat_ggplot3, batch = c('b1','b3'), parameter =  "k1'", color_facet = "red", xylim= c(-2,10.5),legend = "none", font_size=24)
# p
# # dev.off()
# 
# # svg(filename = 'Figures/Fig6B_k2.svg', width=5, height=5)
# p <- make_condition_plot(data = dat_ggplot3, batch = c('b1','b3'), parameter =  "k2", color_facet = "yellow", xylim= c(-3,9),legend = "none", font_size=24)
# p
# # dev.off()
# 
# # svg(filename = 'Figures/Fig6B_k2prime.svg', width=5, height=5)
# p <- make_condition_plot(data = dat_ggplot3, batch = c('b1','b3'), parameter =  "k2'", color_facet = "green", xylim= c(-3,1),legend = "none", font_size=24)
# p
# # dev.off()
# 
# # svg(filename = 'Figures/Fig6B_kdeg.svg', width=5, height=5)
# p <- make_condition_plot(data = dat_ggplot3, batch = c('b1','b3'), parameter =  "kdeg", color_facet = "blue", xylim= c(-4,0.5),legend = "none", font_size=24)
# p
# # dev.off()
# 
# # svg(filename = 'Figures/Fig6B_legend.svg', width=5, height=5)
# p <- make_condition_plot(data = dat_ggplot3, batch = c('b1','b3'), parameter =  "kdeg", color_facet = "blue", xylim= c(-4,0.5),legend = "only", font_size=24)
# grid.draw(p)
# # dev.off()
# 
# # svg(filename = 'Figures/Fig6B_k1prime_over_k2.svg', width=5, height=5)
# p <- make_condition_plot(data = dat_ggplot3, batch = c('b1','b3'), parameter =  "k1'/k2", xylim= c(-0.2,2),legend = "none", font_size=24)
# p
# # dev.off()
# 
# # svg(filename = 'Figures/Fig6B_k1primek2prime_over_k2.svg', width=5, height=5)
# p <- make_condition_plot(data = dat_ggplot3, batch = c('b1','b3'), parameter =  "k1'k2'/k2", xylim= c(-3,0.5),legend = "none", font_size=24)
# p
# # dev.off()
# 
# stop()
# 
# 
# #### Figure 3D - Comparaison of decay with ActD HL ####---------------------------
# dat_ggplot_HL <- dat_ggplot3[dat_ggplot3$Parameter_name == 'kdeg',]
# 
# dat_ggplot_HL$ActD_HL <- BMDM_dataset$halflife$Naive_Basal$HL[match(dat_ggplot_HL$Gene_name, BMDM_dataset$gene_infos$Symbol[match(row.names(BMDM_dataset$halflife$Naive_Basal),BMDM_dataset$gene_infos$Geneid)])]
# dat_ggplot_HL$ActD_CI95_L <- BMDM_dataset$halflife$Naive_Basal$CI_95_LB[match(dat_ggplot_HL$Gene_name, BMDM_dataset$gene_infos$Symbol[match(row.names(BMDM_dataset$halflife$Naive_Basal),BMDM_dataset$gene_infos$Geneid)])]
...
# 
# 
# dat_ggplot_HL_2 <- cbind(dat_ggplot_HL[dat_ggplot_HL$Batch == 'b1',], dat_ggplot_HL[dat_ggplot_HL$Batch == 'b3',23:30])
# colnames(dat_ggplot_HL_2)[31:38] <- paste(colnames(dat_ggplot_HL_2)[23:30], '_b3', sep='')
# 
# dat_ggplot_HL$Batch <- factor(dat_ggplot_HL$Batch, levels=c('all','b1','b2','b3'), labels=c('Together', 'Rep1', 'Rep2', 'Rep3'))
# 
# # svg(filename = 'Figures/Fig3D.svg', width=5, height=5)
# p <- ggplot(dat_ggplot_HL[dat_ggplot_HL$Batch %in% c('Rep1','Rep3'),], aes(x=HL_Naive,y=ActD_HL)) + geom_point(mapping = aes(shape=Batch))
# p <- p + geom_segment(data = dat_ggplot_HL_2, aes(x = HL_Naive, xend = HL_Naive_b3, y = ActD_HL, yend = ActD_HL), alpha=0.1, linetype=2)
# p <- p + scale_x_continuous(trans='log10', limits=c(5,10000)) + scale_y_continuous(trans='log10', limits = c(10,500))
# p <- p + geom_abline(slope = 1, col='green') 
# p <- p + geom_abline(slope = 1, intercept = log10(2), col='red', linetype=2)
# p <- p + geom_abline(slope = 1, intercept = -log10(2), col='red', linetype=2)
...
# quantile(dat_ggplot_HL$ActD_HL, c(0.05,0.25,0.5,0.75,0.95), na.rm = T)
# quantile(dat_ggplot_HL$HL_Naive[dat_ggplot_HL$Batch == 'Rep1'], c(0.05,0.25,0.5,0.75,0.95), na.rm = T)
# quantile(dat_ggplot_HL$HL_Naive[dat_ggplot_HL$Batch == 'Rep3'], c(0.05,0.25,0.5,0.75,0.95), na.rm = T)
# 
# cor(log(dat_ggplot_HL$ActD_HL[dat_ggplot_HL$Batch == 'Rep1']),log(dat_ggplot_HL$HL_Naive[dat_ggplot_HL$Batch == 'Rep1']),use = 'pairwise.complete.obs', method = 'spearman')
# cor(log(dat_ggplot_HL$ActD_HL[dat_ggplot_HL$Batch == 'Rep3']),log(dat_ggplot_HL$HL_Naive[dat_ggplot_HL$Batch == 'Rep3']),use = 'pairwise.complete.obs', method='spearman')
# 
# #### Figure S3D - Comparison ActD HL Basal vs 1h ####-------------------------------
# dat_ggplot_HL <- dat_ggplot3[dat_ggplot3$Parameter_name == 'kdeg',]
# 
# dat_ggplot_HL$ActD_HL_basal <- BMDM_dataset$halflife$Naive_Basal$HL[match(dat_ggplot_HL$Gene_name, BMDM_dataset$gene_infos$Symbol[match(row.names(BMDM_dataset$halflife$Naive_Basal),BMDM_dataset$gene_infos$Geneid)])]
# dat_ggplot_HL$ActD_CI95_L_basal <- BMDM_dataset$halflife$Naive_Basal$CI_95_LB[match(dat_ggplot_HL$Gene_name, BMDM_dataset$gene_infos$Symbol[match(row.names(BMDM_dataset$halflife$Naive_Basal),BMDM_dataset$gene_infos$Geneid)])]
# dat_ggplot_HL$ActD_CI95_U_basal <- BMDM_dataset$halflife$Naive_Basal$CI_95_UB[match(dat_ggplot_HL$Gene_name, BMDM_dataset$gene_infos$Symbol[match(row.names(BMDM_dataset$halflife$Naive_Basal),BMDM_dataset$gene_infos$Geneid)])]
# dat_ggplot_HL$ActD_HL_1h <- BMDM_dataset$halflife$Naive_LPA_1h$HL[match(dat_ggplot_HL$Gene_name, BMDM_dataset$gene_infos$Symbol[match(row.names(BMDM_dataset$halflife$Naive_Basal),BMDM_dataset$gene_infos$Geneid)])]
# dat_ggplot_HL$ActD_CI95_L_1h <- BMDM_dataset$halflife$Naive_LPA_1h$CI_95_LB[match(dat_ggplot_HL$Gene_name, BMDM_dataset$gene_infos$Symbol[match(row.names(BMDM_dataset$halflife$Naive_Basal),BMDM_dataset$gene_infos$Geneid)])]
# dat_ggplot_HL$ActD_CI95_U_1h <- BMDM_dataset$halflife$Naive_LPA_1h$CI_95_UB[match(dat_ggplot_HL$Gene_name, BMDM_dataset$gene_infos$Symbol[match(row.names(BMDM_dataset$halflife$Naive_Basal),BMDM_dataset$gene_infos$Geneid)])]
# dat_ggplot_HL$ActD_HL_3h <- BMDM_dataset$halflife$Naive_LPA_3h$HL[match(dat_ggplot_HL$Gene_name, BMDM_dataset$gene_infos$Symbol[match(row.names(BMDM_dataset$halflife$Naive_Basal),BMDM_dataset$gene_infos$Geneid)])]
# dat_ggplot_HL$ActD_CI95_L_3h <- BMDM_dataset$halflife$Naive_LPA_3h$CI_95_LB[match(dat_ggplot_HL$Gene_name, BMDM_dataset$gene_infos$Symbol[match(row.names(BMDM_dataset$halflife$Naive_Basal),BMDM_dataset$gene_infos$Geneid)])]
# dat_ggplot_HL$ActD_CI95_U_3h <- BMDM_dataset$halflife$Naive_LPA_3h$CI_95_UB[match(dat_ggplot_HL$Gene_name, BMDM_dataset$gene_infos$Symbol[match(row.names(BMDM_dataset$halflife$Naive_Basal),BMDM_dataset$gene_infos$Geneid)])]
# 
# dat_ggplot_HL$HL_Naive <- log(2)/10^dat_ggplot_HL$Parameter_value_Naive
# dat_ggplot_HL$HL_CIU_0.95_Min_Naive <- log(2)/10^dat_ggplot_HL$CIL_0.95_Max_Naive
# dat_ggplot_HL$HL_CIU_0.95_Max_Naive <- log(2)/10^dat_ggplot_HL$CIL_0.95_Min_Naive
...
# dat_ggplot_HL$HL_CIL_0.95_Max_Naive <- log(2)/10^dat_ggplot_HL$CIU_0.95_Min_Naive
# 
# dat_ggplot_HL_2 <- cbind(dat_ggplot_HL[dat_ggplot_HL$Batch == 'b1',], dat_ggplot_HL[dat_ggplot_HL$Batch == 'b3',c(13,23:36)])
# colnames(dat_ggplot_HL_2)[37:51] <- paste(colnames(dat_ggplot_HL_2)[c(13,23:36)], '_b3', sep='')
# dat_ggplot_HL_2$col_naive_max <- apply(dat_ggplot_HL_2[,c(13,37)],1,max)
# 
# p <- ggplot(data = dat_ggplot_HL_2, aes(x=ActD_HL_basal, y=ActD_HL_1h)) + geom_point()
...
# stop()
# 
# #### Figure 5A - Decay vs Transport ####---------------------
# head(dat_ggplot2)
# 
# dat_ggplot4 <- cbind(dat_ggplot[dat_ggplot$Parameter_name=="kdeg" & dat_ggplot$Batch %in% c('b1','b3'),-c(3,12,13)],
#                      dat_ggplot[dat_ggplot$Parameter_name=="k1'k2'/k2" & dat_ggplot$Batch %in% c('b1','b3'),-c(1,3,10,11,12,13,14)])
# names(dat_ggplot4)[2] <- "kdeg"
# names(dat_ggplot4)[12] <- "transport"
# 
# dat_ggplot4 <- cbind(dat_ggplot4,data.frame(log(2)/10^dat_ggplot4[,c(2:4,6:7)],dat_ggplot4[,c(5,8)]))
# 
# names(dat_ggplot4)[3:8] <- paste0(names(dat_ggplot4)[3:8],'_kdeg')
# names(dat_ggplot4)[13:18] <- paste0(names(dat_ggplot4)[13:18],"_transport")
# names(dat_ggplot4)[19] <- "HL"
# names(dat_ggplot4)[20:25] <- paste0(names(dat_ggplot4)[20:25],"_HL")
# names(dat_ggplot4)[20:25] <- gsub("CIL","@", names(dat_ggplot4)[20:25])
# names(dat_ggplot4)[20:25] <- gsub("CIU","CIL", names(dat_ggplot4)[20:25])
# names(dat_ggplot4)[20:25] <- gsub("@","CIU", names(dat_ggplot4)[20:25], fixed=T)
# names(dat_ggplot4)[20:25] <- gsub("Min","@", names(dat_ggplot4)[20:25])
# names(dat_ggplot4)[20:25] <- gsub("Max","Min", names(dat_ggplot4)[20:25])
# names(dat_ggplot4)[20:25] <- gsub("@","Max", names(dat_ggplot4)[20:25], fixed=T)
# 
# cor_test <- cor.test(dat_ggplot4$transport, dat_ggplot4$kdeg, use="pairwise.complete.obs")
# # svg(filename = 'Figures/Figure5A_transport_vs_HL.svg', width=4,height=4)
# p <- ggplot(dat_ggplot4, aes(y=transport, x=HL)) + geom_point(mapping=aes(shape=Batch))
# p <- p + theme_bw() + theme(aspect.ratio=1) + scale_x_continuous(trans='log10', limits = c(3,1000)) + scale_y_continuous(limits=c(-2.6,0.5))
# p <- p + geom_segment(aes(x=CIL_0.95_Max_HL,xend=CIU_0.95_Min_HL, y= transport, yend=transport), alpha=0.05)
# p <- p + geom_segment(aes(x=CIL_0.95_Min_HL,xend=CIL_0.95_Max_HL, y= transport, yend=transport), alpha=0.05, linetype=2)
# p <- p + geom_segment(aes(x=CIU_0.95_Min_HL,xend=CIU_0.95_Max_HL, y= transport, yend=transport), alpha=0.05, linetype=2)
...
# p <- p + geom_segment(aes(x=HL,xend=HL, y= CIL_0.95_Min_transport, yend=CIL_0.95_Max_transport), alpha=0.05, linetype=2)
# p <- p + geom_segment(aes(x=HL,xend=HL, y= CIU_0.95_Min_transport, yend=CIU_0.95_Max_transport), alpha=0.05, linetype=2)
# p <- p + scale_shape(labels = c("Rep1","Rep3"))
# p <- p + labs(y=parse(text = "`Relative Transport `(log[10])"), x='HL (min)', shape="Replicate")
# # p <- p + annotate(geom = "text", x=1000, y=0.5, hjust=1, vjust=1,label=bquote("Pearson's" ~ rho == .(signif(cor_test$estimate,2)) ~ ', p-value' == .(ifelse(cor_test$p.value < 2.2e-16, "< 2.2e-16", signif(cor_test$p.value,2)))))
# p <- p + annotate(geom = "text", x=1000, y=0.5, hjust=1, vjust=1,label=bquote("Pearson's" ~ rho == .(signif(cor_test$estimate,2))))
# p + theme(legend.position = "none")
...
# stop()
# 
# #### Figure 4C - Malt1 and Egr same graph ####-------------------------------------
# dat <- rbind(cbind(dat_g_Malt1, Name="Malt1"),cbind(dat_g_Egr1, Name="Egr1"))
# dat$Cpt <- factor(dat$Cpt, levels=c("Chromatin", "Nucleoplasm", "Cytoplasm"),ordered=T)
# 
# svg(filename = 'Figures/Figure4C_Malt1_Egr1.svg', width=7, height=3)
# p <- ggplot(data = dat[dat$Condition == "naive",], mapping = aes(x=Time,y=RPKM, col=Name, group=interaction(Name,batch))) + geom_point(data = dat[dat$Type == "Experiment" & dat$Condition == "naive",]) + facet_grid(.~Cpt)
# p <- p + geom_line(data=dat[dat$Type == "Fit" & dat$Condition == "naive",], alpha=0.5)
# p <- p + scale_color_manual(values = c(Malt1="darkred", Egr1="blue"))
# p <- p + theme_bw() + theme(aspect.ratio=1) + scale_x_continuous(breaks = c(0,30,60,90,120))
# p <- p + labs(x="Time (min)", y=parse(text="log[2](FPKM)"))
# g <- ggplotGrob(p)
...
#                    b3 = PI_naive_b3[PI_naive_b3$Intron_id %in% intron_annotated$Intrond_id[intron_annotated$Gene_id %in% g_keep], ])
# 
# # # restrict to U introns
# # PI_naive_b1_U <- PI_naive_b1[PI_naive_b1$Intron_id %in% intron_annotated$Intrond_id[intron_annotated$Attribute == "U"], ]
# # PI_naive_b3_U <- PI_naive_b3[PI_naive_b3$Intron_id %in% intron_annotated$Intrond_id[intron_annotated$Attribute == "U"], ]
# 
# avg_late_tp <- c(90,120)
# avg_early_tp <- c(0,10)
# 
# for (b in names(PI_naive_U)){
#   Time <- as.numeric(strsplit(PI_naive_U[[b]]$Time, split=',')[[1]])
#   PI_naive_U[[b]]$Total_Junction_Counts <- sapply(1:nrow(l), function(i){paste0(as.numeric(strsplit(PI_naive_U[[b]]$Inclusion_counts[i], split = ',')[[1]]) + as.numeric(strsplit(PI_naive_U[[b]]$Skipping_counts_EE[i], split = ',')[[1]]), collapse= ",")})
# 
#   # average late time points by batch
#   PI_naive_U[[b]]$PI_Junction_avg_late <- sapply(PI_naive_U[[b]]$PI_Junction, function(x){mean(as.numeric(strsplit(x, split = ',')[[1]][Time %in% avg_late_tp]), na.rm=T)})
#   PI_naive_U[[b]]$PI_Junction_avg_late_count <- sapply(PI_naive_U[[b]]$Total_Junction_Counts, function(x){mean(as.numeric(strsplit(x, split = ',')[[1]][Time %in% avg_late_tp]), na.rm=T)})
#   PI_naive_U[[b]]$PI_Junction_wavg_late <- sapply(1:nrow(l), function(i){sum(as.numeric(strsplit(PI_naive_U[[b]]$PI_Junction[i], split = ',')[[1]][Time %in% avg_late_tp])*(as.numeric(strsplit(PI_naive_U[[b]]$Total_Junction_Counts[i], split = ',')[[1]][Time %in% avg_late_tp])/sum(as.numeric(strsplit(PI_naive_U[[b]]$Total_Junction_Counts[i], split = ',')[[1]][Time %in% avg_late_tp]))), na.rm=T)})
# 
#   # average early time points by batch
#   PI_naive_U[[b]]$PI_Junction_avg_early <- sapply(PI_naive_U[[b]]$PI_Junction, function(x){mean(as.numeric(strsplit(x, split = ',')[[1]][Time %in% avg_early_tp]), na.rm=T)})
#   PI_naive_U[[b]]$PI_Junction_avg_early_count <- sapply(PI_naive_U[[b]]$Total_Junction_Counts, function(x){mean(as.numeric(strsplit(x, split = ',')[[1]][Time %in% avg_early_tp]), na.rm=T)})
#   PI_naive_U[[b]]$PI_Junction_wavg_early <- sapply(1:nrow(l), function(i){sum(as.numeric(strsplit(PI_naive_b1_U$PI_Junction[i], split = ',')[[1]][Time %in% avg_early_tp])*(as.numeric(strsplit(PI_naive_U[[b]]$Total_Junction_Counts[i], split = ',')[[1]][Time %in% avg_early_tp])/sum(as.numeric(strsplit(PI_naive_U[[b]]$Total_Junction_Counts[i], split = ',')[[1]][Time %in% avg_early_tp]))), na.rm=T)})
# }
# 
# # check plot
# 
...
#     b_g <- PI_naive_U[[b]][PI_naive_U[[b]]$Gene_id==g,]
# 
#     PI_naive_max_avg_early <- max(b_g$PI_Junction_avg_early[b_g$PI_Junction_avg_early_count >= count_threshold])
#     PI_naive_max_wavg_early <- max(b_g$PI_Junction_wavg_early[b_g$PI_Junction_avg_early_count >= count_threshold])
#     
#     PI_naive_max_avg_early_late <- max(rowMeans(cbind(b_g$PI_Junction_avg_early, b_g$PI_Junction_avg_late), na.rm=T)[b_g$PI_Junction_avg_early_count >= count_threshold  & b_g$PI_Junction_avg_late_count >= count_threshold ])
#     PI_naive_max_wavg_early_late <- max(rowMeans(cbind(b_g$PI_Junction_wavg_early, b_g$PI_Junction_wavg_late), na.rm=T)[b_g$PI_Junction_avg_early_count >= count_threshold & b_g$PI_Junction_avg_late_count >= count_threshold ])
#     
#     PI_naive_max_avg_late <- max(b_g$PI_Junction_avg_late[b_g$PI_Junction_avg_late_count >= count_threshold])
#     PI_naive_max_wavg_late <- max(b_g$PI_Junction_wavg_late[b_g$PI_Junction_avg_late_count >= count_threshold])
#     
...
#     
#     tmp <- data.frame(GeneId=g, 
#                       PI_naive_max_avg_early = PI_naive_max_avg_early,
#                       PI_naive_max_wavg_early = PI_naive_max_wavg_early,
#                       PI_naive_max_avg_late = PI_naive_max_avg_late,
#                       PI_naive_max_wavg_late = PI_naive_max_wavg_late,
#                       PI_naive_max_avg_early_late = PI_naive_max_avg_early_late,
#                       PI_naive_max_wavg_early_late = PI_naive_max_wavg_early_late,
#                       PI_naive_prod_avg_late = PI_naive_prod_avg_late,
#                       PI_naive_prod_wavg_late = PI_naive_prod_wavg_late
#     )
#     tmp[,unlist(lapply(tmp, is.infinite))] <- NA
...
#   if (! g %in% intron_transcript_mapping_filtered_naive$Gene_id ){ # not in the intron_annotation file i.e. no intron for the observed transcripts
#     tmp <- data.frame(GeneId=g, 
#                       PI_naive_max_avg_early = 0,
#                       PI_naive_max_wavg_early = 0,
#                       PI_naive_max_avg_late = 0,
#                       PI_naive_max_wavg_late = 0,
#                       PI_naive_max_avg_early_late = 0,
#                       PI_naive_max_wavg_early_late = 0,
#                       PI_naive_prod_avg_late = 1,
#                       PI_naive_prod_wavg_late = 1
#     )
#     for(b in names(all_intron_summary_measure)){
...
# 
# for (b in names(all_intron_summary_measure)){
#   all_intron_summary_measure[[b]]$Name <- gene_infos$external_gene_name[match(substr(intron_summary_measure_b1$GeneId,1,18), gene_infos$ensembl_gene_id)]
# 
# }
# 
# #### Figure 5B - Plot introns vs Transport ####--------------------------------------
# # Add parameters
# dat <- c()
# for (b in names(all_intron_summary_measure)){
#   for (g in all_intron_summary_measure[[b]]$Name){
#     dat <- rbind(dat, cbind(all_intron_summary_measure[[b]][all_intron_summary_measure[[b]]$Name == g,], Batch=b, 
#                             t(unlist(all_best_param$naive[[b]][g, c("kdeg", "k1'k2'/k2")])),
#                             data.frame(CIL_0.95_Min_kdeg = all_param_CI$naive[[b]][[g]]$CI["kdeg", "lower_min"],
#                                        CIL_0.95_Max_kdeg = all_param_CI$naive[[b]][[g]]$CI["kdeg", "lower"],
#                                        CIL_0.95_Method_kdeg = all_param_CI$naive[[b]][[g]]$CI["kdeg", "method_lower"],
#                                        CIU_0.95_Min_kdeg = all_param_CI$naive[[b]][[g]]$CI["kdeg", "upper"],
...
#   }
# }
# dat$HL <- log(2)/10^dat$kdeg
# 
# cor(log10(dat$PI_naive_prod_avg_late), dat$`k1'k2'/k2`, use="pairwise.complete.obs")
# cor(log10(dat$PI_naive_prod_avg_late)[ dat$Batch == 'b1'], dat$`k1'k2'/k2`[ dat$Batch == 'b1'], use="pairwise.complete.obs")
# cor(log10(dat$PI_naive_prod_avg_late)[dat$Batch == 'b3'], dat$`k1'k2'/k2`[ dat$Batch == 'b3'], use="pairwise.complete.obs")
# 
# cor(dat$kdeg, dat$`k1'k2'/k2`, use="pairwise.complete.obs")
# cor(dat$kdeg[dat$Batch == 'b1'], dat$`k1'k2'/k2`[dat$Batch == 'b1'], use="pairwise.complete.obs")
# cor(dat$kdeg[dat$Batch == 'b3'], dat$`k1'k2'/k2`[dat$Batch == 'b3'], use="pairwise.complete.obs")
# 
# cor(log10(dat$PI_naive_prod_avg_late), dat$kdeg, use="pairwise.complete.obs")
# cor(log10(dat$PI_naive_prod_avg_late)[dat$Batch == 'b1'], dat$kdeg[dat$Batch == 'b1'], use="pairwise.complete.obs")
# cor(log10(dat$PI_naive_prod_avg_late)[dat$Batch == 'b3'], dat$kdeg[dat$Batch == 'b3'], use="pairwise.complete.obs")
# 
# cor_test <- cor.test(log10(dat$PI_naive_prod_avg_late), dat$`k1'k2'/k2`, use="pairwise.complete.obs")
# 
# # svg(filename = 'Figures/Figure5B_transport vs PI.svg', width=4, height=4)
# p <- ggplot(dat, aes(x=PI_naive_prod_avg_late, y=`k1'k2'/k2`, color=HL)) + geom_point(mapping = aes(shape=Batch)) 
# p <- p + scale_color_continuous(limits=log(2)/10^c(-0.5,-3), oob=squish, trans="log10")
# p <- p + theme_bw() + theme(aspect.ratio=1)
# p <- p + scale_x_continuous(trans="log10") + scale_shape_discrete(labels=c("b1"="Rep1", "b3"="Rep3")) 
# p <- p + labs(x = parse(text="prod((1-PI[intron]), intron)"), y=bquote("Relative Transport "~(log[10])), color=parse(text="HL (min)"), shape="Replicate")
# p <- p + geom_segment(aes(x=PI_naive_prod_avg_late,xend=PI_naive_prod_avg_late, y= CIL_0.95_Max_transport, yend=CIU_0.95_Min_transport), alpha=0.05)
# p <- p + geom_segment(aes(x=PI_naive_prod_avg_late,xend=PI_naive_prod_avg_late, y= CIL_0.95_Min_transport, yend=CIL_0.95_Max_transport), alpha=0.05, linetype=2)
# p <- p + geom_segment(aes(x=PI_naive_prod_avg_late,xend=PI_naive_prod_avg_late, y= CIU_0.95_Min_transport, yend=CIU_0.95_Max_transport), alpha=0.05, linetype=2)
# p <- p + coord_cartesian(ylim=c(-2.5,0.25))
# # p <- p + annotate(geom = "text", x=0.001, y=0.5, hjust=0, vjust=1,label=bquote("Pearson's" ~ rho == .(signif(cor_test$estimate,2)) ~ ', p-value' == .(ifelse(cor_test$p.value < 2.2e-16, "< 2.2e-16", signif(cor_test$p.value,2)))))
# p <- p + annotate(geom = "text", x=0.001, y=0.25, hjust=0, vjust=1,label=bquote("Pearson's" ~ rho == .(signif(cor_test$estimate,2))))
# p + theme(legend.position = "none")
# # dev.off()
# 
# # svg(filename = 'Figures/Figure5B_legend.svg', width=5, height=5)
# grid.draw(gglegend(p))
# # dev.off()
# 
# fit <- rlm(`k1'k2'/k2` ~ kdeg, data = dat)
# summary(aov(fit))
# fit
# 
# fit1 <- rlm(`k1'k2'/k2` ~ log10(PI_naive_prod_avg_late), data = dat)
# summary(aov(fit1))
# fit1
# 
# fit2 <- rlm(`k1'k2'/k2` ~ log10(PI_naive_prod_avg_late) + kdeg, data = dat)
# summary(aov(fit2))
# fit2
# 
# anova(fit, fit1, fit2)
# 
# fit <- lm(`k1'k2'/k2` ~ kdeg, data = dat)
# summary(aov(fit1))
# fit
# 
# fit1 <- lm(`k1'k2'/k2` ~ log10(PI_naive_prod_avg_late), data = dat)
# summary(aov(fit1))
# fit1
# 
# fit2 <- lm(`k1'k2'/k2` ~ log10(PI_naive_prod_avg_late) + kdeg, data = dat)
# summary(aov(fit2))
# fit2
# 
# fit3 <- lm(`k1'k2'/k2` ~ log10(PI_naive_prod_avg_late) * kdeg, data = dat)
# summary(aov(fit3))
# fit3
# anova(fit, fit1, fit2, fit3)
# 
# 
# fit <- lm(log10(PI_naive_prod_avg_late) ~ kdeg, data = dat)
# summary(aov(fit))
# fit
# 
# fit1 <- lm(log10(PI_naive_prod_avg_late) ~ `k1'k2'/k2`, data = dat)
# summary(aov(fit1))
# fit1
# 
# fit2 <- lm(log10(PI_naive_prod_avg_late) ~ `k1'k2'/k2` + kdeg, data = dat)
# summary(aov(fit2))
# fit2
# 
# fit3 <- lm(log10(PI_naive_prod_avg_late) ~ `k1'k2'/k2` * kdeg, data = dat)
# summary(aov(fit3))
# fit3
# anova(fit, fit1, fit2, fit3)
# 
...
# #### To Clean up ####----------------------
# 
# # Plot PI example Nfkbie Arl5c?
# dat <- data.frame()
# 
# for ( g in unique(PI_naive_b1_lpa_U$Gene_id)){ 
#   b1_g <- PI_naive_b1_lpa_U[PI_naive_b1_lpa_U$Gene_id==g,]
#   b3_g <- PI_naive_b3_lpa_U[PI_naive_b3_lpa_U$Gene_id==g,]
#   
#   time_b1 <- as.numeric(unlist(strsplit(unique(b1_g$Time), split = ',')))
#   for(t in 1:length(time_b1)){
#     dat <- rbind(dat, data.frame(GeneId = g, Symbol = gene_list_mm89$external_gene_name[match(substr(g,1,18), gene_list_mm89$ensembl_gene_id)], Intron_id = b1_g$Intron_id , 
#                                  inclusion_count = as.numeric(unlist(lapply(strsplit(b1_g$Inclusion_counts, split = ','),function(x){x[t]}))), 
#                                  skipping_count = as.numeric(unlist(lapply(strsplit(b1_g$Skipping_counts, split = ','),function(x){x[t]}))), 
#                                  PI_Junction = as.numeric(unlist(lapply(strsplit(b1_g$PI_Junction, split = ','),function(x){x[t]}))), 
#                                  Intron_length = b1_g$End - b1_g$Start + 1,
#                                  Time=time_b1[t], Batch = 'b1' ))
#   }
#   
#   time_b3 <- as.numeric(unlist(strsplit(unique(b3_g$Time), split = ',')))
#   for(t in 1:length(time_b3)){
#     dat <- rbind(dat, data.frame(GeneId = g, Symbol = gene_list_mm89$external_gene_name[match(substr(g,1,18), gene_list_mm89$ensembl_gene_id)], Intron_id = b3_g$Intron_id , 
#                                  inclusion_count = as.numeric(unlist(lapply(strsplit(b3_g$Inclusion_counts, split = ','),function(x){x[t]}))), 
#                                  skipping_count = as.numeric(unlist(lapply(strsplit(b3_g$Skipping_counts, split = ','),function(x){x[t]}))), 
#                                  PI_Junction = as.numeric(unlist(lapply(strsplit(b3_g$PI_Junction, split = ','),function(x){x[t]}))), 
#                                  Intron_length = b3_g$End - b3_g$Start + 1,
#                                  Time=time_b3[t], Batch = 'b3' ))
#   }
# }
# 
# p <- ggplot(dat[dat$Symbol == "Nfkbie", ], aes(x=Time, y=PI_Junction, col=Batch)) + geom_line() + facet_wrap(~Intron_id) 
# p
# p <- ggplot(dat[dat$Symbol == "Arl5c", ], aes(x=Time, y=PI_Junction, col=Batch)) + geom_line() + facet_wrap(~Intron_id) 
# p
# 
# levels(dat$Batch) <- c("b1","b3","b2")
# gg_color_hue <- function(n) {
#   hues = seq(15, 375, length = n + 1)
#   hcl(h = hues, l = 65, c = 100)[1:n]
# }
# 
# dat$total_count <- dat$inclusion_count + dat$skipping_count
# 
# p <- ggplot(dat[dat$Symbol == "Nfkbie" & dat$Intron_length > 80 , ], aes(x=Time, y=PI_Junction, col=Batch)) 
# p <- p + geom_line()
# p <- p + geom_point(data = dat[dat$Symbol == "Nfkbie" & dat$Intron_length > 80 & dat$total_count > 10,] ) 
# p <- p + geom_point(data = dat[dat$Symbol == "Nfkbie" & dat$Intron_length > 80 & dat$total_count <= 10,] , alpha=0.3) 
# p <- p + facet_grid(Intron_id ~ . ) 
# p <- p + scale_y_continuous(limits = c(0,1), breaks = c(0,0.5,1)) + theme_bw() + labs(x = "Time (min)", y = "Nucleoplasmic Intron Retention", title="Nfkbie")
# p + scale_color_manual(values = c("b1"=gg_color_hue(3)[1],"b3"=gg_color_hue(3)[3] )) + theme(aspect.ratio=0.1)
# 
# p <- ggplot(dat[dat$Symbol == "Arl5c" & dat$Intron_length > 80 , ], aes(x=Time, y=PI_Junction, col=Batch)) 
# p <- p + geom_line()
# p <- p + geom_point(data = dat[dat$Symbol == "Arl5c" & dat$Intron_length > 80 & dat$total_count > 10,] ) 
# p <- p + geom_point(data = dat[dat$Symbol == "Arl5c" & dat$Intron_length > 80 & dat$total_count <= 10,] , alpha=0.3) 
# p <- p + facet_grid(Intron_id ~ . ) 
# p <- p + scale_y_continuous(limits = c(0,1), breaks = c(0,0.5,1)) + theme_bw() + labs(x = "Time (min)", y = "Nucleoplasmic Intron Retention", title="Arl5c")
# p + scale_color_manual(values = c("b1"=gg_color_hue(3)[1],"b3"=gg_color_hue(3)[3] )) + theme(aspect.ratio=0.1)
# 
# ## Model Change in FC vs HL
# input <- function(t, start = 20, end = 50 , amplitude = 100){
#   if (t < start | t > end) {
#     res <- 1
#   }else{
#     res <- amplitude
...
# }
# 
# fn_all <- function(t, y, par, input) {
#   dx <- par[1] * input(t) - par[2] * y[1]
#   dy <- par[3] * y[1] - par[4] * y[2]
#   dz <- par[5] * y[2] - par[6] * y[3]
#   return(list(c(dx,dy,dz)))
# }
# 
# fn_all_ss <- function(par){
#   return(par[1]/par[2] * c(1, par[3]/par[4] * c(1, par[5]/par[6])))
# }
# 
# library(deSolve)
# 
# par <- c(10, 1, 0.1,0.5,0.2,0.01) 
# tmp <- ode(y = fn_all_ss(par), times = 1:120, func = fn_all, parms = par, input = input)
# tmp
# 
# plot(tmp[,1], tmp[,4])
# 
...
# pt <- proc.time()
# for (i in 1:100){
#   par <- 10^c(1, 0, rnorm(1, -2, 1), -1, rnorm(1, -2, 1),  rnorm(1, -2, 1)) 
#   for(k in seq(-6,4, length.out = 101)){
#     tmp_par <- par * c(1,1,1,10^k,1,1)
#     tmp <- ode(y = fn_all_ss(tmp_par), times = 1:120, func = fn_all, parms = tmp_par, input = input)
#     dat <- rbind(dat, cbind(tmp, k, log2(max(tmp[,4])/tmp[1,4]) - log2(max(tmp[,2])/tmp[1,2])))
#     dat_FC <- rbind(dat_FC, c(Multiplier = k, K2p = tmp_par[4], FC_ca = log2(max(tmp[,2])/tmp[1,2]), FC_np = log2(max(tmp[,3])/tmp[1,3]), FC_cyto=log2(max(tmp[,4])/tmp[1,4])))
#   }
# }
# proc.time() - pt
# 
...
# 
# library(ggplot2)
# plot(dat_FC[,1], dat_FC[,4] - dat_FC[,3], xlab="log10(k2 multiplier)", ylab = "log2(FC_np) - log2(FC_ca)")
# 
# dat_FC <- cbind(dat_FC, Change = dat_FC[,4]-dat_FC[,3])
# p <- ggplot(as.data.frame(dat_FC), aes(x=Multiplier, y= Change)) + geom_point(alpha=0.01) 
# p + theme_bw() + theme(aspect.ratio = 1) + labs(x="log10(k2 multiplier)", y="Change in FC (np vs ca)", title="k2 vs Change in FC")
# 
# #
# dat <- c()
# dat_FC <- c()
...
# pt <- proc.time()
# for (i in 1:10){
#   par <- 10^c(1, -2, rnorm(1, -2, 1), rnorm(1, -2, 1.5), rnorm(1, -2, 1),  -2) 
#   for(k in seq(-6,4, length.out = 101)){
#     tmp_par <- par * c(1,1,1,1,1,10^k)
#     tmp <- ode(y = fn_all_ss(tmp_par), times = 1:120, func = fn_all, parms = tmp_par, input = input)
#     dat <- rbind(dat, cbind(tmp, k, log2(max(tmp[,4])/tmp[1,4]) - log2(max(tmp[,2])/tmp[1,2])))
#     dat_FC <- rbind(dat_FC, c(Multiplier = k, K2p = tmp_par[4], FC_ca = log2(max(tmp[,2])/tmp[1,2]), FC_np = log2(max(tmp[,3])/tmp[1,3]), FC_cyto=log2(max(tmp[,4])/tmp[1,4])))
#   }
...
# proc.time() - pt
# 
# dat_FC <- cbind(dat_FC, Change = dat_FC[,5]-dat_FC[,4])
# 
# p <- ggplot(as.data.frame(dat_FC), aes(x=Multiplier, y= Change, col=K2p)) + geom_point(alpha=0.8) 
# p <- p + scale_color_continuous(trans="log10", name="k2'")
# p + theme_bw() + theme(aspect.ratio = 1) + labs(x="log10(k_cyt_deg multiplier)", y="Change in FC (cyto vs np)", title="kcyt_deg vs Change in FC")
# 
# 
# 
# 
# ## Model Overall loss vs basal FC
# 
# ## Compare Less transport and more transport
# ensembl89 <- useMart(biomart = 'ensembl', host="may2017.archive.ensembl.org")
# 
# mm_89 <- useDataset(dataset = 'mmusculus_gene_ensembl', mart = ensembl89)
# 
# gene_list_mm89 <- getBM(attributes = c("ensembl_gene_id","external_gene_name", "version", "start_position", "end_position", "percentage_gene_gc_content"), filters = "ensembl_gene_id", values = substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb), 1,18), mart = mm_89) 
# 
# transcript_list_mm89 <- getBM(attributes = c("ensembl_gene_id","external_gene_name", "start_position", "end_position", "ensembl_transcript_id", "transcript_start", "transcript_end", "transcription_start_site", "5_utr_start", "5_utr_end", "3_utr_start", "3_utr_end", "cds_length", "transcript_biotype"), filters = "ensembl_gene_id", values = substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb), 1,18), mart = mm_89) 
# transcript_list_mm89_2 <- getBM(attributes = c("ensembl_gene_id","external_gene_name", "start_position", "end_position", "ensembl_transcript_id", "transcript_start", "transcript_end", "transcription_start_site", "transcript_biotype", "transcript_tsl", "transcript_appris"), filters = "ensembl_gene_id", values = substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb), 1,18), mart = mm_89) 
# exons_list_mm89 <- getBM(attributes = c("ensembl_gene_id","external_gene_name","strand", "start_position", "end_position", "ensembl_transcript_id", "transcript_start", "transcript_end", "transcription_start_site", "ensembl_exon_id", "cds_start", "cds_end", "exon_chrom_start", "exon_chrom_end", "rank","is_constitutive", "transcript_biotype"), filters = "ensembl_gene_id", values = substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb), 1,18), mart = mm_89) 
# 
# sequences_3utr_list_mm89 <- getBM(attributes = c("ensembl_gene_id","external_gene_name","strand", "start_position", "end_position", "ensembl_transcript_id", "transcript_start", "transcript_end", "transcript_biotype" , "3utr"), filters = "ensembl_gene_id", values = substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb), 1,18), mart = mm_89) 
# sequences_5utr_list_mm89 <- getBM(attributes = c("ensembl_gene_id","external_gene_name","strand", "start_position", "end_position", "ensembl_transcript_id", "transcript_start", "transcript_end", "transcript_biotype" , "5utr"), filters = "ensembl_gene_id", values = substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb), 1,18), mart = mm_89) 
# 
# sequences_3utr_list_mm89$Relative_transport <- NA
# sequences_3utr_list_mm89$Relative_transport[sequences_3utr_list_mm89$ensembl_gene_id %in%  substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18)[order(rel_transport)][which(sort(rel_transport) <= quantile(rel_transport, probs = 0.25))]] <- "less"
# sequences_3utr_list_mm89$Relative_transport[sequences_3utr_list_mm89$ensembl_gene_id %in%  substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18)[order(rel_transport,decreasing = T)][which(sort(rel_transport,decreasing = T) >= quantile(rel_transport, probs = 0.75))]] <- "more"
# 
# sequences_5utr_list_mm89$Relative_transport <- NA
# sequences_5utr_list_mm89$Relative_transport[sequences_5utr_list_mm89$ensembl_gene_id %in%  substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18)[order(rel_transport)][which(sort(rel_transport) <= quantile(rel_transport, probs = 0.25))]] <- "less"
# sequences_5utr_list_mm89$Relative_transport[sequences_5utr_list_mm89$ensembl_gene_id %in%  substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18)[order(rel_transport,decreasing = T)][which(sort(rel_transport,decreasing = T) >= quantile(rel_transport, probs = 0.75))]] <- "more"
# 
# 
# file.create('3utr_sequences.txt')
# for (g in unique(sequences_3utr_list_mm89$ensembl_gene_id)){
#   dat_g <- sequences_3utr_list_mm89[sequences_3utr_list_mm89$ensembl_gene_id == g, ]
#   dat_g2 <- dat_g[dat_g$transcript_biotype == "protein_coding",]
#   
#   if (nrow(dat_g2) > 0){
#     for(i in 1:nrow(dat_g2)){
#       cat(paste0('> ', paste(dat_g2$ensembl_gene_id[i], dat_g2$external_gene_name[i] , dat_g2$ensembl_transcript_id[i], dat_g2$Relative_transport[i] , sep = ' | '),'\n'), file = '3utr_sequences.txt', append = T)
#       cat(paste0(dat_g2[i,'3utr'], '\n'), file = '3utr_sequences_APPRIS.txt', append = T)
#     }
#   }else{
#     print(g)
#     cat(paste0('> ', paste(g, unique(dat_g$external_gene_name), unique(dat_g$Relative_transport) , sep = ' | '), '\n'), file = '3utr_sequences.txt', append = T)
#     cat('\n', file = '3utr_sequences_APPRIS.txt', append = T)
#   }
# }
# 
# file.create('5utr_sequences.txt')
# for (g in unique(sequences_5utr_list_mm89$ensembl_gene_id)){
#   dat_g <- sequences_5utr_list_mm89[sequences_5utr_list_mm89$ensembl_gene_id == g, ]
#   dat_g2 <- dat_g[dat_g$transcript_biotype == "protein_coding",]
#   
#   if (nrow(dat_g2) > 0){
#     for(i in 1:nrow(dat_g3)){
#       cat(paste0('> ', paste(dat_g2$ensembl_gene_id[i], dat_g2$external_gene_name[i] , dat_g2$ensembl_transcript_id[i], dat_g2$Relative_transport[i] , sep = ' | '),'\n'), file = '5utr_sequences.txt', append = T)
#       cat(paste0(dat_g2[i,'5utr'], '\n'), file = '5utr_sequences', append = T)
#     }
#   }else{
#     cat(paste0('> ', paste(g, unique(dat_g$external_gene_name), unique(dat_g$Relative_transport) , sep = ' | '), '\n'), file = '5utr_sequences.txt', append = T)
#     cat('\n', file = '5utr_sequences.txt', append = T)
...
# }
# 
# file.create('3utr_sequences_APPRIS.txt')
# for (g in unique(sequences_3utr_list_mm89$ensembl_gene_id)){
#   dat_g <- sequences_3utr_list_mm89[sequences_3utr_list_mm89$ensembl_gene_id == g, ]
#   dat_g2 <- dat_g[dat_g$transcript_biotype == "protein_coding",]
#   dat_g2$APPRIS <- transcript_list_mm89_2$transcript_appris[match(dat_g2$ensembl_transcript_id, transcript_list_mm89_2$ensembl_transcript_id)]
#   dat_g3 <- dat_g2[grep('principal', dat_g2$APPRIS),]
#   
#   if (nrow(dat_g3) > 0){
#     for(i in 1:nrow(dat_g3)){
#       cat(paste0('> ', paste(dat_g3$ensembl_gene_id[i], dat_g3$external_gene_name[i] , dat_g3$ensembl_transcript_id[i], dat_g3$Relative_transport[i] , sep = ' | '),'\n'), file = '3utr_sequences_APPRIS.txt', append = T)
#       cat(paste0(dat_g3[i,'3utr'], '\n'), file = '3utr_sequences_APPRIS.txt', append = T)
#     }
#   }else{
#     print(g)
#     cat(paste0('> ', paste(g, unique(dat_g$external_gene_name), unique(dat_g$Relative_transport) , sep = ' | '), '\n'), file = '3utr_sequences_APPRIS.txt', append = T)
...
# }
# 
# file.create('5utr_sequences_APPRIS.txt')
# for (g in unique(sequences_5utr_list_mm89$ensembl_gene_id)){
#   dat_g <- sequences_5utr_list_mm89[sequences_5utr_list_mm89$ensembl_gene_id == g, ]
#   dat_g2 <- dat_g[dat_g$transcript_biotype == "protein_coding",]
#   dat_g2$APPRIS <- transcript_list_mm89_2$transcript_appris[match(dat_g2$ensembl_transcript_id, transcript_list_mm89_2$ensembl_transcript_id)]
...
#   if (nrow(dat_g3) > 0){
#     for(i in 1:nrow(dat_g3)){
#       cat(paste0('> ', paste(dat_g3$ensembl_gene_id[i], dat_g3$external_gene_name[i] , dat_g3$ensembl_transcript_id[i], dat_g3$Relative_transport[i] , sep = ' | '),'\n'), file = '5utr_sequences_APPRIS.txt', append = T)
#       cat(paste0(dat_g3[i,'5utr'], '\n'), file = '5utr_sequences_APPRIS.txt', append = T)
#     }
#   }else{
#     cat(paste0('> ', paste(g, unique(dat_g$external_gene_name), unique(dat_g$Relative_transport) , sep = ' | '), '\n'), file = '5utr_sequences_APPRIS.txt', append = T)
#     cat('\n', file = '5utr_sequences_APPRIS.txt', append = T)
...
# 
# 
# gene_list_mm89$transcript_length_avg <- NA
# gene_list_mm89$UTR5_length_avg <- NA
# gene_list_mm89$UTR3_length_avg <- NA
# gene_list_mm89$CDS_length_avg <- NA
# gene_list_mm89$ratio_CDS_length_avg <- NA
# gene_list_mm89$intron_number <- NA
# gene_list_mm89$max_intron_length <- NA
# gene_list_mm89$max_intron_ratio <- NA
# gene_list_mm89$first_intron_length <- NA
# gene_list_mm89$last_intron_length <- NA
# gene_list_mm89$first_intron_ratio <- NA
# gene_list_mm89$last_intron_ratio <- NA
# 
# for (g in unique(gene_list_mm89$ensembl_gene_id)){
#   dat_gt <- transcript_list_mm89[transcript_list_mm89$ensembl_gene_id == g & transcript_list_mm89$transcript_biotype == "protein_coding", ]
#   dat_ge <- exons_list_mm89[exons_list_mm89$ensembl_gene_id == g & exons_list_mm89$transcript_biotype == "protein_coding", ]
#   
#   t_length <- t_5utr_length <- t_3utr_length <- t_cds_length <- t_ratio_cds_length <- t_intron_number <- t_max_intron_length <- t_max_intron_ratio <- t_first_intron_length <- t_last_intron_length <- t_first_intron_length_ratio <- t_last_intron_length_ratio <- c()
#   
#   for (transcript in unique(dat_gt$ensembl_transcript_id)){
#     dat_t <- dat_gt[dat_gt$ensembl_transcript_id == transcript,]
#     dat_te <- dat_ge[dat_ge$ensembl_transcript_id == transcript,]
#     dat_te <- dat_te[order(dat_te$rank),]
#     
#     t_length <- c(t_length, unique(dat_t$transcript_end - dat_t$transcript_start + 1))
#     t_5utr_length <- c(t_5utr_length , unique(dat_t[,"5_utr_end"] - dat_t[,"5_utr_start"] + 1))
#     t_3utr_length <- c(t_3utr_length , unique(dat_t[,"3_utr_end"] - dat_t[,"3_utr_start"] + 1))
#     t_cds_length <- c(t_cds_length, unique(dat_t$cds_length))
#     t_ratio_cds_length <- c(t_ratio_cds_length, unique(dat_t$cds_length/(dat_t$transcript_end - dat_t$transcript_start + 1)))
#     t_intron_number <- c(t_intron_number, max(dat_te$rank)-1)
#     
#     if(max(dat_te$rank)-1 > 0){ 
#       if(unique(dat_te$strand) == 1){
#         introns_length <- dat_te$exon_chrom_start[-1] - dat_te$exon_chrom_end[-nrow(dat_te)] + 1
#       }else{
#         introns_length <- dat_te$exon_chrom_start[-nrow(dat_te)] - dat_te$exon_chrom_end[-1] + 1
#       }
#       
#       t_max_intron_length <- c(t_max_intron_length, max(introns_length))
#       t_max_intron_ratio <- c(t_max_intron_ratio, max(introns_length/unique(dat_t$transcript_end - dat_t$transcript_start + 1)))
#       t_first_intron_length <- c(t_first_intron_length, introns_length[1])
#       t_last_intron_length <- c(t_last_intron_length, introns_length[length(introns_length)])
#       t_first_intron_length_ratio <- c(t_first_intron_length_ratio, introns_length[1]/unique(dat_t$transcript_end - dat_t$transcript_start + 1))
#       t_last_intron_length_ratio <- c(t_last_intron_length_ratio, introns_length[length(introns_length)]/unique(dat_t$transcript_end - dat_t$transcript_start + 1))
#     }else{
#       t_max_intron_length <-c(t_max_intron_length, NA)
#       t_max_intron_ratio <- c(t_max_intron_ratio, NA)
#       t_first_intron_length <- c(t_first_intron_length, NA)
#       t_last_intron_length <- c(t_last_intron_length, NA)
#       t_first_intron_length_ratio <- c(t_first_intron_length_ratio, NA)
#       t_last_intron_length_ratio <- c(t_last_intron_length_ratio, NA)
#     }
#   }  
#   
#   gene_list_mm89[gene_list_mm89$ensembl_gene_id == g, "transcript_length_avg"] <- mean(t_length, na.rm = T)
#   gene_list_mm89[gene_list_mm89$ensembl_gene_id == g, "UTR5_length_avg"] <- mean(t_5utr_length, na.rm = T)
#   gene_list_mm89[gene_list_mm89$ensembl_gene_id == g, "UTR3_length_avg"] <- mean(t_3utr_length, na.rm = T)
#   gene_list_mm89[gene_list_mm89$ensembl_gene_id == g, "CDS_length_avg"] <- mean(t_cds_length, na.rm = T)
#   gene_list_mm89[gene_list_mm89$ensembl_gene_id == g, "ratio_CDS_length_avg"] <- mean(t_ratio_cds_length, na.rm = T)
#   gene_list_mm89[gene_list_mm89$ensembl_gene_id == g, "intron_number"] <- mean(t_intron_number, na.rm = T)
#   if(length(t_intron_number) == 1 ){
#     gene_list_mm89[gene_list_mm89$ensembl_gene_id == g, "max_intron_length"] <- max(t_max_intron_length)
#     gene_list_mm89[gene_list_mm89$ensembl_gene_id == g, "max_intron_ratio"] <- mean(t_max_intron_ratio)
#     gene_list_mm89[gene_list_mm89$ensembl_gene_id == g, "first_intron_length"] <- mean(t_first_intron_length)
#     gene_list_mm89[gene_list_mm89$ensembl_gene_id == g, "last_intron_length"] <- mean(t_last_intron_length)
#     gene_list_mm89[gene_list_mm89$ensembl_gene_id == g, "first_intron_length_ratio"] <- mean(t_first_intron_length_ratio)
#     gene_list_mm89[gene_list_mm89$ensembl_gene_id == g, "last_intron_length_ratio"] <- mean(t_last_intron_length_ratio)
#   }else{
#     gene_list_mm89[gene_list_mm89$ensembl_gene_id == g, "max_intron_length"] <- max(t_max_intron_length, na.rm = T)
#     gene_list_mm89[gene_list_mm89$ensembl_gene_id == g, "max_intron_ratio"] <- mean(t_max_intron_ratio, na.rm = T)
#     gene_list_mm89[gene_list_mm89$ensembl_gene_id == g, "first_intron_length"] <- mean(t_first_intron_length, na.rm = T)
#     gene_list_mm89[gene_list_mm89$ensembl_gene_id == g, "last_intron_length"] <- mean(t_last_intron_length, na.rm = T)
#     gene_list_mm89[gene_list_mm89$ensembl_gene_id == g, "first_intron_length_ratio"] <- mean(t_first_intron_length_ratio, na.rm = T)
#     gene_list_mm89[gene_list_mm89$ensembl_gene_id == g, "last_intron_length_ratio"] <- mean(t_last_intron_length_ratio, na.rm = T)
#   }
# } 
# 
# 
...
# 
# 
# # Correlation loss with half-life
# rel_transport <- par_best_naive_old[,2]+par_best_naive_old[,4]-par_best_naive_old[,3]
# plot(rel_transport, par_best_naive_old[,5], xlim=c(-5,0.5), ylim=c(-4.5,0), xlab='Relative transport', ylab='log10(decay)')
# quantile(rel_transport, probs = c(0.25,0.75))
# lines(x=rep(quantile(rel_transport, probs = c(0.25,0.75))[1],2),y=c(-10,10), col='red', lty=2)
# lines(x=rep(quantile(rel_transport, probs = c(0.25,0.75))[2],2),y=c(-10,10), col='red', lty=2)
# 
# dat <- data.frame(Rel_transport = rel_transport, Decay = par_best_naive_old[,5])
# p <- ggplot(dat, aes(x=Rel_transport, y=Decay)) + geom_point() 
# p <- p + geom_vline(xintercept = quantile(rel_transport, probs = c(0.25,0.75))[1], color="red", linetype=2) + geom_vline(xintercept = quantile(rel_transport, probs = c(0.25,0.75))[2], color="red", linetype=2)
# p <- p + theme_bw() + coord_cartesian(xlim=c(-3,0.5), ylim=c(-3,-0.5))
# p <- p + labs(y= "Cytoplamic Decay", x=TeX("$log_{10}(\\,\\frac{k_1' \\, k_2'}{k_2}\\,)$"), title = "Relative Overall Transport vs Intron Retention - LPA", col="Length (kb)")
# p + theme(aspect.ratio = 1) + scale_y_continuous(limits=c(-5,0.5))
# 
# 
# # boxplot comparison 
# dat <- data.frame(Genes = rownames(caRNA_Naive_exons_rpkm_D40000_5kb), 
#                   Relative_transport=rel_transport, 
#                   decay=par_best_naive_old[,5], 
#                   PI = PI_naive_max_avg_late$PI_max[match(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),PI_naive_max_avg_late[,1])], 
#                   group = replace(replace(rep("Middle", length(rel_transport)),rel_transport >= quantile(rel_transport, probs = c(0.25,0.75))[2],"Upper"),rel_transport <= quantile(rel_transport, probs = c(0.25,0.75))[1],"Lower"), 
#                   percentile = 25, 
#                   Length = as.vector(gene_list_mm89$end_position - gene_list_mm89$start_position)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)]/1000,
#                   GC = as.vector(gene_list_mm89$percentage_gene_gc_content)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)],
#                   UTR3 = as.vector(gene_list_mm89$UTR3_length_avg)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)], 
#                   UTR5 = as.vector(gene_list_mm89$UTR5_length_avg)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)],
#                   CDS = as.vector(gene_list_mm89$CDS_length_avg)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)],
#                   CDS_ratio = as.vector(gene_list_mm89$ratio_CDS_length_avg)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)],
#                   intron_number = as.vector(gene_list_mm89$intron_number)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)],
#                   max_intron_length = as.vector(gene_list_mm89$max_intron_length)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)]/1000,
#                   max_intron_ratio = as.vector(gene_list_mm89$max_intron_ratio)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)],
#                   first_intron_length = as.vector(gene_list_mm89$first_intron_length)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)]/1000,
#                   last_intron_length = as.vector(gene_list_mm89$last_intron_length)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)]/1000,
#                   first_intron_length_ratio = as.vector(gene_list_mm89$first_intron_length_ratio)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)],
#                   last_intron_length_ratio = as.vector(gene_list_mm89$last_intron_length_ratio)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)])
# # dat <- rbind(dat, data.frame(Genes = rownames(caRNA_Naive_exons_rpkm_D40000_5kb), Relative_transport=rel_transport, decay=par_best_naive_old[,5], 
# #                              PI = PI_naive_max_avg_late$PI_max[match(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),PI_naive_max_avg_late[,1])], 
# #                              group = replace(replace(rep("Middle", length(rel_transport)),rel_transport >= quantile(rel_transport, probs = c(0.1,0.9))[2],"Upper"),rel_transport <= quantile(rel_transport, probs = c(0.1,0.9))[1],"Lower"), 
# #                              percentile = 10, Length=as.vector(gene_list_mm89$end_position - gene_list_mm89$start_position)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)]/1000,
# #                              GC = as.vector(gene_list_mm89$percentage_gene_gc_content)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)],
# #                              UTR3 = as.vector(gene_list_mm89$UTR3_length_avg)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)], 
# #                              UTR5 = as.vector(gene_list_mm89$UTR5_length_avg)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)],
# #                              CDS = as.vector(gene_list_mm89$CDS_length_avg)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)],
...
# # dat <- rbind(dat, data.frame(Genes = rownames(caRNA_Naive_exons_rpkm_D40000_5kb), Relative_transport=rel_transport, decay=par_best_naive_old[,5], 
# #                              PI = PI_naive_max_avg_late$PI_max[match(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),PI_naive_max_avg_late[,1])], 
# #                              group = replace(replace(rep("Middle", length(rel_transport)),rel_transport >= quantile(rel_transport, probs = c(1/3,2/3))[2],"Upper"),rel_transport <= quantile(rel_transport, probs = c(1/3,2/3))[1],"Lower"), 
# #                              percentile = 33, Length = as.vector(gene_list_mm89$end_position - gene_list_mm89$start_position)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)]/1000,
# #                              GC = as.vector(gene_list_mm89$percentage_gene_gc_content)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)],
# #                              UTR3 = as.vector(gene_list_mm89$UTR3_length_avg)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)], 
# #                              UTR5 = as.vector(gene_list_mm89$UTR5_length_avg)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)],
# #                              CDS = as.vector(gene_list_mm89$CDS_length_avg)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)],
...
# #                              first_intron_length_ratio = as.vector(gene_list_mm89$first_intron_length_ratio)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)],
# #                              last_intron_length_ratio = as.vector(gene_list_mm89$last_intron_length_ratio)[match(substr(rownames(caRNA_Naive_exons_rpkm_D40000_5kb),1,18), gene_list_mm89$ensembl_gene_id)]))
# dat$PI[which(is.infinite(dat$PI))] <- NA
# 
# # add intron number
# # max intron length
# # max intron length / total length
# 
# library('ggpubr')
# p <- ggplot(dat, aes(x=group, y=decay, fill=group)) + geom_boxplot() + facet_wrap(~percentile,drop = TRUE)
# p <- p + stat_compare_means(comparisons = list(c("Lower", "Upper")),method = "wilcox.test", label.y = -0.1) + theme_bw()
# p <- p + labs(x='Transport', y='log10(decay)') + scale_y_continuous(limits=c(-3.5,0))
# p + theme(aspect.ratio = 1)
# 
# p <- ggplot(dat, aes(x=group, y=PI, fill=group)) + geom_boxplot() + facet_wrap(~percentile,drop = TRUE)
# p <- p + stat_compare_means(comparisons = list(c("Lower", "Upper")),method = "t.test") + theme_bw()
# p <- p + labs(x='Transport', y='PI')
# p
# 
# p <- ggplot(dat, aes(x=group, y=PI, fill=group)) + geom_boxplot() + facet_wrap(~percentile,drop = TRUE)
# p <- p + stat_compare_means(comparisons = list(c("Lower", "Upper")),method = "wilcox.test") + theme_bw()
# p <- p + labs(x='Transport', y='PI')
# p
# 
# p <- ggplot(dat, aes(x=group, y=Length, fill=group)) + geom_boxplot() + facet_wrap(~percentile,drop = TRUE)
# p <- p + stat_compare_means(comparisons = list(c("Lower", "Upper")),method = "wilcox.test") + theme_bw()
# p <- p + labs(x='Transport', y='Length (kb)') + scale_y_continuous(trans="log10") + theme(aspect.ratio = 1)
# p
# 
# p <- ggplot(dat, aes(x=group, y=GC, fill=group)) + geom_boxplot() + facet_wrap(~percentile,drop = TRUE)
# p <- p + stat_compare_means(comparisons = list(c("Lower", "Upper")),method = "wilcox.test") + theme_bw()
# p <- p + labs(x='Transport', y="GC %")
# p
# 
# p <- ggplot(dat, aes(x=group, y=UTR3, fill=group)) + geom_boxplot() + facet_wrap(~percentile,drop = TRUE)
# p <- p + stat_compare_means(comparisons = list(c("Lower", "Upper")),method = "wilcox.test") + theme_bw()
# p <- p + labs(x='Transport', y="3'UTR Length")
# p
# 
# p <- ggplot(dat, aes(x=group, y=UTR5, fill=group)) + geom_boxplot() + facet_wrap(~percentile,drop = TRUE)
# p <- p + stat_compare_means(comparisons = list(c("Lower", "Upper")),method = "wilcox.test") + theme_bw()
# p <- p + labs(x='Transport', y="5'UTR Length")
# p
# 
# p <- ggplot(dat, aes(x=group, y=CDS, fill=group)) + geom_boxplot() + facet_wrap(~percentile,drop = TRUE)
# p <- p + stat_compare_means(comparisons = list(c("Lower", "Upper")),method = "wilcox.test") + theme_bw()
# p <- p + labs(x='Transport', y="CDS Length")
# p
# 
# p <- ggplot(dat, aes(x=group, y=CDS_ratio, fill=group)) + geom_boxplot() + facet_wrap(~percentile,drop = TRUE)
# p <- p + stat_compare_means(comparisons = list(c("Lower", "Upper")),method = "wilcox.test") + theme_bw()
# p <- p + labs(x='Transport', y="CDS Length / Total Length")
# p
# 
# p <- ggplot(dat, aes(x=group, y=intron_number, fill=group)) + geom_boxplot() + facet_wrap(~percentile,drop = TRUE)
# p <- p + stat_compare_means(comparisons = list(c("Lower", "Upper")),method = "wilcox.test", label.y = 35) + theme_bw()
# p <- p + labs(x='Transport', y="Intron number - Max") + scale_y_continuous(limits=c(0,40))
# p + theme(aspect.ratio = 1)
# p
# 
# p <- ggplot(dat, aes(x=Relative_transport, y=intron_number)) + geom_point()
# p <- p + stat_compare_means(comparisons = list(c("Lower", "Upper")),method = "wilcox.test") + theme_bw()
# p <- p + labs(x='Transport', y="Intron number") + scale_y_continuous(limits=c(0,40))
# p + theme(aspect.ratio = 1)
# 
# 
# p <- ggplot(dat, aes(x=group, y=max_intron_length, fill=group)) + geom_boxplot() + facet_wrap(~percentile,drop = TRUE)
# p <- p + stat_compare_means(comparisons = list(c("Lower", "Upper")),method = "wilcox.test") + theme_bw()
# p <- p + labs(x='Transport', y="Max Intron Length (kb)")
# p
# 
# p <- ggplot(dat, aes(x=group, y=max_intron_ratio, fill=group)) + geom_boxplot() + facet_wrap(~percentile,drop = TRUE)
# p <- p + stat_compare_means(comparisons = list(c("Lower", "Upper")),method = "wilcox.test") + theme_bw()
# p <- p + labs(x='Transport', y="Max Intron Length / Total Length")
# p
# 
# p <- ggplot(dat, aes(x=group, y=first_intron_length, fill=group)) + geom_boxplot() + facet_wrap(~percentile,drop = TRUE)
# p <- p + stat_compare_means(comparisons = list(c("Lower", "Upper")),method = "wilcox.test") + theme_bw()
# p <- p + labs(x='Transport', y="First Intron Length (kb)")
# p
# 
# p <- ggplot(dat, aes(x=group, y=first_intron_length_ratio, fill=group)) + geom_boxplot() + facet_wrap(~percentile,drop = TRUE)
# p <- p + stat_compare_means(comparisons = list(c("Lower", "Upper")),method = "wilcox.test") + theme_bw()
# p <- p + labs(x='Transport', y="First Intron Length / Total Length")
# p
# 
# p <- ggplot(dat, aes(x=group, y=last_intron_length, fill=group)) + geom_boxplot() + facet_wrap(~percentile,drop = TRUE)
# p <- p + stat_compare_means(comparisons = list(c("Lower", "Upper")),method = "wilcox.test") + theme_bw()
# p <- p + labs(x='Transport', y="Last Intron Length (kb)")
# p
# 
# p <- ggplot(dat, aes(x=group, y=last_intron_length_ratio, fill=group)) + geom_boxplot() + facet_wrap(~percentile,drop = TRUE)
# p <- p + stat_compare_means(comparisons = list(c("Lower", "Upper")),method = "wilcox.test") + theme_bw()
# p <- p + labs(x='Transport', y="Last Intron Length / Total Length")
# p
# 
# # 25% less Intron retention
# # 25% more Intron retention
# 
# # Look at sequence for 5'utr in the less loss
# # 25% less transport (i.e. more loss)
# rownames(caRNA_Naive_exons_rpkm_D40000_5kb)[order(rel_transport)][which(sort(rel_transport) <= quantile(rel_transport, probs = 0.25))]
# 
# # 25% more transport (i.e. less loss)
# paste(sapply(rownames(caRNA_Naive_exons_rpkm_D40000_5kb)[which(rel_transport >= quantile(rel_transport, probs = 0.75))], function(g){unique(gene_infos$external_gene_name[gene_infos$ensembl_gene_id == substr(g,1,18)])}), collapse=" ")
# 
# 
# ## Compare timing and loss between ca and cyto
# library(pracma)
# source(file = '../../../../Functions/xcorr.R')
# 
# Time_ca_Naive <- as.numeric(sapply(colnames(caRNA_Naive_exons_rpkm_D40000_5kb), function(x){strsplit(x,fixed = T, split = ".")[[1]][3]}))
# Batch_ca_Naive <- sapply(colnames(caRNA_Naive_exons_rpkm_D40000_5kb), function(x){strsplit(x,fixed = T, split = ".")[[1]][4]})
# 
# Time_np_Naive <- as.numeric(sapply(colnames(npRNA_Naive_exons_rpkm_D40000_5kb), function(x){strsplit(x,fixed = T, split = ".")[[1]][3]}))
# Batch_np_Naive <- sapply(colnames(npRNA_Naive_exons_rpkm_D40000_5kb), function(x){strsplit(x,fixed = T, split = ".")[[1]][4]})
# 
# Time_cyto_Naive <- as.numeric(sapply(colnames(cytoRNA_Naive_exons_rpkm_D40000_5kb), function(x){strsplit(x,fixed = T, split = ".")[[1]][3]}))
# Batch_cyto_Naive <- sapply(colnames(cytoRNA_Naive_exons_rpkm_D40000_5kb), function(x){strsplit(x,fixed = T, split = ".")[[1]][4]})
# 
# t_ca2np <- t_np2cyto <- t_ca2cyto <- c()
# cor_ca2np <- cor_np2cyto <- cor_ca2cyto <- c()
# 
# for (gene in rownames(caRNA_Naive_exons_rpkm_D40000_5kb)){
#   dat_ca <- predict(loess(caRNA_Naive_exons_rpkm_D40000_5kb[rownames(caRNA_Naive_exons_rpkm_D40000_5kb) == gene,] ~ Time_ca_Naive), newdata = 0:120)
#   dat_np <- predict(loess(npRNA_Naive_exons_rpkm_D40000_5kb[rownames(npRNA_Naive_exons_rpkm_D40000_5kb) == gene,] ~ Time_np_Naive), newdata = 0:120)
#   dat_cyto <- predict(loess(cytoRNA_Naive_exons_rpkm_D40000_5kb[rownames(cytoRNA_Naive_exons_rpkm_D40000_5kb) == gene,] ~ Time_cyto_Naive), newdata = 0:120)
#   
#   tmp_ca2np <- xcorr_derivative(dat_np, dat_ca, lag = 0:60, type = 'coeff')
#   tmp_np2cyto <- xcorr_derivative(dat_cyto, dat_np, lag = 0:60, type = 'coeff')
#   tmp_ca2cyto <- xcorr_derivative(dat_cyto, dat_ca, lag = 0:60, type = 'coeff')
#   
#   cor_ca2np <- c(cor_ca2np, max(tmp_ca2np))
#   t_ca2np <- c(t_ca2np, names(tmp_ca2np)[which.max(tmp_ca2np)])
#   cor_np2cyto <- c(cor_np2cyto, max(tmp_np2cyto))
#   t_np2cyto <- c(t_np2cyto, names(tmp_np2cyto)[which.max(tmp_np2cyto)])
#   cor_ca2cyto <- c(cor_ca2cyto, max(tmp_ca2cyto))
#   t_ca2cyto <- c(t_ca2cyto, names(tmp_ca2cyto)[which.max(tmp_ca2cyto)])
# }
# 
# 
# plot(as.numeric(t_ca2cyto), rel_transport, main="Correlation of delay and relative transport", ylim=c(-5,1), xlab="Delay between chromatin and cytoplasm\nObtained by cross-correlation between the derivatives of the interpolated values", ylab="Relative transport")
# cor(as.numeric(t_ca2cyto)[-which(rel_transport >= 5)], rel_transport[-which(rel_transport >= 5)])
# 
# library(ggplot2)
# library(scales)
# dat <- data.frame(obs_delay = as.numeric(t_ca2cyto), rel_tranpsort = rel_transport, decay=par_best_naive_old[,5])
# p <- ggplot(dat, aes(x=obs_delay, y=rel_transport, col=decay)) + geom_point()
# p <- p + theme_bw() + coord_cartesian(ylim=c(-3,0.5), xlim=c(0,50))
# p <- p + labs(x = "Observed Delay between Chromatin and Cytoplasm", y=TeX("$log_{10}(\\,\\frac{k_1' \\, k_2'}{k_2}\\,)$"), title = "Relative Overall Transport vs Observed Delay", col="Decay rate")
# p + theme(aspect.ratio = 1) + scale_color_continuous(limits=c(-3,0), oob=squish)
# 
# p <- ggplot(dat, aes(x=obs_delay, y=decay, col=rel_transport)) + geom_point()
# p <- p + theme_bw() + coord_cartesian(ylim=c(-5,-0.5), xlim=c(0,50))
# p <- p + labs(x = "Observed Delay between Chromatin and Cytoplasm", y="Decay rate constant", col=TeX("$log_{10}(\\,\\frac{k_1' \\, k_2'}{k_2}\\,)$"), title = "Decay vs Observed Delay")
# p + theme(aspect.ratio = 1) + scale_color_continuous(limits=c(-3,0.5), oob=squish)
# 
# 
# 
# 
...
#     dat_ggplot$whichPar[dat_ggplot$whichPar %in% c("k2", "kdeg") & dat_ggplot$batch == "b2"] <- "k2 or kdeg"
#     dat_ggplot$whichPar <- factor(dat_ggplot$whichPar, 
#                                  levels = c("k1'", "k2", "k2'", "kdeg", "k1'k2'", "k2 or kdeg", "k1'/k2", "k1'k2'/k2"),
#                                  labels = c("log[10](k[1]*minute)", "log[10](k[2])", "log[10](k[2]*minute)", "log[10](k[deg])", "log[10](paste(k[1]*minute, k[2]*minute))", "log[10](paste(k[2], ' or ', k[deg]))", "log[10](paste(k[1]*minute/k[2]))", "log[10](paste(k[1]*minute,k[2]*minute)/k[2])"))
#     
#     dat_CI$whichPar <- as.character(dat_CI$whichPar)
#     dat_CI$whichPar[dat_CI$whichPar %in% c("min(k2,kdeg)", "max(k2,kdeg)") & dat_CI$batch == "b2"] <- "k2 or kdeg"
#     dat_CI$whichPar <- factor(dat_CI$whichPar, 
